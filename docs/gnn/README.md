# GNN Processing Core Documentation

## Overview

The GNN Processing Core is a comprehensive system for processing graphs using various Graph Neural Network architectures. It provides a flexible, scalable, and production-ready framework for graph-based machine learning tasks.
...
from src.gnn.batch_processor import GraphBatchProcessor

# Parse a GNN model definition
parser = GNNParser()
model_def = parser.parse_file("models/base/explorer.gnn.md")
...
The parser reads and validates `.gnn.md` model definition files:

```python
from src.gnn.parser import GNNParser

parser = GNNParser()
model = parser.parse_file("path/to/model.gnn.md")
...
```python
from src.gnn.monitoring_dashboard import create_dashboard
...
```python
from src.gnn.alerting import get_alert_manager, AlertRule, AlertType, AlertSeverity
...
```python
from src.gnn.monitoring import monitor_performance, log_operation
...
```python
import logging
from src.gnn.monitoring import get_logger
...
```python
from src.gnn.layers import MessagePassing
