...
Policy evaluation and action selection using expected free energy.
...
##### `DiscreteExpectedFreeEnergy(PolicySelector)`
Expected free energy calculation for discrete models.

**Methods:**
- `compute_expected_free_energy(beliefs, A, B, C, num_actions, horizon)` - EFE calculation
- `evaluate_policies(beliefs, generative_model)` - Policy evaluation
- `select_action(policy_values)` - Softmax action selection

**Example:**
```python
from inference.engine import DiscreteExpectedFreeEnergy, PolicyConfig

config = PolicyConfig(precision=2.0, planning_horizon=3)
policy_selector = DiscreteExpectedFreeEnergy(config)
action_probs = policy_selector.evaluate_policies(beliefs, generative_model)
...
# Create EFE policy selector
policy_selector = create_policy_selector('discrete_efe', policy_config)
```
...
for t in range(100):
    ...
    # Select action
    G_values = policy_selector.evaluate_policies(belief, model)
    action = policy_selector.select_action(G_values)
