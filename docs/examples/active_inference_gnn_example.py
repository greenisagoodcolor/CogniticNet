...
from inference.engine import (
    ...
    GNNActiveInferenceAdapter, GNNIntegrationConfig
)
...
class GraphEnvironment:
    ...
    def __init__(self, num_nodes: int = 10, edge_prob: float = 0.3):
...
class GraphGNN(nn.Module):
    ...
    def __init__(self, input_dim: int = 4, hidden_dim: int = 32, output_dim: int = 16):
        super().__init__()
...
class GraphActiveInferenceAgent:
    ...
    def __init__(self, env: GraphEnvironment):
        ...
        self.policy_selector = DiscreteExpectedFreeEnergy(self.policy_config)
...
def run_gnn_active_inference_episode(env: GraphEnvironment, agent: GraphActiveInferenceAgent,
                                   max_steps: int = 20) -> Tuple[List[int], List[torch.Tensor]]:
    ...
    for step in range(max_steps):
        ...
        print(f"Step {step}: Node {env.current_node}, Action {action}, "
...
def plot_comparison_results(results: dict):
    ...
    ax1.bar(['Standard', 'GNN-Enhanced'], [standard_success, gnn_success],
    ...
    # Add value labels
    ax1.text(0, standard_success + 0.02, f'{standard_success:.2f}',
             ha='center', va='bottom')
    ax1.text(1, gnn_success + 0.02, f'{gnn_success:.2f}',
             ha='center', va='bottom')

    # Steps comparison (for successful episodes only)
    standard_steps = [s for s, success in zip(results['standard']['steps'],
                                            results['standard']['success']) if success]
    gnn_steps = [s for s, success in zip(results['gnn_enhanced']['steps'],
