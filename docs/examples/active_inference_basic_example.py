...
def run_active_inference_episode(env: SimpleGridWorld, model: DiscreteGenerativeModel,
                                max_steps: int = 20) -> Tuple[List[int], List[torch.Tensor], List[int]]:
    ...
    for step in range(max_steps):
        ...
        print(f"Step {step}: Pos={env.agent_pos}, Obs={observation}, "
...
def plot_free_energy(beliefs: List[torch.Tensor], observations: List[int],
