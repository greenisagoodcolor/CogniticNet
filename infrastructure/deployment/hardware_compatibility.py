...
class HardwareDetector:
    ...
    def _determine_profile_name(
        self, arch: str, ram_gb: float, gpu_available: bool
    ) -> str:
...
class CompatibilityTester:
    ...
    def _test_cpu_performance(
        ...
        duration = time.time() - start_time
        ops_per_second = (
            (iterations * size * size * size * 2) / duration / 1e9
        )  # GFLOPS
    ...
    def _test_memory_allocation(
        ...
        for size_mb in test_sizes_mb:
            if (
                size_mb > hardware_profile.ram_gb * 1024 * 0.5
            ):  # Don't exceed 50% of RAM
                break
    ...
    def _test_resource_limits(
        ...
        try:
            ...
            if (
                limits["max_memory"][0] != -1
                and limits["max_memory"][0] < 1024 * 1024 * 1024
            ):
