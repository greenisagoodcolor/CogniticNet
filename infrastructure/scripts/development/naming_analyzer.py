...
# Import from kebab-case file name
spec = importlib.util.spec_from_file_location("metadata_extractor", Path(__file__).parent / "metadata-extractor.py")
metadata_extractor = importlib.util.module_from_spec(spec)
spec.loader.exec_module(metadata_extractor)
ExtendedMetadata = metadata_extractor.ExtendedMetadata
except ImportError:
    from traversal import FileInfo, FileType
