...
class AnomalyDetector:
    ...
    def __init__(self, sensitivity: float = 0.1, history_window: int = 1000):
        self.sensitivity = sensitivity
        self.history_window = history_window

        # Statistical models
        self.isolation_forest = IsolationForest(
            contamination=sensitivity, random_state=42
        )
    ...
    def detect_anomalies(
        ...
        return AnomalyReport(
            anomalies=anomalies, summary_stats=summary_stats, risk_level=risk_level
        )

    def _detect_behavior_anomalies(self, agent_state: Dict[str, Any]) -> List[Anomaly]:
            ...
            if fe > self.thresholds["free_energy_spike"]:
                anomalies.append(
                    Anomaly(
                        ...
                        severity=min(
                            1.0, fe / (2 * self.thresholds["free_energy_spike"])
                        ),
            ...
            if loss_change > self.thresholds["learning_rate_spike"]:
                anomalies.append(
                    Anomaly(
                        ...
                        severity=min(
                            1.0,
                            loss_change / (2 * self.thresholds["learning_rate_spike"]),
                        ),
    ...
    def train_detector(self, historical_data: List[Dict[str, Any]]) -> None:
        """
    ...
    def _update_history(
        ...
        if learning_metrics:
            self.metric_history.append(
                {"timestamp": datetime.utcnow(), "metrics": learning_metrics.copy()}
            )
