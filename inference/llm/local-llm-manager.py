...
class LocalLLMManager:
    ...
    def _initialize_providers(self):
        ...
        if (
            self.config.provider == LocalLLMProvider.LLAMA_CPP
            or not self.current_provider
        ):
    ...
    def _try_generate(
        self, provider: LocalLLMProvider, prompt: str, **kwargs
    ) -> LLMResponse:
    ...
    def optimize_for_hardware(
        ...
        config = LocalLLMConfig(
            provider=self.config.provider, model_name=self.config.model_name
        )
