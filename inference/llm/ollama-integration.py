...
class OllamaManager:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.delete(
                    f"{self.api_base}/delete", json=data
                ) as response:
                    return response.status == 200
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.api_base}/generate", json=data
                ) as response:
                    if response.status != 200:
    ...
    async def create_embedding(self, model: str, prompt: str) -> List[float]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.api_base}/embeddings", json=data
                ) as response:
                    if response.status == 200:
                        result = await response.json()
    ...
    def recommend_model(
        self, ram_gb: float, storage_gb: float, use_case: str = "general"
    ) -> str:
        """
...
Be vigilant, protective, and responsive.""",
    ...
    async def think(
        self, context: str, options: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        ...
        async for chunk in self.manager.generate(
            model, context, stream=True, **(options or {})
        ):
