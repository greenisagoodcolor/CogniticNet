    ...
    def _apply_pattern_to_model(
        ...
        if pattern_type == "successful_action_sequence":
            if "C_pref" in model["connections"]:
                ...
                if action and action not in model["connections"]["C_pref"].get(
                    "action_biases", {}
                ):
                    if "action_biases" not in model["connections"]["C_pref"]:
                        model["connections"]["C_pref"]["action_biases"] = {}
                    bias = pattern.get("success_rate", 0.5) * pattern.get(
                        "confidence", 0.8
                    )
                    model["connections"]["C_pref"]["action_biases"][action] = round(
                        bias, 3
                    )
            ...
            if (
                "C_pref" in model["connections"]
                and "preferences" in model["connections"]["C_pref"]
            ):
                ...
                for pref, adjustment in adjustments.items():
                    if pref in model["connections"]["C_pref"]["preferences"]:
                        ...
                        model["connections"]["C_pref"]["preferences"][pref] = round(
                            new_value, 3
                        )
                        change_record["changes"].append(
                            {
