...
class CacheManager:
    ...
    def __init__(self, cache_dir: str='.cache/gnn', max_memory_mb: int=1024, max_disk_gb: int=10, ttl_hours: int=24, compression: bool=True):
        """
        ...
        self.stats = {'hits': 0, 'misses': 0, 'evictions': 0, 'memory_size': 0, 'disk_size': 0}
    ...
    def get(self, key: str, loader_fn: Optional[Callable]=None, loader_args: Optional[Tuple]=None, loader_kwargs: Optional[Dict]=None) -> Optional[Any]:
        """
    ...
    def set(self, key: str, data: Any, metadata: Optional[Dict[str, Any]]=None):
        ...
        now = datetime.now()
        entry = CacheEntry(key=key, data=data, size_bytes=size_bytes, created_at=now, last_accessed=now, metadata=metadata or {})
        self._add_to_memory(key, data, entry)
        if size_bytes > self.max_memory_bytes * 0.1 or self._memory_size > self.max_memory_bytes * 0.8:
    ...
    def _add_to_memory(self, key: str, data: Any, entry: Optional[CacheEntry]=None):
        ...
        with self._lock:
            if entry is None:
                size_bytes = self._estimate_size(data)
                now = datetime.now()
                entry = CacheEntry(key=key, data=data, size_bytes=size_bytes, created_at=now, last_accessed=now)
            while self._memory_size + entry.size_bytes > self.max_memory_bytes and self._memory_cache:
        ...
        try:
            ...
            with open(cache_file, 'wb') as f:
                if self.compression:
                    ...
                    pickle.dump(data, f)
            self.disk_metadata[key] = {'size_bytes': entry.size_bytes, 'created_at': entry.created_at.isoformat(), 'last_accessed': entry.last_accessed.isoformat(), 'access_count': entry.access_count, 'metadata': entry.metadata}
            self._save_disk_metadata()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {'hits': self.stats['hits'], 'misses': self.stats['misses'], 'hit_rate': hit_rate, 'evictions': self.stats['evictions'], 'memory_size_mb': self.stats['memory_size'] / 1024 / 1024, 'disk_size_mb': self.stats['disk_size'] / 1024 / 1024, 'memory_entries': len(self._memory_cache), 'disk_entries': len(self.disk_metadata)}
...
class GraphFeatureCache(CacheManager):
    ...
    def cache_graph_features(self, graph_id: str, feature_type: str, features: torch.Tensor, metadata: Optional[Dict[str, Any]]=None):
        """Cache graph features with specific handling"""
        key = f'features_{graph_id}_{feature_type}'
        if metadata is None:
            metadata = {}
        metadata.update({'graph_id': graph_id, 'feature_type': feature_type, 'shape': list(features.shape), 'dtype': str(features.dtype)})
...
class ModelCache(CacheManager):
    ...
    def cache_model(self, model_id: str, model_state: Dict[str, Any], performance_metrics: Dict[str, float], config: Dict[str, Any]):
        """Cache trained model with metadata"""
        key = f'model_{model_id}'
        data = {'state_dict': model_state, 'metrics': performance_metrics, 'config': config, 'cached_at': datetime.now().isoformat()}
        ...
        with self._lock:
            for key, entry in self._memory_cache.items():
                if key.startswith('model_'):
                    models.append({'model_id': entry.metadata.get('model_id', key[6:]), 'cached_at': entry.created_at.isoformat(), 'size_mb': entry.size_bytes / 1024 / 1024, 'location': 'memory'})
        for key, metadata in self.disk_metadata.items():
            if key.startswith('model_'):
                models.append({'model_id': metadata['metadata'].get('model_id', key[6:]), 'cached_at': metadata['created_at'], 'size_mb': metadata['size_bytes'] / 1024 / 1024, 'location': 'disk'})
        return models
