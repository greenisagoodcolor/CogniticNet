...
class CacheManager:
    ...
    def _add_to_memory(self, key: str, data: Any, entry: Optional[CacheEntry] = None):
        ...
        with self._lock:
            ...
            while (
                self._memory_size + entry.size_bytes > self.max_memory_bytes
    ...
    def _evict_from_disk(self):
        ...
        sorted_entries = sorted(
            self.disk_metadata.items(), key=lambda x: x[1]["last_accessed"]
        )
...
class GraphFeatureCache(CacheManager):
    ...
    def get_graph_features(
        self, graph_id: str, feature_type: str
...
if __name__ == "__main__":
    cache = CacheManager(
        cache_dir=".cache/gnn_example", max_memory_mb=512, max_disk_gb=5
    )
