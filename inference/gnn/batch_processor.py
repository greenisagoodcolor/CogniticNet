...
class BatchedGraphData:
    ...
    edge_index: torch.Tensor  # Batched edge indices
    edge_attr: Optional[torch.Tensor] = None  # Batched edge attributes
    edge_weight: Optional[torch.Tensor] = None  # Batched edge weights
    batch: torch.Tensor = None  # Node-to-graph assignment
    ptr: torch.Tensor = None  # Pointer to graph boundaries
    graph_attr: Optional[torch.Tensor] = None  # Graph-level features
    target: Optional[torch.Tensor] = None  # Targets
    mask: Optional[torch.Tensor] = None  # Node masks
    num_graphs: int = 0
...
class GraphBatchProcessor:
    ...
    def _create_batch_manual(self, graphs: List[GraphData]) -> BatchedGraphData:
        ...
        if any(g.edge_attr is not None for g in graphs):
            edge_attr = torch.cat([
                g.edge_attr if g.edge_attr is not None
        ...
        if any(g.edge_weight is not None for g in graphs):
            edge_weight = torch.cat([
                g.edge_weight if g.edge_weight is not None
        ...
        if any(g.target is not None for g in graphs):
            target = torch.stack([
                g.target if g.target is not None
                else torch.zeros(1)
                for g in graphs
            ])
        else:
    ...
    def _batch_graph_attributes(self, graphs: List[GraphData]) -> torch.Tensor:
        ...
        if self.pad_graph_features:
            ...
            max_dim = max(
                g.graph_attr.size(-1) if g.graph_attr is not None else 0
                for g in graphs
            )
            ...
            return torch.cat([
                g.graph_attr if g.graph_attr is not None
                else torch.zeros(1)
                for g in graphs
            ])
...
class DynamicBatchSampler:
    ...
    def __init__(
        self,
        graph_sizes: List[Tuple[int, int]],  # (num_nodes, num_edges)
        batch_size: int,
    ...
    def _group_by_size(self) -> List[List[int]]:
        ...
        for i in range(len(self.graph_sizes)):
            ...
            for j in range(i + 1, len(self.graph_sizes)):
                ...
                node_ratio = abs(nodes - base_nodes) / (base_nodes + 1e-6)
                edge_ratio = abs(edges - base_edges) / (base_edges + 1e-6)
