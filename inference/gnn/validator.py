...
class GNNValidator:
    ...
    """
    VALID_TYPES = {'Real', 'Integer', 'Boolean', 'String', 'H3Cell', 'AgentID', 'Resource', 'Action', 'List', 'Set', 'Dict', 'Distribution', 'Observation', 'State', 'Belief', 'Goal', 'Timestamp', 'Duration', 'Message', 'TradeOffer'}
    BUILTIN_FUNCTIONS = {'bayesian_update', 'softmax', 'sigmoid', 'tanh', 'exp', 'log', 'sqrt', 'abs', 'min', 'max', 'sum', 'mean', 'variance', 'normalize', 'argmin', 'argmax', 'clip', 'round'}
    ...
    def _validate_basic_structure(self, model: GNNModel, result: ValidationResult):
        ...
        if not model.name:
            ...
            result.add_error('model.name', f'Model name too long (max {ValidationConstraints.MAX_NAME_LENGTH})', 'NAME_TOO_LONG')
        elif not re.match(ValidationConstraints.VALID_NAME_PATTERN, model.name):
            result.add_error('model.name', 'Model name contains invalid characters', 'INVALID_NAME_FORMAT')
        if not model.description:
            result.add_warning('model.description', 'Model description is recommended', 'MISSING_DESCRIPTION')
    ...
    def _validate_state_space(self, state_space: Dict[str, Any], result: ValidationResult):
        ...
        if len(state_space) > ValidationConstraints.MAX_VARIABLE_COUNT:
            result.add_error('state_space', f'Too many state variables (max {ValidationConstraints.MAX_VARIABLE_COUNT})', 'TOO_MANY_VARIABLES')
        for var_name, type_def in state_space.items():
            if not isinstance(var_name, str):
                result.add_error(f'state_space.{var_name}', 'Variable name must be a string', 'INVALID_VAR_NAME_TYPE')
                continue
            if not re.match(ValidationConstraints.VALID_NAME_PATTERN, var_name):
                result.add_error(f'state_space.{var_name}', f'Invalid variable name: {var_name}', 'INVALID_VAR_NAME')
                continue
            if len(var_name) > ValidationConstraints.MAX_NAME_LENGTH:
                result.add_error(f'state_space.{var_name}', 'Variable name too long', 'VAR_NAME_TOO_LONG')
                continue
    ...
    def _validate_observations(self, observations: Dict[str, Any], result: ValidationResult):
        ...
        for obs_name, type_def in observations.items():
            ...
            if not isinstance(obs_name, str):
                result.add_error(f'observations.{obs_name}', 'Observation name must be a string', 'INVALID_OBS_NAME_TYPE')
                continue
            if not re.match(ValidationConstraints.VALID_NAME_PATTERN, obs_name):
                result.add_error(f'observations.{obs_name}', f'Invalid observation name: {obs_name}', 'INVALID_OBS_NAME')
            self._validate_type_definition(f'observations.{obs_name}', type_def, result)

    def _validate_type_definition(self, field: str, type_def: Dict[str, Any], result: ValidationResult):
    ...
    def _validate_real_constraints(self, field: str, constraints: Any, result: ValidationResult):
    ...
    def _validate_integer_constraints(self, field: str, constraints: Any, result: ValidationResult):
    ...
    def _validate_h3cell_constraints(self, field: str, constraints: Any, result: ValidationResult):
        ...
        if 'resolution' in constraints:
            try:
                ...
                if res < 0 or res > 15:
                    result.add_error(field, f'H3 resolution must be 0-15, got {res}', 'INVALID_H3_RESOLUTION')
            except (ValueError, TypeError):
                result.add_error(field, f"Invalid resolution: {constraints['resolution']}", 'INVALID_RESOLUTION_TYPE')
        else:
            result.add_warning(field, 'H3Cell should specify resolution', 'MISSING_H3_RESOLUTION')

    def _validate_collection_constraints(self, field: str, constraints: Any, result: ValidationResult):
        ...
        if element_type:
            if element_type not in self.VALID_TYPES and element_type not in self.defined_variables:
                result.add_warning(field, f'Unknown element type: {element_type}', 'UNKNOWN_ELEMENT_TYPE')
    ...
    def _validate_distribution_constraints(self, field: str, constraints: Any, result: ValidationResult):
        ...
        if element_type:
            if element_type not in self.VALID_TYPES and element_type not in self.defined_variables:
                result.add_warning(field, f'Unknown distribution over: {element_type}', 'UNKNOWN_DIST_TYPE')

    def _validate_connections(self, connections: List[Dict[str, Any]], result: ValidationResult):
        ...
        if len(connections) > ValidationConstraints.MAX_CONNECTION_COUNT:
            result.add_error('connections', f'Too many connections (max {ValidationConstraints.MAX_CONNECTION_COUNT})', 'TOO_MANY_CONNECTIONS')
        ...
        for i, conn in enumerate(connections):
            ...
            if not all([source, target, conn_type]):
                result.add_error(f'connections[{i}]', 'Connection must have source, target, and type', 'INCOMPLETE_CONNECTION')
                continue
            if not isinstance(source, str) or not isinstance(target, str):
                result.add_error(f'connections[{i}]', 'Source and target must be strings', 'INVALID_CONNECTION_FIELDS')
                continue
            if source == target:
                result.add_warning(f'connections[{i}]', f'Self-loop detected: {source} -> {target}', 'SELF_LOOP')
            conn_key = (source, target)
            if conn_key in seen_connections:
                result.add_warning(f'connections[{i}]', f'Duplicate connection: {source} -> {target}', 'DUPLICATE_CONNECTION')
            seen_connections.add(conn_key)
            self.referenced_variables.add(source)
            self.referenced_variables.add(target)

    def _validate_update_equations(self, equations: Dict[str, str], result: ValidationResult):
        ...
        for var_name, equation in equations.items():
            if not isinstance(equation, str):
                result.add_error(f'update_equations.{var_name}', 'Equation must be a string', 'INVALID_EQUATION_TYPE')
                continue
            if len(equation) > ValidationConstraints.MAX_EQUATION_LENGTH:
                result.add_error(f'update_equations.{var_name}', 'Equation too long', 'EQUATION_TOO_LONG')
                continue
            if var_name not in self.defined_variables:
                result.add_warning(f'update_equations.{var_name}', f'Updating undefined variable: {var_name}', 'UNDEFINED_UPDATE_VAR')
            self._validate_equation(f'update_equations.{var_name}', equation, result)

    def _validate_equation(self, field: str, equation: str, result: ValidationResult):
        """Validate an equation for syntax, references, and security."""
        dangerous_patterns = ['__', 'exec', 'eval', 'import', 'open', 'file', 'subprocess', 'os.', 'sys.', 'globals', 'locals']
        for pattern in dangerous_patterns:
            if pattern in equation:
                result.add_error(field, f"Potentially dangerous pattern '{pattern}' in equation", 'DANGEROUS_PATTERN')
    ...
    def _validate_preferences(self, preferences: Dict[str, Any], result: ValidationResult):
        """Validate preference functions."""
        if not preferences:
            result.add_warning('preferences', 'No preferences defined (required for Active Inference)', 'NO_PREFERENCES')
            return
        ...
        for pref_name, pref_def in preferences.items():
            if not pref_name.endswith('_pref'):
                result.add_warning(f'preferences.{pref_name}', "Preference names should end with '_pref'", 'INVALID_PREF_NAME')
            if not isinstance(pref_def, dict):
                result.add_error(f'preferences.{pref_name}', 'Preference must be a dictionary', 'INVALID_PREF_TYPE')
                continue
            input_type = pref_def.get('input')
            output_type = pref_def.get('output')
            if not input_type:
                result.add_error(f'preferences.{pref_name}', 'Preference must specify input type', 'MISSING_PREF_INPUT')
            if not output_type:
                result.add_error(f'preferences.{pref_name}', 'Preference must specify output type', 'MISSING_PREF_OUTPUT')
            details = pref_def.get('details', [])
            if not details:
                result.add_warning(f'preferences.{pref_name}', 'Preference should specify weighted components', 'NO_PREF_DETAILS')
            elif isinstance(details, list) and len(details) > ValidationConstraints.MAX_PREFERENCE_WEIGHTS:
                result.add_warning(f'preferences.{pref_name}', f'Too many preference weights (max {ValidationConstraints.MAX_PREFERENCE_WEIGHTS})', 'TOO_MANY_WEIGHTS')
    ...
    def _validate_references(self, result: ValidationResult):
        ...
        allowed_undefined = {'movement_cost', 'resource_gain', 'decay_factor', 'learning_rate', 'planning_horizon', 'temperature'}
        for var in undefined:
            if var not in allowed_undefined:
                result.add_warning('references', f'Reference to undefined variable: {var}', 'UNDEFINED_REFERENCE')
    ...
    def _validate_consistency(self, model: GNNModel, result: ValidationResult):
        ...
        if not model.state_space and (not model.observations):
            result.add_error('model', 'Model must define either state space or observations', 'EMPTY_MODEL')
        for var in model.update_equations:
            if var not in model.state_space and var not in model.observations:
                result.add_warning('update_equations', f'Update equation for non-existent variable: {var}', 'ORPHANED_EQUATION')
        if model.connections:
            self._check_circular_dependencies(model.connections, result)

    def _check_circular_dependencies(self, connections: List[Dict[str, Any]], result: ValidationResult):
        ...
        for node in graph:
            if node not in visited:
                if has_cycle(node):
                    result.add_warning('connections', 'Circular dependency detected', 'CIRCULAR_DEPENDENCY')
                    break
    ...
    def _validate_security(self, model: GNNModel, result: ValidationResult):
        ...
        suspicious_patterns = ['<script', 'javascript:', 'onclick', '\\$\\{', '\\{\\{', '\\\\x[0-9a-fA-F]{2}', '\\\\u[0-9a-fA-F]{4}']
        for string in all_strings:
            if not string:
                continue
            for pattern in suspicious_patterns:
                if re.search(pattern, string, re.IGNORECASE):
                    result.add_error('security', f'Suspicious pattern detected: {pattern}', 'SECURITY_RISK')
