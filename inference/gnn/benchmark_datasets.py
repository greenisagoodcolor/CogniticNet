...
class BenchmarkDatasets:
    ...
    def load_karate_club(self) -> List[GraphData]:
        ...
        # Create node features (one-hot encoding of node ID)
        ...
        labels = torch.tensor([
            G.nodes[i]['club'] == 'Officer' for i in range(num_nodes)
        ], dtype=torch.long)

        graph = GraphData(
            node_features=node_features,
            edge_index=edge_index,
            target=labels  # Node-level labels
        )
    ...
    def generate_synthetic_large(self) -> List[GraphData]:
        ...
        for i in range(50):
            ...
            # Mix of sparse and dense graphs
            if i < 25:
                # Sparse graphs
                if i % 2 == 0:
                    G = nx.barabasi_albert_graph(num_nodes, 2)
            ...
            # Add graph label based on structure
            label = i % 10
    ...
    def generate_grid_graphs(self) -> List[GraphData]:
        ...
        for i in range(100):
            ...
            # Add some random edges for variation
            if i % 4 > 0:
            ...
            # Label based on grid properties
            if rows == cols:
                label = 0  # Square
            elif rows > cols:
                label = 1  # Tall
            ...
            if i % 4 == 3:
                label = 3  # Modified (with extra edges)

            # Convert nodes from tuples to integers
    ...
    def generate_community_graphs(self) -> List[GraphData]:
        ...
        for i in range(50):
            ...
            community_sizes = [
                np.random.randint(15, 35) for _ in range(num_communities)
            ]

            # Create stochastic block model
            p_within = 0.3  # Probability of edge within community
            p_between = 0.02  # Probability of edge between communities
            ...
            # Create node labels based on community
            node_labels = []
            ...
            graph = GraphData(
                node_features=node_features,
                edge_index=edge_index,
                target=node_labels  # Node-level community labels
            )
            graphs.append(graph)
