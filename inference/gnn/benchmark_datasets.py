...
class BenchmarkDatasets:
    ...
    def get_dataset_info(self) -> Dict[str, DatasetInfo]:
        ...
        return {'karate_club': DatasetInfo(name='Zachary Karate Club', num_graphs=1, num_classes=2, avg_nodes=34, avg_edges=78, node_features=34, task_type='node_classification', description='Classic social network dataset'), 'synthetic_small': DatasetInfo(name='Synthetic Small Graphs', num_graphs=100, num_classes=3, avg_nodes=15, avg_edges=30, node_features=32, task_type='graph_classification', description='Small synthetic graphs for quick testing'), 'synthetic_medium': DatasetInfo(name='Synthetic Medium Graphs', num_graphs=100, num_classes=5, avg_nodes=50, avg_edges=150, node_features=64, edge_features=8, task_type='graph_classification', description='Medium-sized synthetic graphs'), 'synthetic_large': DatasetInfo(name='Synthetic Large Graphs', num_graphs=50, num_classes=10, avg_nodes=200, avg_edges=800, node_features=128, edge_features=16, task_type='graph_classification', description='Large synthetic graphs for stress testing'), 'grid_graphs': DatasetInfo(name='Grid Graphs', num_graphs=100, num_classes=4, avg_nodes=25, avg_edges=40, node_features=16, task_type='graph_classification', description='2D grid-based graphs'), 'tree_graphs': DatasetInfo(name='Tree Graphs', num_graphs=100, num_classes=3, avg_nodes=31, avg_edges=30, node_features=32, task_type='graph_classification', description='Tree-structured graphs'), 'community_graphs': DatasetInfo(name='Community Graphs', num_graphs=50, num_classes=4, avg_nodes=100, avg_edges=500, node_features=64, task_type='node_classification', description='Graphs with community structure')}
    ...
    def load_karate_club(self) -> List[GraphData]:
        ...
        labels = torch.tensor([G.nodes[i]['club'] == 'Officer' for i in range(num_nodes)], dtype=torch.long)
    ...
    def generate_synthetic_medium(self) -> List[GraphData]:
        ...
        for i in range(100):
            ...
            graph = self._networkx_to_graphdata(G, feature_dim=64, edge_feature_dim=8, graph_label=graph_type)
            graphs.append(graph)
        return graphs
    ...
    def generate_synthetic_large(self) -> List[GraphData]:
        ...
        for i in range(50):
            ...
            label = i % 10
            graph = self._networkx_to_graphdata(G, feature_dim=128, edge_feature_dim=16, graph_label=label)
            graphs.append(graph)
        return graphs
    ...
    def generate_community_graphs(self) -> List[GraphData]:
        ...
        for i in range(50):
            ...
            graph = GraphData(node_features=node_features, edge_index=edge_index, target=node_labels)
            graphs.append(graph)
        return graphs

    def _networkx_to_graphdata(self, G: nx.Graph, feature_dim: int, edge_feature_dim: Optional[int]=None, graph_label: Optional[int]=None) -> GraphData:
        ...
        if graph_label is not None:
            ...
            target = None
        return GraphData(node_features=node_features, edge_index=edge_index, edge_attr=edge_attr, target=target)
    ...
    def get_dataset(self, name: str) -> List[GraphData]:
        ...
        Returns:
            List of graphs
        """
        datasets = {'karate_club': self.load_karate_club, 'synthetic_small': self.generate_synthetic_small, 'synthetic_medium': self.generate_synthetic_medium, 'synthetic_large': self.generate_synthetic_large, 'grid_graphs': self.generate_grid_graphs, 'tree_graphs': self.generate_tree_graphs, 'community_graphs': self.generate_community_graphs}
    ...
    def save_dataset(self, name: str, save_dir: str):
        """
        ...
        with open(save_path / f'{name}_info.json', 'w') as f:
            json.dump({'name': info.name, 'num_graphs': info.num_graphs, 'num_classes': info.num_classes, 'avg_nodes': info.avg_nodes, 'avg_edges': info.avg_edges, 'node_features': info.node_features, 'edge_features': info.edge_features, 'task_type': info.task_type, 'description': info.description}, f, indent=2)
        for i, graph in enumerate(graphs):
            torch.save({'node_features': graph.node_features, 'edge_index': graph.edge_index, 'edge_attr': graph.edge_attr, 'edge_weight': graph.edge_weight, 'target': graph.target}, save_path / f'{name}_graph_{i}.pt')
    ...
    def load_dataset_from_disk(self, name: str, load_dir: str) -> List[GraphData]:
        ...
        while True:
            ...
            graph = GraphData(node_features=data['node_features'], edge_index=data['edge_index'], edge_attr=data.get('edge_attr'), edge_weight=data.get('edge_weight'), target=data.get('target'))
            graphs.append(graph)
            i += 1
        return graphs

def create_benchmark_splits(graphs: List[GraphData], train_ratio: float=0.8, val_ratio: float=0.1, test_ratio: float=0.1, shuffle: bool=True, seed: int=42) -> Tuple[List[GraphData], List[GraphData], List[GraphData]]:
    """
