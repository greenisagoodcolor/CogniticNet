    ...
    def optimize_belief_update(
        ...
        if observation.dim() == 1:
            ...
            likelihood = torch.sparse.mm(
                sparse_A.t(), observation.unsqueeze(1)
            ).squeeze()

        # Posterior computation
        log_posterior = torch.log(likelihood + self.config.eps) + torch.log(
            prior + self.config.eps
        )
...
class ParallelInference:
    ...
    def parallel_expected_free_energy(
        ...
        if not self.config.use_parallel_processing:
            return torch.stack(
                [self._single_efe(qs, A, B, C, action) for action in actions]
            )
...
class BatchProcessor:
    ...
    def add_request(self, request_id: str, computation: Callable, *args, **kwargs):
    ...
    def _batch_belief_updates(self, requests: List[Dict]):
        ...
        # Batch computation
        likelihoods = torch.matmul(
            A_matrices.transpose(1, 2), observations.unsqueeze(2)
        ).squeeze()
