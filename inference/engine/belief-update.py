...
class GraphObservationModel(ABC):
    ...
    @abstractmethod
    def compute_likelihood(self, graph_features: torch.Tensor, states: torch.Tensor) -> torch.Tensor:
...
class DirectGraphObservationModel(GraphObservationModel):
    ...
    def __init__(self, config: BeliefUpdateConfig, state_dim: int, feature_dim: int):
        super().__init__(config)
        self.state_dim = state_dim
        self.feature_dim = feature_dim
        self.observation_matrix = nn.Parameter(torch.randn(feature_dim, state_dim, device=self.device) * 0.1)
        self.log_precision = nn.Parameter(torch.zeros(feature_dim, device=self.device))
    ...
    def compute_likelihood(self, graph_features: torch.Tensor, states: torch.Tensor) -> torch.Tensor:
        ...
        log_likelihood = -0.5 * torch.sum(precision * diff ** 2, dim=-1) - 0.5 * torch.sum(torch.log(2 * np.pi / precision))
...
class LearnedGraphObservationModel(GraphObservationModel):
    ...
    def __init__(self, config: BeliefUpdateConfig, state_dim: int, feature_dim: int, hidden_dim: int=64):
        super().__init__(config)
        self.state_dim = state_dim
        self.feature_dim = feature_dim
        self.feature_encoder = nn.Sequential(nn.Linear(feature_dim, hidden_dim), nn.ReLU(), nn.Dropout(0.1), nn.Linear(hidden_dim, hidden_dim), nn.ReLU()).to(self.device)
        self.state_encoder = nn.Sequential(nn.Linear(state_dim, hidden_dim), nn.ReLU(), nn.Dropout(0.1), nn.Linear(hidden_dim, hidden_dim), nn.ReLU()).to(self.device)
        self.likelihood_net = nn.Sequential(nn.Linear(hidden_dim * 2, hidden_dim), nn.ReLU(), nn.Dropout(0.1), nn.Linear(hidden_dim, 1)).to(self.device)
        self.observation_decoder = nn.Sequential(nn.Linear(feature_dim, hidden_dim), nn.ReLU(), nn.Dropout(0.1), nn.Linear(hidden_dim, state_dim)).to(self.device)
...
class GNNBeliefUpdater:
    ...
    def __init__(self, config: BeliefUpdateConfig, generative_model: GenerativeModel, inference_algorithm: InferenceAlgorithm, observation_model: GraphObservationModel):
        self.config = config
        ...
        if config.feature_weighting == 'learned':
            self.feature_weights = nn.Parameter(torch.ones(1, device=self.device))
        elif config.feature_weighting == 'attention':
            self.attention_net = nn.Sequential(nn.Linear(observation_model.feature_dim, 64), nn.Tanh(), nn.Linear(64, 1)).to(self.device)
        if config.temporal_integration:
            self.temporal_decay = nn.Parameter(torch.tensor(0.9, device=self.device))

    def update_beliefs(self, current_beliefs: torch.Tensor, graph_features: torch.Tensor, actions: Optional[torch.Tensor]=None, previous_beliefs: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
    ...
    def _bayesian_update(self, current_beliefs: torch.Tensor, graph_features: torch.Tensor, actions: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
    ...
    def _gradient_update(self, current_beliefs: torch.Tensor, graph_features: torch.Tensor, actions: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
    ...
    def _hybrid_update(self, current_beliefs: torch.Tensor, graph_features: torch.Tensor, actions: Optional[torch.Tensor]=None, previous_beliefs: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
    ...
    def update_with_graph_sequence(self, initial_beliefs: torch.Tensor, graph_sequence: List[Dict[str, torch.Tensor]], actions: Optional[List[torch.Tensor]]=None) -> List[torch.Tensor]:
        """
...
class AttentionGraphBeliefUpdater(GNNBeliefUpdater):
    ...
    def __init__(self, config: BeliefUpdateConfig, generative_model: GenerativeModel, inference_algorithm: InferenceAlgorithm, observation_model: GraphObservationModel, num_heads: int=4):
        ...
        self.attention = nn.MultiheadAttention(embed_dim=feature_dim, num_heads=num_heads, dropout=0.1).to(self.device)
        self.query_net = nn.Linear(state_dim, feature_dim).to(self.device)

    def _compute_attended_features(self, beliefs: torch.Tensor, graph_features: torch.Tensor) -> torch.Tensor:
        """
        ...
        """
        queries = self.query_net(beliefs).unsqueeze(0)
        if graph_features.dim() == 2:
            graph_features = graph_features.unsqueeze(1)
        keys = values = graph_features.transpose(0, 1)
...
class HierarchicalBeliefUpdater:
    ...
    def __init__(self, config: BeliefUpdateConfig, level_updaters: List[GNNBeliefUpdater]):
        self.config = config
        self.level_updaters = level_updaters
        self.num_levels = len(level_updaters)
        self.device = torch.device('cuda' if config.use_gpu and torch.cuda.is_available() else 'cpu')
        self.bottom_up_nets = nn.ModuleList([nn.Linear(level_updaters[i].generative_model.dims.num_states, level_updaters[i + 1].generative_model.dims.num_states).to(self.device) for i in range(self.num_levels - 1)])
        self.top_down_nets = nn.ModuleList([nn.Linear(level_updaters[i + 1].generative_model.dims.num_states, level_updaters[i].generative_model.dims.num_states).to(self.device) for i in range(self.num_levels - 1)])

    def update_hierarchical_beliefs(self, current_beliefs: List[torch.Tensor], graph_features_per_level: List[torch.Tensor], actions_per_level: Optional[List[torch.Tensor]]=None) -> List[torch.Tensor]:
        """
        ...
        for level in range(self.num_levels):
            ...
            updated_beliefs[level] = self.level_updaters[level].update_beliefs(current_beliefs[level], features, actions)
        for level in range(self.num_levels - 2, -1, -1):
            ...
            if isinstance(self.level_updaters[level].generative_model, DiscreteGenerativeModel):
                updated_beliefs[level] = updated_beliefs[level] / (updated_beliefs[level].sum(dim=-1, keepdim=True) + self.config.eps)
        return updated_beliefs

def create_belief_updater(updater_type: str, config: Optional[BeliefUpdateConfig]=None, **kwargs) -> Union[GNNBeliefUpdater, AttentionGraphBeliefUpdater, HierarchicalBeliefUpdater]:
    """
    ...
    if updater_type == 'standard':
        ...
        return AttentionGraphBeliefUpdater(config, generative_model, inference_algorithm, observation_model, num_heads)
