...
class BeliefTracker:
    ...
    def record_belief(self, belief: torch.Tensor, timestamp: Optional[float] = None):
        """Record belief state"""
...
class FreeEnergyMonitor:
    ...
    def plot_free_energy_components(
        ...
        if len(self.vfe_history) > 0:
            axes[0].plot(
                timestamps, self.vfe_history, "k-", linewidth=2, label="Total VFE"
            )
        ...
        if len(self.efe_history) > 0:
            axes[1].plot(
                timestamps[: len(self.efe_history)], self.efe_history, "g-", linewidth=2
            )
        ...
        if save_path or self.config.save_figures:
            save_path = (
                save_path or self.config.figure_dir / "free_energy_components.png"
            )
            save_path.parent.mkdir(parents=True, exist_ok=True)
            plt.savefig(save_path, dpi=self.config.dpi)
...
class InferenceVisualizer:
    ...
    def visualize_inference_graph(
        ...
        # Draw nodes
        state_nodes = [
            n for n in G_obs.nodes() if G_obs.nodes[n].get("node_type") == "state"
        ]
        obs_nodes = [
            n for n in G_obs.nodes() if G_obs.nodes[n].get("node_type") == "observation"
        ]
        ...
        nx.draw_networkx_edges(
            G_obs, pos, width=np.array(weights) * 5, alpha=0.6, ax=ax1
        )
        ...
        def update(frame):
            ...
            if len(belief_tracker.belief_history) > 0:
                ...
                for i in range(belief_tracker.num_states):
                    ax1.plot(
                        timestamps, beliefs[:, i], label=belief_tracker.state_labels[i]
                    )
...
class DiagnosticSuite:
    ...
    def _setup_logging(self):
        ...
        log_file = (
            self.config.log_dir / f"diagnostics_{time.strftime('%Y%m%d_%H%M%S')}.log"
        )
...
if __name__ == "__main__":
    # Configuration
    config = DiagnosticConfig(
        enable_realtime=True, track_gradients=True, save_figures=True
    )
