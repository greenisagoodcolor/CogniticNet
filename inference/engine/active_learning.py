...
class InformationSeeker(ABC):
    ...
    @abstractmethod
    def compute_information_value(self, beliefs: torch.Tensor, possible_observations: torch.Tensor) -> torch.Tensor:
        """Compute the value of potential observations for reducing uncertainty"""
        pass

    @abstractmethod
    def select_informative_action(self, beliefs: torch.Tensor, available_actions: torch.Tensor) -> torch.Tensor:
...
class EntropyBasedSeeker(InformationSeeker):
    ...
    def __init__(self, config: ActiveLearningConfig, generative_model: GenerativeModel):
...
class MutualInformationSeeker(InformationSeeker):
    ...
    def __init__(self, config: ActiveLearningConfig, generative_model: GenerativeModel, inference_algorithm: InferenceAlgorithm):
        super().__init__(config)
        self.generative_model = generative_model
        self.inference = inference_algorithm

    def compute_mutual_information(self, beliefs: torch.Tensor, observation_dist: torch.Tensor) -> torch.Tensor:
        """
        ...
        if isinstance(self.generative_model, DiscreteGenerativeModel):
            ...
            conditional_entropy = -torch.sum(joint * torch.log(joint / (marginal_obs.unsqueeze(2) + self.config.eps) + self.config.eps)).mean()
        else:
...
class ActiveLearningAgent:
    ...
    def __init__(self, config: ActiveLearningConfig, generative_model: GenerativeModel, inference_algorithm: InferenceAlgorithm, policy_selector: PolicySelector, information_seeker: Optional[InformationSeeker]=None):
        self.config = config
        self.generative_model = generative_model
        self.inference = inference_algorithm
    ...
    def compute_epistemic_value(self, beliefs: torch.Tensor, policies: List[Policy]) -> torch.Tensor:
        """
    ...
    def compute_pragmatic_value(self, beliefs: torch.Tensor, policies: List[Policy], preferences: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
        ...
        for policy in policies:
            G = self.policy_selector.compute_expected_free_energy(policy, beliefs, self.generative_model, preferences)
            pragmatic_values.append(-G)
        return torch.stack(pragmatic_values)

    def select_exploratory_action(self, beliefs: torch.Tensor, available_actions: torch.Tensor, preferences: Optional[torch.Tensor]=None) -> Tuple[int, Dict[str, float]]:
        """
        ...
        total_values = self.exploration_rate * epistemic_values + (1 - self.exploration_rate) * pragmatic_values + self.config.novelty_weight * novelty_values
        ...
        info = {'epistemic_value': epistemic_values[best_policy_idx].item(), 'pragmatic_value': pragmatic_values[best_policy_idx].item(), 'novelty_value': novelty_values[best_policy_idx].item(), 'exploration_rate': self.exploration_rate}
    ...
    def _generate_policies(self, available_actions: torch.Tensor) -> List[Policy]:
        ...
        for _ in range(self.config.num_samples):
            ...
            policy = Policy(actions=actions.tolist(), horizon=self.config.planning_horizon)
...
class InformationGainPlanner:
    ...
    def __init__(self, config: ActiveLearningConfig, generative_model: GenerativeModel, information_seeker: InformationSeeker):
        self.config = config
        self.generative_model = generative_model
        self.info_seeker = information_seeker
        self.device = torch.device('cuda' if config.use_gpu and torch.cuda.is_available() else 'cpu')

    def plan_information_gathering(self, current_beliefs: torch.Tensor, target_uncertainty: float=0.1, max_steps: int=10) -> List[int]:
        """
        ...
        for step in range(max_steps):
            ...
            if isinstance(self.generative_model, DiscreteGenerativeModel):
                ...
                for a in range(num_actions):
                    next_beliefs = torch.matmul(beliefs, B_matrix[:, :, a].T)
                    next_entropy = -torch.sum(next_beliefs * torch.log(next_beliefs + self.config.eps), dim=-1).mean()
...
def create_active_learner(learner_type: str, config: Optional[ActiveLearningConfig]=None, **kwargs) -> Union[ActiveLearningAgent, InformationGainPlanner]:
    """
