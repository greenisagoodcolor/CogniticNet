...
class TreeNode:
    ...
    def __init__(self, state: torch.Tensor, action: Optional[int]=None, parent: Optional['TreeNode']=None, depth: int=0):
        self.state = state
        self.action = action
        self.parent = parent
        self.depth = depth
...
class TemporalPlanner(ABC):
    ...
    def __init__(self, config: PlanningConfig, policy_selector: PolicySelector, inference_algorithm: InferenceAlgorithm):
        self.config = config
    ...
    @abstractmethod
    def plan(self, initial_beliefs: torch.Tensor, generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> Tuple[Policy, float]:
        """Plan over temporal horizon"""
        pass

    @abstractmethod
    def evaluate_trajectory(self, trajectory: List[TreeNode], generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> float:
...
class MonteCarloTreeSearch(TemporalPlanner):
    ...
    def _expand(self, node: TreeNode, generative_model: GenerativeModel) -> TreeNode:
        ...
        untried_actions = [a for a in range(generative_model.dims.num_actions) if a not in tried_actions]
    ...
    def _simulate(self, node: TreeNode, generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> float:
        ...
        for t in range(self.config.planning_horizon - node.depth):
            ...
            G = self.policy_selector.compute_expected_free_energy(policy, current_beliefs, generative_model, preferences)
    ...
    def evaluate_trajectory(self, trajectory: List[TreeNode], generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> float:
        ...
        for i in range(len(trajectory) - 1):
            ...
            if next_node.action is not None:
                ...
                G = self.policy_selector.compute_expected_free_energy(policy, node.state, generative_model, preferences)
                total_G += discount * G.item()
                discount *= self.config.discount_factor
        return -total_G
...
class BeamSearchPlanner(TemporalPlanner):
        ...
        for depth in range(min(self.config.planning_horizon, self.config.max_depth)):
            ...
            for cost, actions, beliefs in beam:
                for action in range(generative_model.dims.num_actions):
                    policy = Policy([action])
                    G = self.policy_selector.compute_expected_free_energy(policy, beliefs, generative_model, preferences)
    ...
    def evaluate_trajectory(self, trajectory: List[TreeNode], generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> float:
        ...
        G = self.policy_selector.compute_expected_free_energy(policy, trajectory[0].state, generative_model, preferences)
        return -G.item()
...
class AStarPlanner(TemporalPlanner):
    ...
    def plan(self, initial_beliefs: torch.Tensor, generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> Tuple[Policy, float]:
        ...
        while open_set and len(closed_set) < self.config.max_nodes:
            ...
            for action in range(generative_model.dims.num_actions):
                ...
                h_score = self._heuristic(next_beliefs, len(actions) + 1, generative_model, preferences)
    ...
    def _heuristic(self, beliefs: torch.Tensor, depth: int, generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> float:
    ...
    def evaluate_trajectory(self, trajectory: List[TreeNode], generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> float:
        ...
        for i in range(len(trajectory) - 1):
            if trajectory[i + 1].action is not None:
                ...
                G = self.policy_selector.compute_expected_free_energy(policy, trajectory[i].state, generative_model, preferences)
                total_G += discount * G.item()
                discount *= self.config.discount_factor
        return -total_G
...
class TrajectorySampling(TemporalPlanner):
    ...
    def plan(self, initial_beliefs: torch.Tensor, generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> Tuple[Policy, float]:
        ...
        for _ in range(self.config.num_trajectories):
            trajectory, value = self._sample_trajectory(initial_beliefs, generative_model, preferences)
            if value > best_value:
                best_value = value
    ...
    def _sample_trajectory(self, initial_beliefs: torch.Tensor, generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> Tuple[List[TreeNode], float]:
        ...
        discount = 1.0
        for depth in range(min(self.config.planning_horizon, self.config.max_depth)):
            temp_policy, probs = self.policy_selector.select_policy(current_beliefs, generative_model, preferences)
...
class AdaptiveHorizonPlanner(TemporalPlanner):
    ...
    def __init__(self, config: PlanningConfig, policy_selector: PolicySelector, inference_algorithm: InferenceAlgorithm, base_planner: TemporalPlanner):
        super().__init__(config, policy_selector, inference_algorithm)
    ...
    def plan(self, initial_beliefs: torch.Tensor, generative_model: GenerativeModel, preferences: Optional[torch.Tensor]=None) -> Tuple[Policy, float]:
        ...
        if uncertainty > self.uncertainty_threshold:
            adaptive_horizon = min(int(self.config.planning_horizon * 1.5), self.config.max_depth * 2)
        else:
            adaptive_horizon = max(int(self.config.planning_horizon * 0.7), 3)
...
def create_temporal_planner(planner_type: str, config: Optional[PlanningConfig]=None, **kwargs) -> TemporalPlanner:
    """
    ...
    if planner_type == 'mcts':
        ...
        base_planner = create_temporal_planner(base_planner_type, config, policy_selector=policy_selector, inference_algorithm=inference_algorithm)
...
if __name__ == '__main__':
    ...
    planning_config = PlanningConfig(planning_horizon=5, num_simulations=50, use_gpu=False)
