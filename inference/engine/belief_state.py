...
class BeliefState(ABC):
    ...
    def __init__(self, config: BeliefStateConfig):
        self.config = config
    ...
    @abstractmethod
    def set_beliefs(self, beliefs: torch.Tensor):
...
class DiscreteBeliefState(BeliefState):
    ...
    def __init__(self,
        ...
        if initial_beliefs is not None:
            ...
            # Uniform prior
            self.beliefs = torch.ones(num_states, device=self.device, dtype=config.dtype) / num_states
    ...
    def set_beliefs(self, beliefs: torch.Tensor):
        ...
        if beliefs.shape != self.beliefs.shape:
            raise ValueError(f"Belief shape {beliefs.shape} doesn't match expected {self.beliefs.shape}")
    ...
    def update_beliefs(self, evidence: torch.Tensor, update_method: str = 'bayes') -> 'DiscreteBeliefState':
        """
        ...
        self.metadata['last_update_method'] = update_method
        self.update_count += 1

        return self

    def _bayesian_update(self, likelihood: torch.Tensor):
    ...
    def _linear_update(self, evidence: torch.Tensor, alpha: float = 0.1):
    ...
    def _momentum_update(self, evidence: torch.Tensor, momentum: float = 0.9):
    ...
    def _validate_beliefs(self):
        ...
        if abs(belief_sum - 1.0) > 1e-3:
            logger.warning(f"Beliefs not normalized (sum={belief_sum:.6f}), normalizing")
            self._normalize()
    ...
    def kl_divergence(self, other: 'DiscreteBeliefState') -> torch.Tensor:
        ...
        log_ratio = torch.log(self.beliefs + self.config.eps) - torch.log(other.beliefs + self.config.eps)
        return torch.sum(self.beliefs * log_ratio)

    def most_likely_state(self) -> int:
        """Get index of most likely state."""
        return torch.argmax(self.beliefs).item()
    ...
    def sample_state(self, temperature: float = 1.0) -> int:
        ...
        return torch.multinomial(scaled_beliefs, 1).item()

    def reset_to_uniform(self):
        """Reset beliefs to uniform distribution."""
        self.beliefs = torch.ones(self.num_states, device=self.device, dtype=self.config.dtype) / self.num_states
        self.update_count = 0
    ...
    def clone(self) -> 'DiscreteBeliefState':
        ...
        history_limit = min(len(self.belief_history), 10)  # Limit copied history
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'entropy_history': self.entropy_history[-10:]  # Keep recent history
        }
    ...
    def load(cls, filepath: Union[str, Path]) -> 'DiscreteBeliefState':
        ...
        if filepath.suffix == '.json':
            with open(filepath, 'r') as f:
                data = json.load(f)
            return cls(num_states=1, config=BeliefStateConfig()).from_dict(data)
        elif filepath.suffix == '.pkl':
    ...
    def __repr__(self) -> str:
        ...
        return (f"DiscreteBeliefState(states={self.num_states}, "
...
class ContinuousBeliefState(BeliefState):
    ...
    def get_beliefs(self) -> Tuple[torch.Tensor, torch.Tensor]:
        """Get current belief parameters (mean, covariance)."""
        return self.mean.clone(), self.cov.clone()

    def set_beliefs(self, beliefs: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]):
    ...
    def update_beliefs(self, evidence: Tuple[torch.Tensor, torch.Tensor],
                      update_method: str = 'bayes') -> 'ContinuousBeliefState':
        """
        Update Gaussian belief with evidence.

        Args:
            evidence: (observation, observation_cov) or (mean_update, cov_update)
            update_method: 'bayes', 'kalman', 'variational'
        """
        self._add_to_history()

        if update_method == 'bayes':
            self._gaussian_bayesian_update(evidence)
        elif update_method == 'kalman':
            self._kalman_update(evidence)
        elif update_method == 'variational':
            self._variational_update(evidence)
        else:
            raise ValueError(f"Unknown update method: {update_method}")
    ...
    def _gaussian_bayesian_update(self, evidence: Tuple[torch.Tensor, torch.Tensor]):
        """Standard Gaussian Bayesian update."""
        obs, obs_cov = evidence

        # Bayesian update for Gaussian
        # posterior_cov^{-1} = prior_cov^{-1} + obs_cov^{-1}
        cov_inv = torch.inverse(self.cov + self.config.eps * torch.eye(self.state_dim, device=self.device))
        obs_cov_inv = torch.inverse(obs_cov + self.config.eps * torch.eye(self.state_dim, device=self.device))

        posterior_cov_inv = cov_inv + obs_cov_inv
        self.cov = torch.inverse(posterior_cov_inv)

        # posterior_mean = posterior_cov * (prior_cov^{-1} * prior_mean + obs_cov^{-1} * obs)
        self.mean = self.cov @ (cov_inv @ self.mean + obs_cov_inv @ obs)

        self.log_var = torch.log(torch.diag(self.cov) + self.config.eps)

    def _kalman_update(self, evidence: Tuple[torch.Tensor, torch.Tensor]):
    ...
    def _variational_update(self, evidence: Tuple[torch.Tensor, torch.Tensor]):
        """Variational Bayes update (similar to Bayesian but with regularization)."""
        obs, obs_cov = evidence

        # Add entropy regularization to covariance
        reg_cov = self.cov + self.config.entropy_regularization * torch.eye(self.state_dim, device=self.device)

        # Standard Bayesian update with regularized covariance
        cov_inv = torch.inverse(reg_cov + self.config.eps * torch.eye(self.state_dim, device=self.device))
        obs_cov_inv = torch.inverse(obs_cov + self.config.eps * torch.eye(self.state_dim, device=self.device))
    ...
    def entropy(self) -> torch.Tensor:
        ...
        return 0.5 * (self.state_dim * np.log(2 * np.pi) + torch.log(det_cov + self.config.eps))

    def most_likely_state(self) -> torch.Tensor:
        """Get most likely state (mean of Gaussian)."""
    ...
    def __repr__(self) -> str:
        ...
        return (f"ContinuousBeliefState(dim={self.state_dim}, "
                f"entropy={entropy:.3f}, "
                f"det={det:.3f}, "
                f"updates={self.update_count})")
...
def create_discrete_belief_state(num_states: int,
                                config: Optional[BeliefStateConfig] = None,
                                initial_beliefs: Optional[torch.Tensor] = None) -> DiscreteBeliefState:
...
def create_continuous_belief_state(state_dim: int,
                                  config: Optional[BeliefStateConfig] = None,
                                  initial_mean: Optional[torch.Tensor] = None,
                                  initial_cov: Optional[torch.Tensor] = None) -> ContinuousBeliefState:
...
def create_belief_state(belief_type: str, **kwargs) -> BeliefState:
    """
    Factory function to create belief states by type.
