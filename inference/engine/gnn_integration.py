...
class GraphToStateMapper(ABC):
    ...
    @abstractmethod
    def map_to_states(self, graph_features: torch.Tensor, node_indices: Optional[torch.Tensor]=None) -> torch.Tensor:
        """Map graph features to state representation"""
        pass

    @abstractmethod
    def map_to_observations(self, graph_features: torch.Tensor, node_indices: Optional[torch.Tensor]=None) -> torch.Tensor:
...
class DirectGraphMapper(GraphToStateMapper):
    ...
    def __init__(self, config: GNNIntegrationConfig, state_dim: int, observation_dim: int):
...
class LearnedGraphMapper(GraphToStateMapper):
    ...
    def __init__(self, config: GNNIntegrationConfig, state_dim: int, observation_dim: int):
        ...
        self.state_dim = state_dim
        self.observation_dim = observation_dim
        self.state_mapper = nn.Sequential(nn.Linear(config.output_dim, config.hidden_dim), nn.ReLU(), nn.Dropout(config.dropout), nn.Linear(config.hidden_dim, config.hidden_dim), nn.ReLU(), nn.Dropout(config.dropout), nn.Linear(config.hidden_dim, state_dim)).to(self.device)
        self.obs_mapper = nn.Sequential(nn.Linear(config.output_dim, config.hidden_dim), nn.ReLU(), nn.Dropout(config.dropout), nn.Linear(config.hidden_dim, config.hidden_dim), nn.ReLU(), nn.Dropout(config.dropout), nn.Linear(config.hidden_dim, observation_dim)).to(self.device)
...
class GNNActiveInferenceAdapter:
    ...
    def __init__(self, config: GNNIntegrationConfig, gnn_model: nn.Module, generative_model: GenerativeModel, inference_algorithm: InferenceAlgorithm):
        self.config = config
    ...
    def process_graph(self, node_features: torch.Tensor, edge_index: torch.Tensor, edge_features: Optional[torch.Tensor]=None, batch: Optional[torch.Tensor]=None) -> Dict[str, torch.Tensor]:
        """
        ...
        return {'node_features': graph_features, 'graph_features': aggregated_features, 'edge_index': edge_index}

    def graph_to_beliefs(self, graph_data: Dict[str, torch.Tensor], agent_node_indices: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
    ...
    def graph_to_observations(self, graph_data: Dict[str, torch.Tensor], observation_node_indices: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
    ...
    def update_beliefs_with_graph(self, current_beliefs: torch.Tensor, graph_data: Dict[str, torch.Tensor], agent_node_indices: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
        ...
        if isinstance(self.generative_model, DiscreteGenerativeModel):
            updated_beliefs = self.inference.infer_states(observations, self.generative_model, current_beliefs)
        else:
            updated_beliefs = self.inference.infer_states(observations, self.generative_model, current_beliefs)
        return updated_beliefs

    def compute_expected_free_energy_with_graph(self, policy: Any, graph_data: Dict[str, torch.Tensor], preferences: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
...
class GraphFeatureAggregator:
    ...
    def __init__(self, config: GNNIntegrationConfig):
        ...
        if config.aggregation_method == 'attention':
            self.attention = nn.Sequential(nn.Linear(config.output_dim, config.hidden_dim), nn.Tanh(), nn.Linear(config.hidden_dim, 1)).to(self.device)
...
class HierarchicalGraphIntegration:
    ...
    def __init__(self, config: GNNIntegrationConfig, level_configs: List[Dict[str, Any]]):
        self.config = config
    ...
    def set_generative_models(self, generative_models: List[GenerativeModel], inference_algorithms: List[InferenceAlgorithm]):
        ...
        for i, (gen_model, inf_algo) in enumerate(zip(generative_models, inference_algorithms)):
            self.level_adapters[i] = GNNActiveInferenceAdapter(self.config, self.level_gnns[i], gen_model, inf_algo)
    ...
    def process_hierarchical_graph(self, graph_data_per_level: List[Dict[str, torch.Tensor]]) -> List[Dict[str, torch.Tensor]]:
        ...
        for i, (graph_data, adapter) in enumerate(zip(graph_data_per_level, self.level_adapters)):
            if adapter is not None:
                processed = adapter.process_graph(graph_data['node_features'], graph_data['edge_index'], graph_data.get('edge_features'), graph_data.get('batch'))
                processed_levels.append(processed)
            else:
                processed_levels.append(graph_data)
        return processed_levels

    def hierarchical_belief_update(self, current_beliefs: List[torch.Tensor], graph_data_per_level: List[Dict[str, torch.Tensor]]) -> List[torch.Tensor]:
        """
        ...
        for i in range(self.num_levels):
            if self.level_adapters[i] is not None:
                ...
                processed = self.level_adapters[i].process_graph(graph_data['node_features'], graph_data['edge_index'], graph_data.get('edge_features'), graph_data.get('batch'))
                updated = self.level_adapters[i].update_beliefs_with_graph(current_beliefs[i], processed)
                updated_beliefs.append(updated)
            else:
                updated_beliefs.append(current_beliefs[i])
        return updated_beliefs

def create_gnn_adapter(adapter_type: str, config: Optional[GNNIntegrationConfig]=None, **kwargs) -> Union[GNNActiveInferenceAdapter, HierarchicalGraphIntegration]:
    """
