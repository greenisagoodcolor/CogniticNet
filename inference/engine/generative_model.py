...
class DiscreteGenerativeModel(GenerativeModel):
    ...
    def __init__(self, dimensions: ModelDimensions, parameters: ModelParameters):
        ...
        self.C = self._initialize_C()
        self.D = self._initialize_D()
    ...
    def set_preferences(self, preferences: Union[torch.Tensor, np.ndarray],
        ...
        if timestep is None:
            if preferences.dim() == 1:
                ...
                self.C = preferences.unsqueeze(1).repeat(1, self.dims.time_horizon)
            else:
                self.C = preferences.to(self.device)
        else:
...
class ContinuousGenerativeModel(GenerativeModel):
    ...
    def transition_model(self, states: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            states: Current states [batch_size x num_states] or [num_states]
            actions: Actions [batch_size x num_actions] or [num_actions]

        Returns:
            next_mean: Mean of next state distribution
            next_var: Variance of next state distribution
        """
        if states.dim() == 1:
            states = states.unsqueeze(0)
        if actions.dim() == 1:
