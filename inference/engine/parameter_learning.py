...
class ParameterLearner(ABC):
    ...
    @abstractmethod
    def update_parameters(self, experiences: List[Experience], generative_model: GenerativeModel) -> Dict[str, float]:
...
class DiscreteParameterLearner(ParameterLearner):
    ...
    def __init__(self, config: LearningConfig, model_dims: Dict[str, int]):
        ...
        if config.use_bayesian_learning:
            self.pA = torch.ones(self.num_observations, self.num_states, device=self.device) * config.concentration_A
            self.pB = torch.ones(self.num_states, self.num_states, self.num_actions, device=self.device) * config.concentration_B
    ...
    def update_parameters(self, experiences: List[Experience], generative_model: DiscreteGenerativeModel) -> Dict[str, float]:
        """
    ...
    def _bayesian_update(self, experiences: List[Experience], model: DiscreteGenerativeModel) -> Dict[str, float]:
        ...
        metrics = {'A_update_norm': torch.norm(A_counts).item(), 'B_update_norm': torch.norm(B_counts).item(), 'D_update_norm': torch.norm(D_counts).item(), 'A_entropy': self._compute_entropy(model.A).mean().item(), 'B_entropy': self._compute_entropy(model.B).mean().item()}
        return metrics

    def _gradient_update(self, experiences: List[Experience], model: DiscreteGenerativeModel) -> Dict[str, float]:
        ...
        metrics = {'grad_A_norm': torch.norm(grad_A).item(), 'grad_B_norm': torch.norm(grad_B).item(), 'grad_D_norm': torch.norm(grad_D).item()}
        return metrics
...
class ContinuousParameterLearner(ParameterLearner):
    ...
    def __init__(self, config: LearningConfig, model: ContinuousGenerativeModel):
        super().__init__(config)
        self.model = model
        self.optimizers = {}
        if hasattr(model, 'transition_net'):
            self.optimizers['transition'] = torch.optim.Adam(model.transition_net.parameters(), lr=config.learning_rate_B)
        if hasattr(model, 'observation_net'):
            self.optimizers['observation'] = torch.optim.Adam(model.observation_net.parameters(), lr=config.learning_rate_A)
        if hasattr(model, 'prior_net'):
            self.optimizers['prior'] = torch.optim.Adam(model.prior_net.parameters(), lr=config.learning_rate_D)
        self.schedulers = {name: torch.optim.lr_scheduler.ExponentialLR(opt, self.config.decay_rate) for name, opt in self.optimizers.items()}

    def update_parameters(self, experiences: List[Experience], generative_model: ContinuousGenerativeModel) -> Dict[str, float]:
        """
        ...
        if 'transition' in self.optimizers:
            trans_loss = self._update_transition_model(states, actions, next_states, generative_model)
    ...
    def _update_transition_model(self, states: torch.Tensor, actions: torch.Tensor, next_states: torch.Tensor, model: ContinuousGenerativeModel) -> float:
    ...
    def _update_observation_model(self, states: torch.Tensor, observations: torch.Tensor, model: ContinuousGenerativeModel) -> float:
...
class OnlineParameterLearner:
    ...
    def __init__(self, config: LearningConfig, generative_model: GenerativeModel, parameter_learner: ParameterLearner):
        self.config = config
    ...
    def observe(self, state: torch.Tensor, action: torch.Tensor, observation: torch.Tensor, next_state: torch.Tensor, reward: Optional[torch.Tensor]=None):
        """
        ...
        """
        experience = Experience(state=state.detach(), action=action.detach(), observation=observation.detach(), next_state=next_state.detach(), reward=reward.detach() if reward is not None else None, timestamp=self.total_experiences)
    ...
    def _should_update(self) -> bool:
        ...
        if self.replay_buffer is None:
            return True
        return len(self.replay_buffer) >= self.config.min_buffer_size and self.total_experiences % self.config.update_frequency == 0

    def get_statistics(self) -> Dict[str, Any]:
        """Get learning statistics"""
        stats = {'total_experiences': self.total_experiences, 'buffer_size': len(self.replay_buffer) if self.replay_buffer else 0, 'num_updates': len(self.update_metrics), 'learning_rates': self.parameter_learner.get_learning_rates()}
...
def create_parameter_learner(learner_type: str, config: Optional[LearningConfig]=None, **kwargs) -> Union[ParameterLearner, OnlineParameterLearner]:
    """
    ...
    if learner_type == 'discrete':
        ...
        if isinstance(generative_model, DiscreteGenerativeModel):
            model_dims = {'num_states': generative_model.dims.num_states, 'num_observations': generative_model.dims.num_observations, 'num_actions': generative_model.dims.num_actions}
