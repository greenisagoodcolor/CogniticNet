...
class DemoSimulator:
    ...
    def __init__(self):
        ...
        self.simulation_speed = float(
            os.environ.get("SIMULATION_SPEED", "10").rstrip("x")
        )
            ...
            with self.db_conn.cursor() as cursor:
                if action_result["success"]:
                    ...
                    if action_result["action"] in [
                        ...
                        updates.append(
                            "knowledge_items_shared = knowledge_items_shared + 1"
                        )
        ...
        while self.running:
            try:
                ...
                self.update_agent_stats(
                    agent1["id"], {"action": "negotiate", "success": True}
                )
                self.update_agent_stats(
                    agent2["id"], {"action": "negotiate", "success": True}
                )
