...
try:
    from ...agents.active_inference import create_generative_model, create_policy_selector, DiscreteGenerativeModel, DiscreteExpectedFreeEnergy
...
class GoalUtility(UtilityFunction):
    ...
    def calculate(self, action: Action, context: DecisionContext) -> float:
        ...
        if action.action_type == ActionType.INTERACT and goal.target_agent_id and hasattr(action.target, 'agent_id') and (goal.target_agent_id == action.target.agent_id):
    ...
    def calculate(self, action: Action, context: DecisionContext) -> float:
        ...
        if action.action_type == ActionType.MOVE and energy_ratio < 0.3:
            ...
            if resources and action.target:
                ...
                current_distance = context.agent.position.distance_to(closest_resource.stimulus.position)
...
class DecisionMaker:
    ...
    def __init__(self, strategy: DecisionStrategy=DecisionStrategy.UTILITY_BASED):
        ...
        self.utility_functions: List[UtilityFunction] = [SafetyUtility(), GoalUtility(), ResourceUtility(), SocialUtility()]
        self.utility_weights: Dict[type, float] = {SafetyUtility: 2.0, GoalUtility: 1.5, ResourceUtility: 1.0, SocialUtility: 0.8}
    ...
    def _reactive_decision(self, context: DecisionContext) -> Optional[Action]:
        ...
        threats = context.get_perceived_threats()
        if threats:
            flee_actions = [a for a in context.available_actions if a.action_type == ActionType.FLEE]
            if flee_actions:
                return flee_actions[0]
        if context.agent.resources.energy < 20:
            gather_actions = [a for a in context.available_actions if a.action_type == ActionType.GATHER]
            if gather_actions:
    ...
    def _active_inference_decision(self, context: DecisionContext) -> Optional[Action]:
        ...
        if num_actions == 0:
            return None
        model = create_generative_model(model_type='discrete', num_states=num_states, num_observations=num_observations, num_actions=num_actions)
        policy_selector = create_policy_selector(selector_type='discrete', generative_model=model, planning_horizon=3)
...
class ConditionNode(BehaviorNode):
    ...
    def __init__(self, condition: Callable[[DecisionContext], bool], true_node: BehaviorNode, false_node: Optional[BehaviorNode]=None):
...
class ActionGenerator:
    ...
    def _generate_movement_actions(self, agent: Agent, percepts: List[Percept]) -> List[Action]:
        ...
        for percept in percepts[:5]:
            move_action = Action(ActionType.MOVE, target=percept.stimulus.position, cost=percept.distance * 0.1, duration=percept.distance / 5.0)
            actions.append(move_action)
        threats = [p for p in percepts if p.stimulus.stimulus_type == StimulusType.DANGER]
        if threats:
            actions.append(Action(ActionType.FLEE, cost=agent.resources.energy * 0.1, duration=2.0, parameters={'urgency': 'high'}))
        return actions
    ...
    def _generate_interaction_actions(self, agent: Agent, percepts: List[Percept]) -> List[Action]:
        ...
        for percept in agent_percepts[:3]:
            if percept.distance < 5.0:
                actions.append(Action(ActionType.INTERACT, target=percept.stimulus.source, cost=1.0, duration=3.0))
        for percept in agent_percepts:
            if percept.distance > 5.0 and percept.distance < 20.0:
                actions.append(Action(ActionType.APPROACH, target=percept.stimulus.source, cost=percept.distance * 0.05, duration=percept.distance / 5.0))
        return actions

    def _generate_communication_actions(self, agent: Agent, percepts: List[Percept]) -> List[Action]:
        """Generate communication actions"""
        actions = []
        agent_percepts = [p for p in percepts if p.stimulus.stimulus_type == StimulusType.AGENT and p.distance < 10.0]
        if agent_percepts:
            actions.append(Action(ActionType.COMMUNICATE, cost=0.5, duration=1.0, parameters={'message_type': 'greeting'}))
        return actions
    ...
    def _generate_resource_actions(self, agent: Agent, percepts: List[Percept]) -> List[Action]:
        ...
        for percept in resource_percepts:
            if percept.distance < 2.0:
                actions.append(Action(ActionType.GATHER, target=percept.stimulus, cost=2.0, duration=3.0, effects={'energy': 20.0}))
        return actions

class DecisionSystem:
    """Main decision-making system integrating all components"""

    def __init__(self, state_manager: AgentStateManager, perception_system: PerceptionSystem, movement_controller: MovementController):
    ...
    def register_agent(self, agent: Agent, strategy: DecisionStrategy=DecisionStrategy.HYBRID) -> None:
    ...
    def make_decision(self, agent_id: str) -> Optional[Action]:
        ...
        context = DecisionContext(agent=agent, percepts=percepts, current_goal=agent.current_goal, available_actions=available_actions, time_pressure=self._calculate_time_pressure(agent))
    ...
    def execute_action(self, agent_id: str, action: Action) -> bool:
        ...
        if action.action_type == ActionType.MOVE and action.target:
            ...
            threats = [p for p in self.perception_system.perceive(agent_id) if p.stimulus.stimulus_type == StimulusType.DANGER]
            if threats:
                ...
                flee_pos = Position(agent.position.x + flee_dir[0], agent.position.y + flee_dir[1], agent.position.z)
    ...
    def _create_default_behavior_tree(self) -> BehaviorTree:
        ...
        tree = BehaviorTree()
        root = SelectorNode([ConditionNode(lambda ctx: len(ctx.get_perceived_threats()) > 0, ActionNode(lambda ctx: next((a for a in ctx.available_actions if a.action_type == ActionType.FLEE), None))), ConditionNode(lambda ctx: ctx.agent.resources.energy < 20, ActionNode(lambda ctx: next((a for a in ctx.available_actions if a.action_type == ActionType.GATHER), None))), ConditionNode(lambda ctx: ctx.current_goal is not None, ActionNode(lambda ctx: ctx.decision_maker._goal_oriented_decision(ctx) if hasattr(ctx, 'decision_maker') else None)), ActionNode(lambda ctx: next((a for a in ctx.available_actions if a.action_type in [ActionType.EXPLORE, ActionType.WAIT]), None))])
        tree.root = root
        return tree
