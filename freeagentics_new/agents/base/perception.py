...
class PerceptionCapabilities:
    ...
    enabled_types: Set[PerceptionType] = field(default_factory=lambda: {PerceptionType.VISUAL, PerceptionType.AUDITORY, PerceptionType.PROXIMITY})
...
class AttentionFilter(PerceptionFilter):
    ...
    def filter(self, percepts: List[Percept], agent: Agent) -> List[Percept]:
        ...
        sorted_percepts = sorted(percepts, key=lambda p: p.confidence * p.metadata.get('importance', 1.0) / (p.distance + 1), reverse=True)
        return sorted_percepts[:capacity]

class PerceptionMemory:
    """Manages short-term perception memory"""

    def __init__(self, memory_duration: float=5.0, max_memories: int=100):
    ...
    def forget_old_memories(self) -> None:
        ...
        while self.memories and self.memories[0].timestamp < cutoff_time:
            ...
            if stimulus_id in self.stimulus_history:
                self.stimulus_history[stimulus_id] = [p for p in self.stimulus_history[stimulus_id] if p.timestamp >= cutoff_time]
...
class SensorSystem:
    ...
    def __init__(self, perception_type: PerceptionType):
        self.perception_type = perception_type

    def sense(self, agent: Agent, stimuli: List[Stimulus], environment: Optional[Any]=None) -> List[Percept]:
...
class VisualSensor(SensorSystem):
    ...
    def sense(self, agent: Agent, stimuli: List[Stimulus], environment: Optional[Any]=None) -> List[Percept]:
        ...
        for stimulus in stimuli:
            if hasattr(stimulus.source, 'agent_id') and stimulus.source.agent_id == agent.agent_id:
                continue
            ...
            percept = Percept(stimulus=stimulus, perception_type=self.perception_type, confidence=confidence, distance=distance, direction=direction, metadata={'angle_from_forward': angle, 'in_peripheral': angle > capabilities.field_of_view / 4})
...
class AuditorySensor(SensorSystem):
    ...
    def sense(self, agent: Agent, stimuli: List[Stimulus], environment: Optional[Any]=None) -> List[Percept]:
        ...
        for stimulus in stimuli:
            ...
            percept = Percept(stimulus=stimulus, perception_type=self.perception_type, confidence=confidence, distance=distance, direction=direction, metadata={'perceived_intensity': perceived_intensity, 'can_localize': confidence > 0.5})
            percepts.append(percept)
        return percepts
...
class ProximitySensor(SensorSystem):
    ...
    def sense(self, agent: Agent, stimuli: List[Stimulus], environment: Optional[Any]=None) -> List[Percept]:
        ...
        for stimulus in stimuli:
            ...
            percept = Percept(stimulus=stimulus, perception_type=self.perception_type, confidence=confidence, distance=distance, direction=direction, metadata={'is_touching': distance < 0.5})
            percepts.append(percept)
        return percepts
...
class PerceptionSystem:
    ...
    def __init__(self, state_manager: AgentStateManager):
        ...
        self.sensors: Dict[PerceptionType, SensorSystem] = {PerceptionType.VISUAL: VisualSensor(), PerceptionType.AUDITORY: AuditorySensor(), PerceptionType.PROXIMITY: ProximitySensor()}
    ...
    def register_agent(self, agent: Agent, capabilities: Optional[PerceptionCapabilities]=None) -> None:
    ...
    def create_agent_stimulus(self, agent: Agent) -> Stimulus:
        ...
        return Stimulus(stimulus_id=f'agent_{agent.agent_id}', stimulus_type=StimulusType.AGENT, position=agent.position, intensity=1.0, source=agent, metadata={'agent_name': agent.name, 'agent_status': agent.status.value})

    def create_sound_stimulus(self, position: Position, intensity: float, radius: float, source: Optional[Any]=None) -> Stimulus:
        """Create a sound stimulus"""
        import uuid
        return Stimulus(stimulus_id=f'sound_{uuid.uuid4().hex[:8]}', stimulus_type=StimulusType.SOUND, position=position, intensity=intensity, radius=radius, source=source, metadata={'sound_type': 'generic'})
    ...
    def can_perceive(self, observer_id: str, target_id: str, perception_type: PerceptionType=PerceptionType.VISUAL) -> bool:
