...
class EmbeddingSystem:
    ...
    def create_experience_embedding(self, experience: ExperienceNode) -> np.ndarray:
        ...
        location_hash = int(
            hashlib.md5(experience.location.encode()).hexdigest()[:8], 16
        )
...
class AgentKnowledgeGraph:
    ...
    def add_experience(self, experience: ExperienceNode) -> str:
        ...
        for sim_id, similarity in similar_experiences:
            self.graph.add_edge(
                experience.id, sim_id, weight=similarity, relation="similar_to"
            )
            # Bidirectional similarity
            self.graph.add_edge(
                sim_id, experience.id, weight=similarity, relation="similar_to"
            )
    ...
    def add_pattern(self, pattern: PatternNode) -> str:
        ...
        # Add node to graph
        self.graph.add_node(pattern.id, type=NodeType.PATTERN, data=pattern)

        # Connect to source experiences
        for exp_id in pattern.source_experiences:
            if self.graph.has_node(exp_id):
                self.graph.add_edge(pattern.id, exp_id, relation="derived_from")
        ...
        for sim_id, similarity in similar_patterns:
            self.graph.add_edge(
                pattern.id, sim_id, weight=similarity, relation="similar_to"
            )
    ...
    def add_belief(self, belief: BeliefNode) -> str:
        ...
        # Add node to graph
        self.graph.add_node(belief.id, type=NodeType.BELIEF, data=belief)

        # Connect to supporting/contradicting patterns
        for pat_id in belief.supporting_patterns:
            if self.graph.has_node(pat_id):
                self.graph.add_edge(belief.id, pat_id, relation="supported_by")

        for pat_id in belief.contradicting_patterns:
            if self.graph.has_node(pat_id):
                self.graph.add_edge(belief.id, pat_id, relation="contradicted_by")
    ...
    def query_similar_experiences(
        self, observation: Dict[str, Any], k: int = 5
    ) -> List[Tuple[ExperienceNode, float]]:
    ...
    def get_applicable_patterns(
        self, observation: Dict[str, Any], threshold: float = 0.6
    ) -> List[PatternNode]:
        ...
        for node_id, node_data in self.graph.nodes(data=True):
            if node_data["type"] == NodeType.PATTERN:
                ...
                match_score = self._calculate_condition_match(
                    pattern.trigger_conditions, observation
                )
    ...
    def update_pattern_success(self, pattern_id: str, success: bool):
        ...
        pattern.confidence = (
            min(1.0, pattern.occurrence_count / 100.0) * pattern.success_rate
        )

        logger.debug(
            f"Updated pattern {pattern_id}: success_rate={pattern.success_rate:.2f}"
        )
    ...
    def _find_similar_experiences(
        self, experience: ExperienceNode, k: int = 5
        ...
        for node_id, node_data in self.graph.nodes(data=True):
            if node_data["type"] == NodeType.EXPERIENCE and node_id != experience.id:
                node_embedding = self.embeddings_cache.get(node_id)
                if node_embedding is not None:
                    similarity = self.embedding_system.calculate_similarity(
                        experience.embedding, node_embedding
                    )
    ...
    def _find_similar_patterns(
        self, pattern: PatternNode, k: int = 3
    ) -> List[Tuple[str, float]]:
        ...
        for node_id, node_data in self.graph.nodes(data=True):
            if node_data["type"] == NodeType.PATTERN and node_id != pattern.id:
                node_embedding = self.embeddings_cache.get(node_id)
                if node_embedding is not None:
                    similarity = self.embedding_system.calculate_similarity(
                        pattern.embedding, node_embedding
                    )
                    if similarity >= self.SIMILARITY_THRESHOLD:
                        similarities.append((node_id, similarity))
    ...
    def _extract_patterns(self):
        ...
        recent_experiences.sort(key=lambda x: x[1].timestamp, reverse=True)
        ...
        for action_key, occurrences in action_sequences.items():
            if len(occurrences) >= 2:  # Pattern must occur at least twice
                ...
                pattern = PatternNode(
                    ...
                    success_rate=(
                        0.5 if avg_fe_delta < 0 else 0.8
                    ),  # FE reduction = success
                    source_experiences=[
                        exp_id for o in occurrences for exp_id in o["experiences"]
                    ],
                )
    ...
    def _calculate_condition_match(
        self, conditions: Dict[str, Any], observation: Dict[str, Any]
    ) -> float:
    ...
    def save_to_file(self, filepath: str):
        ...
        for source, target, edge_data in self.graph.edges(data=True):
            data["edges"].append(
                {"source": source, "target": target, "data": edge_data}
            )
...
if __name__ == "__main__":
    ...
    patterns = kg.get_applicable_patterns({"resources": {"food": 30, "water": 70}})
