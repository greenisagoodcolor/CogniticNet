...
from ..models.validated_models import Position, ResourceType
                ...
                "cells": [
                    {
                        ...
                        "distance": h3.h3_distance(self.current_cell.hex_id, cell.hex_id),
            ...
            "movement_options": [
                {
                    ...
                    "hex_id": hex_id,
                    "terrain": next((c.terrain.value for c in self.visible_cells if c.hex_id == hex_id), "unknown")
                }
...
class MovementPerceptionSystem:
    ...
    def __init__(self, world: H3World):
        """
    ...
    def get_valid_moves(self, current_hex: str) -> List[Tuple[Direction, str]]:
        ...
        current_lat, current_lng = h3.h3_to_geo(current_hex)
        ...
        for neighbor_hex in neighbors:
            ...
            if neighbor_hex in self.world.cells:
                ...
                neighbor_lat, neighbor_lng = h3.h3_to_geo(neighbor_hex)
    ...
    def _calculate_direction(self, 
                            ...
                            to_lat: float, to_lng: float) -> Direction:
    ...
    def can_move_to(self, from_hex: str, to_hex: str) -> Tuple[bool, Optional[str]]:
        ...
        if h3.h3_distance(from_hex, to_hex) != 1:
    ...
    def get_agent_observation(self,
                             agent_position: str,
                             other_agents: List[Dict[str, Any]] = None) -> Observation:
        """
    ...
    def _apply_line_of_sight(self,
                            observer_hex: str,
                            potential_visible: List[HexCell]) -> List[HexCell]:
        """
        ...
        for target_cell in potential_visible:
            ...
            path = h3.h3_line(observer_hex, target_cell.hex_id)
            ...
            for i, hex_id in enumerate(path[1:-1], 1):  # Skip observer and target
                ...
                progress = i / len(path)
                expected_elevation = (
                    observer_elevation * (1 - progress) +
                    target_cell.elevation * progress
                )
    ...
    def find_path_astar(self,
                       start_hex: str,
                       goal_hex: str,
                       max_cost: float = 100.0) -> Optional[List[str]]:
        """
        ...
        f_score = {start_hex: h3.h3_distance(start_hex, goal_hex)}
        ...
        while open_set:
            ...
            for direction, neighbor in self.get_valid_moves(current):
                ...
                if neighbor not in g_score or tentative_g < g_score[neighbor]:
                    ...
                    h_score = h3.h3_distance(neighbor, goal_hex) * 1.0
    ...
    def get_exploration_targets(self,
        ...
        for cell in nearby_cells:
            ...
            distance = h3.h3_distance(current_hex, cell.hex_id)
            if distance == 0:
                continue
