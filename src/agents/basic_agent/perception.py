...
class Stimulus:
    ...
    radius: float = 0.0  # For area effects
...
class PerceptionCapabilities:
    ...
    field_of_view: float = math.pi * 2/3  # 120 degrees in radians
    visual_acuity: float = 1.0  # Affects detail perception

    # Auditory perception
    hearing_range: float = 100.0
    hearing_sensitivity: float = 1.0  # Affects quiet sound detection
    ...
    attention_capacity: int = 10  # max simultaneous percepts

    # Enabled perception types
    enabled_types: Set[PerceptionType] = field(default_factory=lambda: {
...
class ImportanceFilter(PerceptionFilter):
    ...
    def _calculate_importance(self, percept: Percept, agent: Agent) -> float:
        ...
        # Closer things are more important
        if percept.distance < 10.0:
            importance *= 1.5

        # Dangerous stimuli are always important
...
class AttentionFilter(PerceptionFilter):
    ...
    def filter(self, percepts: List[Percept], agent: Agent) -> List[Percept]:
        ...
        if not hasattr(agent, "perception_capabilities"):
            return percepts[:10]  # Default capacity

        capacity = agent.perception_capabilities.attention_capacity

        # Sort by salience (combination of confidence, distance, importance)
...
class PerceptionMemory:
    ...
    def __init__(self, memory_duration: float = 5.0, max_memories: int = 100):
        self.memory_duration = memory_duration  # seconds
...
class SensorSystem:
    ...
    def sense(self, agent: Agent, stimuli: List[Stimulus],
...
class VisualSensor(SensorSystem):
    ...
    def sense(self, agent: Agent, stimuli: List[Stimulus],
        ...
        # Get agent's forward direction from orientation
        ...
        for stimulus in stimuli:
            ...
            # Check line of sight (simplified - would need environment for proper check)
            ...
            # Calculate confidence based on distance and acuity
...
class AuditorySensor(SensorSystem):
    ...
    def sense(self, agent: Agent, stimuli: List[Stimulus],
        ...
        for stimulus in stimuli:
            ...
            # Apply distance decay to intensity
            ...
            # Calculate confidence based on intensity and sensitivity
            ...
            percept = Percept(
                ...
                metadata={
                    ...
                    "can_localize": confidence > 0.5  # Can determine direction if loud enough
                }
            )
...
class ProximitySensor(SensorSystem):
    ...
    def sense(self, agent: Agent, stimuli: List[Stimulus],
        ...
        for stimulus in stimuli:
            ...
            # Proximity confidence is inverse of distance
            ...
            percept = Percept(
                ...
                direction=direction,
                metadata={
                    "is_touching": distance < 0.5  # Very close
                }
            )
...
class PerceptionSystem:
    ...
    def register_agent(self, agent: Agent,
        ...
        # Store capabilities on agent for sensor access
    ...
    def perceive(self, agent_id: str, environment: Optional[Any] = None) -> List[Percept]:
        ...
        # Collect percepts from all enabled sensors
        all_percepts = []
    ...
    def create_sound_stimulus(self, position: Position, intensity: float,
                            radius: float, source: Optional[Any] = None) -> Stimulus:
    ...
    def can_perceive(self, observer_id: str, target_id: str,
                    perception_type: PerceptionType = PerceptionType.VISUAL) -> bool:
        ...
        # Create temporary stimulus for target
        target_stimulus = self.create_agent_stimulus(target)

        # Check if sensor would detect it
        if perception_type in self.sensors:
            sensor = self.sensors[perception_type]
