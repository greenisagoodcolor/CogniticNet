...
class MovementConstraints:
    ...
    max_speed: float = 5.0  # units per second
    max_acceleration: float = 2.0  # units per second^2
    max_turn_rate: float = math.pi  # radians per second
    collision_radius: float = 0.5  # units
    step_height: float = 0.5  # maximum step height

    # Mode-specific speed multipliers
    mode_speeds: Dict[MovementMode, float] = field(default_factory=lambda: {
    ...
    terrain_speeds: Dict[TerrainType, float] = field(default_factory=lambda: {
...
class CollisionSystem:
    ...
    def check_collision(self, position: Position, radius: float,
        ...
        for obs_id, obs_pos in self.dynamic_obstacles.items():
            if obs_id != exclude_id:
                ...
                if distance < radius * 2:  # Assume all agents have same radius
                    return True
    ...
    def get_collision_normal(self, position: Position, radius: float) -> Optional[np.ndarray]:
        ...
        for obstacle in self.static_obstacles:
            ...
            if penetration > 0 and distance < min_distance:
                ...
                # Normal points away from obstacle
...
class PathfindingGrid:
    ...
    def get_movement_cost(self, from_pos: Tuple[int, int],
...
class MovementController:
    ...
    def register_agent(self, agent: Agent,
                      constraints: Optional[MovementConstraints] = None) -> None:
    ...
    def update(self, delta_time: float) -> None:
        ...
        for agent_id, state in self.movement_states.items():
            ...
            if state.path and state.path_index < len(state.path):
                ...
                # No active path, apply physics
                self._update_physics(agent, state, delta_time)

    def _update_path_following(self, agent: Agent, state: MovementState,
        ...
        if distance > 0:
            ...
            # Apply speed based on mode and terrain
    ...
    def _update_physics(self, agent: Agent, state: MovementState,
                       delta_time: float) -> None:
        ...
        if not state.is_grounded and state.mode != MovementMode.FLYING:
            state.velocity[2] -= 9.8 * delta_time  # Gravity
    ...
    def _apply_movement(self, agent: Agent, state: MovementState,
                       delta_time: float) -> None:
        ...
        if not self.collision_system.check_collision(
            ...
            agent.velocity = state.velocity  # Update agent's velocity
            ...
            if normal is not None:
                ...
                # Try movement again with adjusted velocity
                movement = state.velocity * delta_time
                new_pos_array = agent.position.to_array() + movement
                new_position = Position(new_pos_array[0], new_pos_array[1], new_pos_array[2])
    ...
    def apply_force(self, agent_id: str, force: np.ndarray) -> None:
        ...
        # F = ma, so a = F/m (assume mass = 1 for simplicity)
        acceleration = force
        ...
        state.velocity += acceleration  # Will be applied over time in update

        # Set agent to moving if not already
        agent = self.state_manager.get_agent(agent_id)
    ...
    def get_movement_info(self, agent_id: str) -> Optional[Dict]:
        ...
        return {
            ...
            } if state.destination else None
        }
...
class SteeringBehaviors:
    ...
    @staticmethod
    def arrive(position: np.ndarray, target: np.ndarray, max_speed: float,
    ...
    @staticmethod
    def wander(velocity: np.ndarray, wander_angle: float,
               wander_rate: float, max_speed: float) -> Tuple[np.ndarray, float]:
        ...
        if np.linalg.norm(velocity) > 0:
            # Use current velocity direction as base
        ...
        wander_force = np.array([
            np.cos(new_angle) * max_speed,
            np.sin(new_angle) * max_speed,
            0
        ])
    ...
    @staticmethod
    def separate(position: np.ndarray, neighbors: List[np.ndarray],
                 separation_radius: float, max_speed: float) -> np.ndarray:
