...
class Memory:
    ...
    def calculate_strength(self, current_time: Optional[datetime] = None) -> float:
        ...
        time_elapsed = (current_time - self.timestamp).total_seconds() / 3600  # Hours
...
class Pattern:
    ...
    pattern_id: str
    pattern_type: str  # "action_outcome", "stimulus_response", etc.
...
class MemoryConsolidator:
    ...
    def consolidate(self, memories: List[Memory]) -> List[Memory]:
        ...
        # Group memories by type and context similarity
        groups = self._group_similar_memories(memories)

        for group in groups:
            if len(group) >= 3:  # Need enough memories to form a pattern
    ...
    def _are_similar(self, memory1: Memory, memory2: Memory) -> bool:
        ...
        matches = sum(1 for k in common_keys
                     if memory1.context[k] == memory2.context[k])
    ...
    def _create_abstract_memory(self, memories: List[Memory]) -> Optional[Memory]:
        ...
        for key in all_keys:
            ...
            if len(values) == len(memories):  # Present in all memories
...
class PatternRecognizer:
    ...
    def extract_patterns(self, experiences: List[Experience]) -> List[Pattern]:
        ...
        for action_key, outcomes in action_outcomes.items():
            if len(outcomes) >= 3:  # Need enough data
    ...
    def _conditions_match(self, pattern_conditions: Dict[str, Any],
                         test_conditions: Dict[str, Any]) -> bool:
...
class MemorySystem:
    ...
    def store_memory(self, content: Any, memory_type: MemoryType,
                    importance: float = 0.5, context: Optional[Dict[str, Any]] = None) -> Memory:
        """Store a new memory"""
        import uuid
    ...
    def retrieve_memories(self, criteria: Dict[str, Any],
    ...
    def get_relevant_memories(self, context: DecisionContext,
        ...
        # Filter by relevance to current situation
        relevant = []
        for memory in memories:
    ...
    def _calculate_relevance(self, memory: Memory, context: DecisionContext) -> float:
        ...
        if memory.memory_type == MemoryType.PROCEDURAL:
            relevance += 0.3  # Procedural memories often relevant

        # Context similarity
        if memory.context:
            # Check for similar percepts
            if "percept_types" in memory.context:
                current_types = {p.stimulus.stimulus_type.value
                               for p in context.percepts}
    ...
    def consolidate_memories(self) -> None:
        ...
        to_consolidate = [m for m in recent_memories
                         if self.consolidator.evaluate_for_consolidation(m)]

        if len(to_consolidate) >= 3:
            # Consolidate into abstract memories
            consolidated = self.consolidator.consolidate(to_consolidate)

            # Store consolidated memories as semantic
    ...
    def extract_patterns(self) -> List[Pattern]:
        ...
        # Store patterns in pattern recognizer and as procedural memories
        for pattern in patterns:
            # Add to pattern recognizer's patterns dict
    ...
    def forget_old_memories(self, max_age_days: int = 30) -> int:
        ...
        for memory in old_memories:
            # Check both age and low importance/strength
    ...
    def save_to_disk(self, filepath: Path) -> None:
        ...
        data = {
            ...
            "memories": [m for m in self.storage.memories.values()],
    ...
    def get_memory_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "oldest_memory": min([m.timestamp for m in self.storage.memories.values()])
                           if self.storage.memories else None,
            "average_importance": np.mean([m.importance for m in self.storage.memories.values()])
                                if self.storage.memories else 0.0
        }
