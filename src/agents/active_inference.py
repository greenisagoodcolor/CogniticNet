...
from ..gnn.executor import GNNExecutor
from ..knowledge.knowledge_graph import AgentKnowledgeGraph
from ..models.validated_models import (
    Position, ResourceType, MessageType, AgentStatus
)
...
class ActiveInferenceAgent:
    ...
    def perceive(self, observation: Observation) -> None:
        ...
        logger.debug(f"Agent {self.id} perceived {observation.type}, updated {len(belief_update)} beliefs")
    ...
    def _calculate_expected_free_energy(
        ...
        energy_cost = self.generative_model['transition_model'].get(
            action_type, {}
        ).get('energy_cost', 0.0)
    ...
    def _execute_action(self, action: Action) -> None:
        ...
        logger.debug(f"Agent {self.id} executed action {action.type} with cost {action.energy_cost}")
    ...
    def integrate_knowledge(self, knowledge_package: Dict[str, Any]) -> None:
        ...
        for belief_key, other_belief in knowledge_package.get('beliefs', {}).items():
            if belief_key in self.beliefs:
                ...
                other_conf = other_belief.confidence if hasattr(other_belief, 'confidence') else other_belief
                ...
                self.beliefs[belief_key] = Belief(
                    state=belief_key,
                    confidence=trust_factor * (other_belief.confidence if hasattr(other_belief, 'confidence') else other_belief),
