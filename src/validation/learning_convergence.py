...
class LearningConvergenceValidator:
    ...
    def _generate_recommendations(
        ...
        if not is_converged:
            ...
            if plateau_detected:
                recommendations.append("Consider increasing learning rate or using momentum")
                recommendations.append("Check if model capacity is sufficient")
            
            if divergence_risk > 0.5:
                recommendations.append("High divergence risk - reduce learning rate immediately")
            ...
            if gradient_norms and np.max(gradient_norms) > 100:
                recommendations.append("Gradient explosion detected - use gradient clipping")
