...
class GraphConsistencyValidator:
    ...
    def validate_graph(self, graph: nx.DiGraph) -> ConsistencyReport:
        ...
        if metrics["node_count"] > self.max_graph_size:
            errors.append(
                f"Graph exceeds maximum size: {metrics['node_count']} > {self.max_graph_size}"
            )
        ...
        for edge_type in ["precedes"]:  # Temporal edges should not have cycles
            ...
            if subgraph.number_of_edges() > 0:
                try:
                    ...
                    if cycles:
                        errors.append(
                            f"Found {len(cycles)} cycles in {edge_type} relationships"
                        )
                except:
            ...
            if attrs.get("type") == "supports":
                ...
                if graph.has_edge(u, v):
                    ...
                    if reverse_attrs.get("type") == "contradicts":
                        errors.append(
                            f"Nodes {u} and {v} have both 'supports' and 'contradicts' relationships"
                        )
        ...
        for node_id, attrs in graph.nodes(data=True):
            if "embedding" in attrs:
                ...
                try:
                    ...
                    if emb_array.ndim != 1:
                        errors.append(
                            f"Node {node_id} has invalid embedding dimensionality"
                        )
                    else:
                    ...
                    if np.any(np.isnan(emb_array)) or np.any(np.isinf(emb_array)):
                        errors.append(
                            f"Node {node_id} has NaN or inf values in embedding"
                        )
