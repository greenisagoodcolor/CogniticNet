...
class OllamaManager:
    ...
    async def generate(self, model: str, prompt: str, 
                      stream: bool = False, **options) -> AsyncIterator[str]:
        """
    ...
    def recommend_model(self, ram_gb: float, storage_gb: float,
        ...
        logger.info(f"Recommended model for {ram_gb}GB RAM, {storage_gb}GB storage, "
                   f"{use_case} use case: {recommended}")
        
        return recommended
    
    async def optimize_model_config(self, model_name: str, 
                                  hardware_config: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def create_modelfile(self, base_model: str, system_prompt: str,
...
Be vigilant, protective, and responsive."""
    ...
    async def think(self, context: str, options: Optional[Dict[str, Any]] = None) -> str:
        ...
        # Generate response
        response_parts = []
        async for chunk in self.manager.generate(
            model, 
            context,
            stream=True,
            **(options or {})
