...
class ModelCompressor:
    ...
    def compress_model(self, model_data: Dict[str, Any], 
                      level: CompressionLevel = CompressionLevel.MEDIUM) -> Tuple[Dict[str, Any], CompressionStats]:
        """
        ...
        if 'gnn_model' in compressed:
            ...
            if config['quantize_bits'] < 32:
                quant_stats = self._quantize_parameters(
                    gnn, config['quantize_bits']
                )
            ...
            if config['prune_threshold'] > 0:
                prune_stats = self._prune_graph(
                    gnn, config['prune_threshold']
                )
    ...
    def _prune_graph(self, gnn: Dict[str, Any], threshold: float) -> Dict[str, Any]:
        ...
        if 'nodes' in gnn:
            ...
            gnn['nodes'] = [
                node for node in gnn['nodes']
                if node_importance.get(node['id'], 0) >= threshold
            ]
        ...
        if 'edges' in gnn:
            ...
            gnn['edges'] = [
                edge for edge in gnn['edges']
                if (abs(edge.get('weight', 1.0)) >= threshold and
                    edge.get('source') in remaining_nodes and
                    edge.get('target') in remaining_nodes)
            ]
    ...
    def validate_compressed_model(self, original: Dict[str, Any], 
                                compressed: Dict[str, Any]) -> Dict[str, bool]:
        """
        ...
        try:
            ...
            orig_critical = {
                n['id'] for n in orig_gnn.get('nodes', [])
                if n.get('type') in critical_node_types
            }
            comp_critical = {
                n['id'] for n in comp_gnn.get('nodes', [])
                if n.get('type') in critical_node_types
            }
            ...
            for edge in comp_gnn.get('edges', []):
                if (edge.get('source') not in comp_nodes or 
                    edge.get('target') not in comp_nodes):
