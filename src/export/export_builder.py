...
class ExportPackageBuilder:
    ...
    def build_package(self, agent, target: HardwareTarget, 
                     readiness_score: Optional[Any] = None) -> ExportPackage:
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.info(f"Package created: {package_path} ({total_size:.1f}MB, "
    ...
    def _export_knowledge(self, agent, knowledge_dir: Path, 
        ...
        if target.storage_gb < 32:
            knowledge_data = self._prune_knowledge(agent.knowledge_graph, 
                                                 max_items=10000)
        else:
            knowledge_data = {
                'experiences': [exp.to_dict() for exp in 
                              agent.knowledge_graph.experiences[-50000:]],
                'patterns': {id: p.to_dict() for id, p in 
                           agent.knowledge_graph.patterns.items()},
    ...
    def _export_config(self, agent, config_dir: Path, target: HardwareTarget,
    ...
    def _prune_knowledge(self, knowledge_graph, max_items: int = 10000) -> Dict[str, Any]:
        ...
        patterns = sorted(
            knowledge_graph.patterns.values(),
