...
class ExportPackageBuilder:
    ...
    def _export_knowledge(
        self, agent, knowledge_dir: Path, target: HardwareTarget
    ) -> float:
        ...
        if target.storage_gb < 32:
            ...
            knowledge_data = {
                ...
                "patterns": {
                    id: p.to_dict() for id, p in agent.knowledge_graph.patterns.items()
                },
    ...
    def _prune_knowledge(
        self, knowledge_graph, max_items: int = 10000
    ) -> Dict[str, Any]:
