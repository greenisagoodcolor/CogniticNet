...
class PipelineOrchestrator:
    ...
    def load_step_module(self, step: Dict[str, Any]):
        ...
        spec = importlib.util.spec_from_file_location(step["module_name"], step["file"])
    ...
    def run(self) -> bool:
        ...
        logger.info(
            f"Will run {len(steps_to_run)} steps: "
        ...
        logger.info(f"\n{'='*60}")
        logger.info(
            f"Pipeline {'completed' if all_success else 'failed'} in {total_elapsed:.2f}s"
        )
        logger.info(f"{'='*60}")
    ...
    def _save_markdown_summary(self, report: Dict[str, Any]):
        ...
        with open(summary_file, "w") as f:
            ...
            f.write(
                f"**Duration**: {report['execution']['total_seconds']:.2f} seconds\n"
            )
            f.write(
                f"**Status**: {'✅ Success' if report['execution']['success'] else '❌ Failed'}\n\n"
            )
            ...
            f.write("\n## Configuration\n\n")
            f.write(
                f"- **Output Directory**: `{report['configuration']['output_dir']}`\n"
            )
            f.write(
                f"- **Models Directory**: `{report['configuration']['models_dir']}`\n"
            )
...
def main():
    ...
    parser.add_argument("--skip-steps", help="Comma-separated list of steps to skip")
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
