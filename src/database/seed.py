...
def create_agents(session: Session, count: int = 10) -> list[Agent]:
    ...
    for i in range(count):
        agent = Agent(
            ...
            config={
                "personality": choice(
                    ["curious", "cautious", "aggressive", "cooperative"]
                ),
            ...
            state={
                "current_task": choice(
                    ["exploring", "trading", "patrolling", "researching", None]
                ),
...
def create_conversations(session: Session, agents: list[Agent]) -> list[Conversation]:
    ...
    for i in range(5):
        conv = Conversation(
            ...
            meta_data={
                "topic": choice(
                    ["resources", "exploration", "strategy", "cooperation"]
                ),
...
def create_knowledge_graphs(
    session: Session, agents: list[Agent]
) -> list[KnowledgeGraph]:
