...
from src.database.models import (
    Agent, AgentStatus, Conversation, ConversationType,
    ConversationParticipant, Message, KnowledgeGraph,
    Coalition, CoalitionMember, SystemLog
)
...
def create_agents(session: Session, count: int = 10) -> list[Agent]:
    ...
    for i in range(count):
        agent = Agent(
            ...
            location=f"8{fake.hexify(text='^^^^^^')}",  # H3 hex format
...
def create_conversations(session: Session, agents: list[Agent]) -> list[Conversation]:
    ...
    for i in range(3):
        ...
        participants = fake.random_elements(
            elements=agents, 
            length=randint(3, min(6, len(agents))), 
            unique=True
        )
...
def create_knowledge_graphs(session: Session, agents: list[Agent]) -> list[KnowledgeGraph]:
    ...
    for agent in agents[:5]:  # Create graphs for first 5 agents
    ...
    # Create a shared knowledge graph
...
def create_coalitions(session: Session, agents: list[Agent]) -> list[Coalition]:
    ...
    for config in coalition_configs:
        coalition = Coalition(
            ...
            activated_at=datetime.utcnow() - timedelta(days=randint(1, 30)) if config["status"] == "active" else None
        )
        ...
        for idx, agent in enumerate(members):
            member = CoalitionMember(
                ...
                share=1.0 / member_count,  # Equal shares for simplicity
                is_active=True
            )
            session.add(member)
    ...
    for _ in range(50):
        log = SystemLog(
            ...
            data={
                ...
                "success": choice([True, True, True, False])  # 75% success rate
            },
            error_trace=fake.text() if choice(levels) == "ERROR" else None,
            timestamp=datetime.utcnow() - timedelta(minutes=randint(0, 1440))  # Last 24 hours
        )
        session.add(log)
...
def seed_demo_data():
    ...
    try:
        ...
        info_broker = Agent(
            ...
            config={
                "personality": "curious",
                "skills": ["data_collection", "pattern_recognition", "information_synthesis"],
        ...
        # Add participants and messages for demo conversation
        for agent in [optimizer, market_maker]:
            participant = ConversationParticipant(
                conversation=market_conv,
                agent=agent,
                role="negotiator",
                is_active=True
            )
