...
from .seed import seed_development_data, seed_demo_data
...
def get_db_config():
    ...
    return {'user': user_pass[0], 'password': user_pass[1] if len(user_pass) > 1 else '', 'host': host_port[0], 'port': host_port[1] if len(host_port) > 1 else '5432', 'database': host_port_db[1] if len(host_port_db) > 1 else 'freeagentics_dev'}
...
def create_db(force):
    ...
    conn = psycopg2.connect(host=config['host'], port=config['port'], user=config['user'], password=config['password'], database='postgres')
