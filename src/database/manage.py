...
def create_db(force):
    ...
    try:
        ...
        click.echo(
            f"Database {config['database']} already exists. Use --force to recreate."
        )
    finally:
        cur.close()
        conn.close()
...
@cli.command()
@click.option(
    "--env", type=click.Choice(["development", "demo", "test"]), default="development"
)
def seed(env):
    ...
    try:
        with engine.connect() as conn:
            ...
            if has_migrations:
                ...
                click.echo(
                    "✗ No migrations applied yet. Run 'python manage.py migrate'"
                )
            result = conn.execute(
                text(
