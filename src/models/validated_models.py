...
class AgentPersonality(BaseModel):
    ...
    exploration: float = Field(
        50.0, ge=0.0, le=100.0, description="Exploration drive (0-100)"
    )
    cooperation: float = Field(
        50.0, ge=0.0, le=100.0, description="Cooperation tendency (0-100)"
    )
    efficiency: float = Field(
        50.0, ge=0.0, le=100.0, description="Resource efficiency (0-100)"
    )
    curiosity: float = Field(
        50.0, ge=0.0, le=100.0, description="Knowledge seeking (0-100)"
    )
    risk_tolerance: float = Field(
        50.0, ge=0.0, le=100.0, description="Risk tolerance (0-100)"
    )
...
class Resource(BaseModel):
    ...
    regeneration_rate: float = Field(
        0.0, ge=0.0, le=1.0, description="Regeneration rate per step"
    )
...
class Experience(BaseModel):
    ...
    confidence: float = Field(
        0.5, ge=0.0, le=1.0, description="Confidence in this experience"
    )
...
class ValidatedAgent(BaseModel):
    ...
    social_bonds: Dict[str, float] = Field(
        default_factory=dict, description="Agent ID -> bond strength (0-1)"
    )
    ...
    _validate_name = validator("name", allow_reuse=True)(validate_name)
    _validate_gnn_path = validator("gnn_model_path", allow_reuse=True)(
        validate_file_path
    )
    ...
    def validate_social_bonds(cls, v):
        ...
        for agent_id, bond_strength in v.items():
            if not 0 <= bond_strength <= 1:
                raise ValueError(
                    f"Bond strength with {agent_id} must be between 0 and 1"
                )
        return v
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "position": {"hex_id": "8a283082a677fff", "resolution": 8},
                "energy": 85.5,
                "status": "active",
            }
        }
...
class WorldConfig(BaseModel):
    ...
    resource_density: float = Field(
        0.1, ge=0.0, le=1.0, description="Resource initialize density"
    )
    regeneration_rate: float = Field(
        0.01, ge=0.0, le=1.0, description="Resource regeneration"
    )
