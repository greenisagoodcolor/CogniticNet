"""
Integration tests for the Spatial Computing API and H3 World.

Tests the integration between spatial_api.py and h3_world.py to ensure
they work together seamlessly for agent spatial operations.
"""
...
class TestSpatialIntegration:
    ...
    def test_pathfinding_respects_terrain(self, spatial_api, h3_world):
        ...
        # Build movement costs from world terrain
        movement_costs = {
            hex_id: cell.movement_cost
            for hex_id, cell in h3_world.cells.items()
        }

        # Find water cells as obstacles
        obstacles = {
            hex_id for hex_id, cell in h3_world.cells.items()
    ...
    def test_resource_distribution_in_world(self, spatial_api, h3_world):
        """Test resource distribution across world hexes."""
        # Create clustered food distribution around forest biomes
        forest_centers = [
            hex_id for hex_id, cell in h3_world.cells.items()
            if cell.biome == Biome.FOREST
        ][:3]  # Use first 3 forests as centers
        ...
        # Verify resources are higher near forests
        for center in forest_centers:
            if center in resources and 'food' in resources[center]:
                # Should have food at forest centers
                assert resources[center]['food'] > 0

    def test_visibility_with_elevation(self, spatial_api, h3_world):
        """Test visibility calculations considering terrain elevation."""
        # Find a high elevation cell (mountain)
        mountain_hex = None
        for hex_id, cell in h3_world.cells.items():
        ...
        if mountain_hex:
            ...
            # Build terrain heights
            terrain_heights = {
                hex_id: cell.elevation
                for hex_id, cell in h3_world.cells.items()
            }
            ...
            obs_model = ObservationModel(
                base_range=2,
                elevation_bonus=0.002  # 2 extra hexes per 1000m
            )
            spatial_api.set_observation_model(obs_model)

            # Get visible hexes from mountain
            visible = spatial_api.get_visible_hexes(
                mountain_hex,
                mountain_cell.elevation,
                terrain_heights
            )

            # Mountain should see further
            assert len(visible) > 0

            # Compare with flat terrain visibility
            flat_hex = None
            for hex_id, cell in h3_world.cells.items():
    ...
    def test_spatial_queries_on_world(self, spatial_api, h3_world):
        ...
        forest_hexes = spatial_api.query_region(
            center_hex,
            radius_hexes=3,
            filter_func=is_forest
        )

        # Verify all returned hexes are forests
        for hex_id in forest_hexes:
            cell = h3_world.get_cell(hex_id)
            assert cell.biome == Biome.FOREST

    def test_agent_movement_path_cost(self, spatial_api, h3_world):
        """Test calculating movement costs for agent paths."""
        # Find two accessible cells
        cells = list(h3_world.cells.keys())
        start = cells[0]
        end = cells[20]
        ...
        # Calculate cost using world data
        simple_cost = h3_world.get_total_path_cost(simple_path)

        # A* path considering terrain
        movement_costs = {
            hex_id: cell.movement_cost
            for hex_id, cell in h3_world.cells.items()
        }

        optimal_path = spatial_api.find_path_astar(
            start, end, movement_costs
        )

        if optimal_path:
            optimal_cost = h3_world.get_total_path_cost(optimal_path)

            # Optimal path should have lower or equal cost
            assert optimal_cost <= simple_cost + 0.1  # Small tolerance

    def test_cache_performance(self, spatial_api, h3_world):
        """Test that caching improves performance."""
        import time

        # Get many neighbors without cache
    ...
    def test_resource_clusters_in_biomes(self, spatial_api, h3_world):
        """Test finding resource clusters in specific biomes."""
        # Find hexes with high food in forests/jungles
        food_hexes = []
        for hex_id, cell in h3_world.cells.items():
        ...
        if len(food_hexes) > 5:
            ...
            # Should find some clusters in forests/jungles
            assert len(clusters) > 0

            # Verify clusters are valid
            for cluster in clusters:
                assert len(cluster) >= 2
                # All hexes in cluster should be from our food_hexes
