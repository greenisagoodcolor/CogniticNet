...
class TestGraphBatchProcessor:
    ...
    def test_create_batch_simple(self):
        ...
        assert batch.x.shape == (7, 16)  # 3 + 4 nodes
        assert batch.edge_index.shape == (2, 5)  # 2 + 3 edges
    ...
    def test_batch_with_edge_attributes(self):
        ...
        assert batch.edge_attr.shape == (3, 4)  # 2 + 1 edges
    ...
    def test_batch_with_padding(self):
        ...
        # With padding, node features should be (num_graphs, max_nodes, features)
        assert batch.x.shape == (2, 5, 16)
        assert batch.mask is not None
        assert batch.mask.shape == (2, 5)
        assert batch.mask[0, :3].all() == True  # First 3 nodes valid
        assert batch.mask[0, 3:].all() == False  # Last 2 nodes padded
        assert batch.mask[1, :2].all() == True  # First 2 nodes valid
        assert batch.mask[1, 2:].all() == False  # Last 3 nodes padded
    ...
    def test_batch_with_graph_attributes(self):
        ...
        graphs = [
            ...
            GraphData(
                node_features=torch.randn(2, 16),
                edge_index=torch.tensor([[0], [1]], dtype=torch.long),
                graph_attr=torch.randn(8)  # Different size
            )
        ]
        ...
        assert batch.graph_attr.shape == (2, 10)  # Padded to max size
    ...
    def test_collate_fn(self):
        ...
        # Create batch of graphs (as would come from DataLoader)
        graphs = [
            GraphData(
        ...
        assert batch.x.shape[0] == 20  # 4 * 5 nodes
...
class TestDynamicBatchSampler:
    ...
    def test_size_grouping(self):
        ...
        # Create graphs with distinct size groups
        graph_sizes = [
            (10, 20), (11, 22),  # Small group
            (50, 100), (52, 105),  # Medium group
            (100, 200), (98, 195)  # Large group
        ]
        ...
        # Each group should have 2 graphs
...
class TestStreamingBatchProcessor:
    ...
    def test_initialization(self):
        ...
        streaming = StreamingBatchProcessor(
            batch_processor=base_processor,
            buffer_size=100
        )
    ...
    def test_process_stream(self):
        ...
        # Process stream
        batches = list(streaming.process_stream(
            graph_generator(),
            batch_size=3
        ))
    ...
    def test_process_stream_with_function(self):
        ...
        # Process function that returns number of nodes
        def process_fn(batch):
            return batch.x.shape[0]

        results = list(streaming.process_stream(
            graph_generator(),
            batch_size=2,
            process_fn=process_fn
        ))

        assert len(results) == 3  # 6 graphs in batches of 2
        assert results[0] == 6  # 2 graphs * 3 nodes
        assert results[1] == 6
        assert results[2] == 6
...
class TestUtilityFunctions:
    ...
    def test_create_mini_batches(self):
        ...
        # Create mini-batches without shuffle
