"""
Unit tests for Active Inference policy selection
"""
...
class TestPolicy:
    ...
    def test_policy_creation_from_list(self):
    ...
    def test_policy_creation_from_tensor(self):
...
class TestDiscreteExpectedFreeEnergy:
    ...
    def test_enumerate_policies_multi_step(self):
    ...
    def test_expected_free_energy_epistemic(self):
        ...
        policy1 = Policy([1])
        G1 = self.selector.compute_expected_free_energy(policy1, beliefs, self.model)

        # Policy that stays (less informative)
        policy0 = Policy([0])
        G0 = self.selector.compute_expected_free_energy(policy0, beliefs, self.model)

        # Both should be finite
        assert torch.isfinite(G0)
        assert torch.isfinite(G1)

    def test_expected_free_energy_pragmatic(self):
    ...
    def test_stochastic_policy_selection(self):
...
class TestContinuousExpectedFreeEnergy:
    ...
    def test_continuous_free_energy(self):
    ...
    def test_continuous_policy_selection(self):
...
class TestSophisticatedInference:
    ...
    def test_sophisticated_refinement(self):
        """Test policy refinement"""
        initial_policy = Policy([0, 1, 0])
        beliefs = torch.tensor([0.5, 0.3, 0.2])

        refined = self.selector._sophisticated_refinement(
            initial_policy, beliefs, self.model
        )
    ...
    def test_sophistication_depth(self):
