...
class TestDirectGraphMapper:
    def test_initialization(self):
        ...
        assert mapper.state_projection is not None  # Needs projection
    ...
    def test_map_to_states(self):
        ...
        assert torch.allclose(states.sum(dim=-1), torch.ones(5), atol=1e-6)  # Valid probabilities
    ...
    def test_map_with_node_indices(self):
        ...
        assert states.shape == (3, 4)  # Only selected nodes
...
class TestGraphFeatureAggregator:
    def test_mean_aggregation(self):
        ...
        # Test data - 3 graphs with varying nodes
        ...
        # Check mean calculation for first graph
        expected_mean = node_features[:3].mean(dim=0)
        assert torch.allclose(aggregated[0], expected_mean, atol=1e-6)
    ...
    def test_max_aggregation(self):
        ...
        assert aggregated.shape == (3, 8)
        # Check max for first graph
        expected_max = node_features[:3].max(dim=0)[0]
        assert torch.allclose(aggregated[0], expected_max, atol=1e-6)
...
class TestGNNActiveInferenceAdapter:
    ...
    def test_graph_to_beliefs(self, setup_adapter):
        ...
        assert beliefs.shape == (1, 4)  # Graph-level belief
        assert torch.allclose(beliefs.sum(dim=-1), torch.ones(1), atol=1e-6)

        # Test with specific nodes
        agent_indices = torch.tensor([0, 2])
        beliefs = adapter.graph_to_beliefs(graph_data, agent_indices)
        assert beliefs.shape == (2, 4)  # Node-level beliefs
    ...
    def test_update_beliefs_with_graph(self, setup_adapter):
        ...
        assert torch.allclose(updated_beliefs.sum(dim=-1), torch.ones(1), atol=1e-6)
...
class TestHierarchicalGraphIntegration:
    ...
    def test_process_hierarchical_graph(self):
        ...
        # Process without adapters (should return raw data)
