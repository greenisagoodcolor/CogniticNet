...
class TestEdgeProcessor:
    ...
    def test_undirected_edges(self):
        ...
        edges = [
            Edge(source=0, target=1),
            Edge(source=1, target=0),  # Duplicate in opposite direction
            Edge(source=1, target=2)
        ]

        batch = processor.process_edges(edges, num_nodes=3)

        # Should remove duplicate and keep only unique undirected edges
        assert batch.edge_index.shape[1] == 2  # Only 2 unique edges
    ...
    def test_bidirectional_edges(self):
        ...
        assert batch.edge_index.shape[1] == 4  # 2 original + 2 reverse
    ...
    def test_distance_features(self):
        ...
        # Check normalization (MinMax scaling)
    ...
    def test_categorical_features(self):
        ...
        assert batch.edge_attr is not None
        # Should have 3 categories (friend, colleague, family)
    ...
    def test_embedding_features(self):
        ...
        edges = [
            ...
            Edge(source=1, target=2),  # No embedding, should generate
            Edge(source=2, target=0, features={"embedding": "edge_123"})  # Hash-based
        ]

        batch = processor.process_edges(edges, num_nodes=3)

        assert batch.edge_attr is not None
        assert batch.edge_attr.shape[1] >= 4  # At least 4D embeddings
    ...
    def test_multiple_features(self):
        ...
        assert batch.edge_attr is not None
        # 1 weight + 1 distance + N categorical features
    ...
    def test_edge_sampling_topk(self):
        ...
        # Should keep top-2 weights (0.9 and 0.7)
        edge_index = batch.edge_index
        ...
        # Check that we kept the highest weights
    ...
    def test_edge_types(self):
        ...
        edges = [
            ...
            Edge(source=0, target=3)  # No type
        ]
        ...
        # Check type mapping
        assert len(processor.edge_type_mapping) >= 2  # At least social and professional
    ...
    def test_edge_statistics(self):
        ...
        edges = [
            ...
            Edge(source=0, target=0, weight=1.0)  # Self-loop
        ]
        ...
        assert stats['mean_weight'] == pytest.approx(0.825, rel=1e-3)
