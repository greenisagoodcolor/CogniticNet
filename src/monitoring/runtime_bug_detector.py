...
class RuntimeBugDetector:
    ...
    async def check_invariant(self, invariant: Invariant) -> Optional[InvariantViolation]:
        ...
        try:
            ...
            if invariant.consecutive_failures >= invariant.max_consecutive_failures:
                logger.error(f"Disabling invariant {invariant.name} after {invariant.consecutive_failures} failures")
    ...
    async def attempt_healing(self, violation: InvariantViolation) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"Successfully healed violation: {violation.invariant_name}")
                return True
            else:
    ...
    async def _process_violation(self, violation: InvariantViolation):
        ...
        if violation.severity == ViolationSeverity.CRITICAL:
            logger.critical(f"CRITICAL violation: {violation.invariant_name} - {violation.message}")
        elif violation.severity == ViolationSeverity.ERROR:
            logger.error(f"ERROR violation: {violation.invariant_name} - {violation.message}")
        elif violation.severity == ViolationSeverity.WARNING:
            logger.warning(f"WARNING violation: {violation.invariant_name} - {violation.message}")
        else:
            logger.info(f"INFO violation: {violation.invariant_name} - {violation.message}")
    ...
    def _update_circuit_breaker(self):
        ...
        self.violations_in_window = [
            ts for ts in self.violations_in_window
        ...
        if len(self.violations_in_window) > self.max_violations_per_minute:
            if not self.circuit_breaker_open:
                logger.error(f"Circuit breaker opened: {len(self.violations_in_window)} violations in last minute")
                self.circuit_breaker_open = True
                
    def _check_circuit_breaker(self):
        """Check if circuit breaker can be closed."""
        if self.circuit_breaker_open and len(self.violations_in_window) < self.max_violations_per_minute / 2:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "healing_success_rate": self.successful_healings / self.total_healings if self.total_healings > 0 else 0,
            "violations_last_hour": len(recent_violations),
            "circuit_breaker_open": self.circuit_breaker_open,
            "active_invariants": sum(1 for inv in self.invariants.values() if inv.enabled),
            "disabled_invariants": sum(1 for inv in self.invariants.values() if not inv.enabled)
        }
            ...
            try:
                for agent in agents:
                    if not world.get_cell(agent.position.hex_id):
                        return False, f"Agent {agent.id} at invalid position {agent.position.hex_id}"
                return True, None
            except Exception as e:
            ...
            try:
                for agent_id, kg in knowledge_graphs.items():
                    ...
                    for node in kg.graph.nodes():
                        if (kg.graph.in_degree(node) == 0 and 
                            kg.graph.out_degree(node) == 0 and
                            len(kg.graph.nodes()) > 1):
                            return False, f"Orphaned node {node} in {agent_id}'s knowledge graph"
        ...
        def check():
            try:
                ...
                if abs(total_energy - initial_total_energy) > 0.01:
                    return False, f"Energy not conserved: {total_energy} != {initial_total_energy}"
                return True, None
            except Exception as e:
...
if __name__ == "__main__":
    ...
    async def test_monitoring():
        ...
        async with detector.monitored_context():
            ...
            for v in violations:
                print(f"Violation: {v.invariant_name} - {v.message} (healed: {v.healing_status.value})")
