        ...
        try:
            ...
            while time.time() - start_time < self.config.startup_timeout:
                ...
                if self.config.pid_file.exists():
                    ...
                    logger.info(
                        f"Service {self.config.agent_name} started (PID: {pid})"
                    )
                    return True
    ...
    async def _test_movement(self, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        try:
            ...
            async with session.post(
                f"{self.base_url}/api/agent/move", json=move_data
            ) as response:
                if response.status == 200:
...
async def verify_deployment(deployment_path: str) -> bool:
    ...
    if "logs" in results["checks"] and results["checks"]["logs"].get("checked"):
        ...
        print(
            f"Log Errors: {logs['error_count']} errors, {logs['warning_count']} warnings"
        )
