...
class HealthMonitor:
    ...
    async def _perform_health_check(self, session: aiohttp.ClientSession,
...
class FunctionalTester:
    ...
    async def _test_movement(self, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        try:
            ...
            async with session.get(
                f"{self.base_url}/api/agent/position"
            ) as response:
                if response.status != 200:
                    return {
                        'test': test_name,
            ...
            async with session.post(
                f"{self.base_url}/api/agent/move",
                json=move_data
            ) as response:
                if response.status == 200:
    ...
    async def _test_perception(self, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        try:
            async with session.get(
                f"{self.base_url}/api/agent/perceive"
            ) as response:
                if response.status == 200:
    ...
    async def _test_communication(self, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        try:
            ...
            async with session.post(
                f"{self.base_url}/api/agent/communicate",
                json=message_data
            ) as response:
                if response.status == 200:
    ...
    async def _test_decision_making(self, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        try:
            ...
            async with session.post(
                f"{self.base_url}/api/agent/decide",
                json=scenario_data
            ) as response:
                if response.status == 200:
...
async def verify_deployment(deployment_path: str) -> bool:
    ...
    if 'resources' in results['checks']:
        ...
        print(f"Resources: CPU {resources.get('cpu_percent', 0):.1f}%, "
