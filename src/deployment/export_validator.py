...
class ExportValidator:
    ...
    def validate_package(self, package_path: Path, 
                        target_platform: Optional[HardwarePlatform] = None) -> List[ValidationResult]:
        """
        ...
        try:
            ...
            if target_platform:
                results.extend(self._check_hardware_compatibility(
    ...
    def _check_manifest(self, package_dir: Path) -> List[ValidationResult]:
        ...
        try:
            ...
            required_fields = [
                'package_name', 'version', 'agent_class',
                'created_at', 'platform', 'files'
            ]
            
            missing_fields = [f for f in required_fields 
                            if f not in manifest_data]
    ...
    def _verify_file_hashes(self, package_dir: Path, 
                          file_hashes: Dict[str, str]) -> List[ValidationResult]:
    ...
    def _check_hardware_compatibility(self, package_dir: Path,
                                    target_platform: HardwarePlatform) -> List[ValidationResult]:
...
class DeploymentVerifier:
    ...
    def verify_deployment(self, package_dir: Path,
