...
class GNNModelRefinement:
    ...
    def __init__(
        self, agent_id: str, model_path: str, knowledge_graph: AgentKnowledgeGraph
    ):
        """
    ...
    def _init_history_dir(self):
        ...
        history_dir = os.path.join(
            os.path.dirname(self.model_path), self.MODEL_HISTORY_DIR, self.agent_id
        )
    ...
    def check_refinement_needed(self) -> bool:
        ...
        if experiences_since_last >= self.REFINEMENT_INTERVAL:
            logger.info(
                f"Refinement triggered: {experiences_since_last} new experiences"
            )
            return True
    ...
    def refine_model(
        self, force: bool = False, reason: str = "Periodic refinement"
        ...
        try:
            ...
            # Create new version
            new_version = self._create_model_version(refined_model, patterns, reason)
    ...
    def _apply_refinements(
        self, model_content: str, patterns: List[ExtractedPattern]
    ) -> str:
        """
    ...
    def _insert_content(self, lines: List[str], after_index: int, content: List[str]):
    ...
    def _generate_preferences_from_patterns(
    ...
    def _extract_resource_preferences(self, pattern: ExtractedPattern) -> List[str]:
    ...
    def _generate_rules_from_patterns(
        ...
        for i, pattern in enumerate(patterns):
            rules.append(f"\n### Learned Pattern {i + 1}")
            rules.append(
                f"*Confidence: {pattern.confidence:.2f}, Success Rate: {pattern.success_rate:.2f}*"
            )
            ...
            for key, value in pattern.trigger_conditions.items():
                if isinstance(value, dict) and "mean" in value:
                    conditions.append(
                        f"{key} ∈ [{value['min']:.1f}, {value['max']:.1f}]"
                    )
                else:
            ...
            rules.append(
                f"- **Expected**: Free energy reduction, {pattern.success_rate*100:.0f}% success"
            )
    ...
    def _update_model_metadata(
        self, lines: List[str], patterns: List[ExtractedPattern]
    ):
    ...
    def _create_model_version(
        self, model_content: str, patterns: List[ExtractedPattern], reason: str
    ) -> ModelVersion:
        ...
        version_path = os.path.join(
            self.history_dir, f"{self.agent_id}_{version_id}.gnn.md"
        )
    ...
    def _calculate_performance_metrics(self) -> Dict[str, float]:
        ...
        success_count = sum(
            ...
            for node_id, node_data in self.knowledge_graph.graph.nodes(data=True)
            if node_data["type"].value == "experience"
            and node_data["data"].outcome.get("success", False)
        )
        ...
        if pattern_confidences:
            metrics["avg_pattern_confidence"] = sum(pattern_confidences) / len(
                pattern_confidences
            )
    ...
    def _update_self_beliefs(
        self, patterns: List[ExtractedPattern], version: ModelVersion
    ):
        ...
        if self.performance_baseline:
            ...
            improvement = current_metrics.get(
                "success_rate", 0
        ...
        for metric in v1.performance_metrics:
            if metric in v2.performance_metrics:
                ...
                comparison["performance_change"][metric] = {
                    ...
                    "change": change,
                    "percent_change": (
                        (change / v1.performance_metrics[metric] * 100)
                        if v1.performance_metrics[metric] != 0
                        else 0
                    ),
                }
...
if __name__ == "__main__":
    ...
    refinement = GNNModelRefinement(
        agent_id="test_agent", model_path="test_agent.gnn.md", knowledge_graph=kg
    )
