...
class GridWorldEnvironment:
    ...
    def _get_observation(self):
        ...
        # Simple observation: encodes position relative to goal
        dx = self.goal_pos[0] - self.agent_pos[0]
        dy = self.goal_pos[1] - self.agent_pos[1]

        if dx > 0 and dy > 0:
            return 0  # Goal is down-right
        elif dx > 0 and dy <= 0:
            return 1  # Goal is down-left
        elif dx <= 0 and dy > 0:
            return 2  # Goal is up-right
        else:
            return 3  # Goal is up-left or reached
...
class ActiveInferenceAgent:
    ...
    def __init__(self, config=None):
        ...
        self.dims = ModelDimensions(
            num_states=16,      # 4x4 position encoding
            num_observations=4,  # 4 relative directions to goal
        ...
        self.param_learner = create_parameter_learner(
            'online',
            self.learning_config,
            generative_model=self.gen_model
        )
    ...
    def _initialize_model(self):
        ...
        # A matrix: observation model (what observations each state generates)
        # Initialize with some structure based on grid positions
        ...
        for s in range(self.dims.num_states):
            ...
            # Determine observation based on position
            if row < 2 and col < 2:
                obs = 0  # Down-right
        ...
        # B matrix: transition model (how actions change states)
        # Initialize with movement dynamics
        ...
        for a in range(self.dims.num_actions):
            for s in range(self.dims.num_states):
                ...
                if a == 0 and row > 0:  # Up
                    next_s = (row - 1) * 4 + col
                elif a == 1 and col < 3:  # Right
                    next_s = row * 4 + (col + 1)
                elif a == 2 and row < 3:  # Down
                    next_s = (row + 1) * 4 + col
                ...
                self.gen_model.B[s, s, a] += 0.1  # Small chance of staying

        # Normalize
        self.gen_model.B = self.gen_model.B / self.gen_model.B.sum(dim=0, keepdim=True)

        # C vector: preferences (prefer being in goal state)
        self.gen_model.C = torch.ones(self.dims.num_observations) * 0.1
        self.gen_model.C[3] = 0.8  # Prefer observation indicating goal reached
        self.gen_model.C = self.gen_model.C / self.gen_model.C.sum()

        # D vector: initial state prior (start at top-left)
    ...
    def observe(self, observation):
        ...
        # Update belief using optimizer
        self.belief = self.optimizer.optimized_belief_update(
            self.belief, obs_vec, self.gen_model.A
        )
    ...
    def act(self, exploration_bonus=0.0):
        ...
        G_values = self.policy_selector.compute_expected_free_energy(
            self.belief, self.gen_model.A, self.gen_model.B, self.gen_model.C,
            num_actions=self.dims.num_actions,
            horizon=self.policy_config.planning_horizon,
            precision=precision
        )

        # Add exploration bonus if using active learning
        if exploration_bonus > 0:
            G_adjusted = self.active_learner.select_informative_action(
                self.belief, G_values
            )
        else:
            G_adjusted = G_values
    ...
    def learn(self, prev_belief, action, observation, reward):
        ...
        # Create experience for parameter learning
        ...
        # Learn parameters
        self.param_learner.observe(
            state=prev_belief,
            action=action_vec,
            observation=obs_vec,
            next_state=self.belief,
...
def main():
    ...
    diag_config = DiagnosticConfig(
        save_figures=True,
        figure_dir=Path("./active_inference_demo_results"),
    ...
    belief_tracker = diagnostics.create_belief_tracker(
        "grid_agent",
        num_states=16,
        state_labels=[f"Pos({i//4},{i%4})" for i in range(16)]
    )
    ...
    # Plot 4: Learned observation model
