...
class CoalitionGoal:
    ...
    def is_completed(self) -> bool:
        ...
        return (self.status == CoalitionGoalStatus.COMPLETED and
                self.current_progress >= self.success_threshold)
    ...
    def update_progress(self, progress: float, member_id: str = None) -> None:
...
class CoalitionMember:
    ...
    def signal_departure(self, reason: str = None) -> None:
...
class Coalition:
    ...
    def is_viable(self) -> bool:
        ...
        return (self.min_members <= active_members <= self.max_members and
                self.status in [CoalitionStatus.FORMING, CoalitionStatus.ACTIVE])
    ...
    def add_member(self, agent_id: str, role: CoalitionRole = CoalitionRole.CONTRIBUTOR,
                   capabilities: Set[str] = None, resources: Dict[str, float] = None) -> bool:
        ...
        logger.info(f"Added member {agent_id} with role {role.value} to coalition {self.coalition_id}")
        return True
    ...
    def update_goal_progress(self, goal_id: str, progress: float, member_id: str = None) -> None:
    ...
    def allocate_resources(self, allocation: Dict[str, Dict[str, float]]) -> bool:
        ...
        for resource, amount in total_allocated.items():
            if amount > available.get(resource, 0):
                logger.warning(f"Insufficient {resource}: need {amount}, have {available.get(resource, 0)}")
                return False
    ...
    def calculate_member_contributions(self) -> Dict[str, float]:
        ...
        for member_id, member in self.members.items():
            ...
            total_contribution = (resource_contribution + capability_contribution +
                                performance_contribution + leadership_bonus)
    ...
    def _elect_new_leader(self) -> None:
        ...
        if candidates:
            ...
            logger.info(f"Elected {new_leader.agent_id} as new leader of coalition {self.coalition_id}")
        else:
            self.leader_id = None
    ...
    def _update_performance_metrics(self) -> None:
        ...
        self.performance_metrics.update({
            "average_member_contribution": sum(m.contribution_score for m in self.members.values()) / len(self.members),
            "average_participation": sum(m.participation_rate for m in self.members.values()) / len(self.members),
            "resource_utilization": len(self.resource_allocation) / max(1, len(self.total_resources)),
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "goals": {
                ...
                "in_progress": sum(1 for g in self.goals.values() if g.status == CoalitionGoalStatus.IN_PROGRESS),
                "failed": sum(1 for g in self.goals.values() if g.status == CoalitionGoalStatus.FAILED)
            },
            "performance": {
                "success_rate": self.success_rate,
