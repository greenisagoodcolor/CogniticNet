...
from ..coalition.coalition_models import Coalition, CoalitionMember, CoalitionRole, CoalitionStatus, CoalitionGoal
from ..coalition.coalition_criteria import CoalitionFormationCriteria
from ..coalition.business_opportunities import BusinessOpportunity
...
class ActiveInferenceFormation:
    ...
    def calculate_free_energy(self, agent_beliefs: Dict[str, float], 
                            coalition_observations: Dict[str, float]) -> float:
        """
    ...
    def evaluate_coalition_fit(self, agent: AgentProfile, 
                             existing_members: List[AgentProfile]) -> float:
    ...
    def form_coalition(self, agents: List[AgentProfile], 
                      goal: Optional[CoalitionGoal] = None,
                      max_size: int = 5) -> FormationResult:
...
class CapabilityBasedFormation:
    ...
    def __init__(self, coverage_weight: float = 0.6, redundancy_penalty: float = 0.3):
        self.coverage_weight = coverage_weight
        self.redundancy_penalty = redundancy_penalty
    
    def calculate_capability_score(self, agents: List[AgentProfile], 
                                 required_capabilities: Set[str] = None) -> float:
        ...
        if required_capabilities:
            ...
            coverage_score = covered_required / len(required_capabilities) if required_capabilities else 1.0
    ...
    def form_coalition(self, agents: List[AgentProfile],
        ...
        if not agents:
            return FormationResult(
                coalition=None, success=False, score=0.0, formation_time=0.0,
                strategy_used=FormationStrategy.CAPABILITY_BASED, participants=[]
            )
        ...
        while len(selected_agents) < max_size and remaining_agents:
            ...
            if best_agent and (not selected_agents or best_score > self.calculate_capability_score(selected_agents, required_capabilities)):
                selected_agents.append(best_agent)
                remaining_agents.remove(best_agent)
            else:
                break
        ...
        formation_time = (datetime.utcnow() - start_time).total_seconds()
        return FormationResult(
            coalition=None, success=False, score=0.0, formation_time=formation_time,
            strategy_used=FormationStrategy.CAPABILITY_BASED, participants=[],
            rejected_agents=[a.agent_id for a in agents]
        )
...
class ResourceOptimizationFormation:
    ...
    def __init__(self, efficiency_weight: float = 0.5, balance_weight: float = 0.3):
    ...
    def calculate_resource_efficiency(self, agents: List[AgentProfile]) -> float:
        ...
        if total_resources:
            ...
            variance = sum((amount - mean_amount) ** 2 for amount in total_resources.values()) / len(total_resources)
            balance_score = 1.0 / (1.0 + variance / max(1.0, mean_amount))
        else:
            balance_score = 0.0
        
        return (diversity_score * 10 + quantity_score) * self.efficiency_weight + balance_score * self.balance_weight
    ...
    def form_coalition(self, agents: List[AgentProfile],
        ...
        if not agents:
            return FormationResult(
                coalition=None, success=False, score=0.0, formation_time=0.0,
                strategy_used=FormationStrategy.RESOURCE_OPTIMIZATION, participants=[]
            )
        ...
        remaining_agents = [a for a in agents if a.agent_id != selected_agents[0].agent_id and 
                           a.current_coalitions < a.max_coalitions]
                ...
                if resource_requirements:
                    ...
                    for resource, required in resource_requirements.items():
                        if resource in total_resources:
                            ...
                            requirement_bonus += coverage * 50  # Significant bonus for meeting requirements
        ...
        if len(selected_agents) >= 2:
            ...
            sorted_agents = sorted(selected_agents, key=lambda a: sum(a.resources.values()), reverse=True)
            for i, agent in enumerate(sorted_agents):
                role = CoalitionRole.LEADER if i == 0 else CoalitionRole.CONTRIBUTOR
                coalition.add_member(
                    agent_id=agent.agent_id,
...
class CoalitionFormationEngine:
    ...
    def form_coalition(self, agents: List[AgentProfile],
        ...
        # Log result
        if result.success:
            logger.info(f"Successfully formed coalition with {len(result.participants)} members "
    ...
    def try_multiple_strategies(self, agents: List[AgentProfile],
                              strategies: List[FormationStrategy] = None,
                              goal: Optional[CoalitionGoal] = None,
                              criteria: Optional[CoalitionFormationCriteria] = None,
                              max_size: int = 5) -> FormationResult:
        """
        ...
        return best_result or FormationResult(
            ...
            participants=[], rejected_agents=[a.agent_id for a in agents]
        )
    ...
    def get_formation_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "average_formation_time": sum(r.formation_time for r in self.formation_history) / len(self.formation_history),
            "average_coalition_size": sum(len(r.participants) for r in successful_formations) / max(1, len(successful_formations)),
