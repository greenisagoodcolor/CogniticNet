...
import time
import threading
from .......agents.base.data-model import Agent, Position, AgentPersonality, AgentResources
from .......agents.base.interaction import InteractionType, MessageType, ResourceType, Message, InteractionRequest, InteractionResult, ResourceExchange, CommunicationProtocol, ResourceManager, ConflictResolver, InteractionSystem

class TestMessage:
    """Test Message class"""

    def test_message_creation(self):
        msg = Message(sender_id='agent1', receiver_id='agent2', message_type=MessageType.REQUEST, content={'request': 'help'})
    ...
    def test_broadcast_message(self):
        msg = Message(sender_id='agent1', receiver_id=None, message_type=MessageType.INFORM, content={'info': 'global update'})
        assert msg.is_broadcast()

    def test_message_with_response_deadline(self):
        deadline = datetime.now() + timedelta(seconds=10)
        msg = Message(sender_id='agent1', receiver_id='agent2', requires_response=True, response_deadline=deadline)
...
class TestInteractionRequest:
    ...
    def test_request_creation(self):
        req = InteractionRequest(initiator_id='agent1', target_id='agent2', interaction_type=InteractionType.COMMUNICATION, parameters={'message': 'hello'})
...
class TestResourceExchange:
    ...
    def test_exchange_creation(self):
        exchange = ResourceExchange(from_agent_id='agent1', to_agent_id='agent2', resource_type=ResourceType.ENERGY, amount=10.0)
    ...
    def test_exchange_execution_success(self):
        ...
        exchange = ResourceExchange(from_agent_id=agent1.id, to_agent_id=agent2.id, resource_type=ResourceType.ENERGY, amount=20.0)
    ...
    def test_exchange_execution_failure(self):
        agent1 = Agent(id='agent1', position=Position(0, 0), resources=AgentResources(energy=10.0))
        agent2 = Agent(id='agent2', position=Position(1, 1), resources=AgentResources(energy=30.0))
        exchange = ResourceExchange(from_agent_id=agent1.id, to_agent_id=agent2.id, resource_type=ResourceType.ENERGY, amount=20.0)
        success = exchange.execute(agent1, agent2)
        assert not success
...
class TestCommunicationProtocol:
    ...
    def test_broadcast_message(self):
        protocol = CommunicationProtocol()
        broadcast = Message(sender_id='agent1', receiver_id=None, content={'announcement': 'global'})
...
class TestConflictResolver:
    ...
    def test_resource_conflict_resolution(self):
        ...
        agent1 = Agent(id='agent1', position=Position(0, 0), priority=2, resources=AgentResources(energy=30.0))
        agent2 = Agent(id='agent2', position=Position(1, 1), priority=1, resources=AgentResources(energy=50.0))
    ...
    def test_spatial_conflict_resolution(self):
        ...
        agent1 = Agent(id='agent1', position=Position(3, 3), priority=1, resources=AgentResources(energy=50.0))
        agent2 = Agent(id='agent2', position=Position(8, 8), priority=2, resources=AgentResources(energy=30.0))
...
class TestInteractionSystem:
    ...
    def test_communication_interaction(self):
        system = InteractionSystem()
        request = InteractionRequest(initiator_id='agent1', target_id='agent2', interaction_type=InteractionType.COMMUNICATION, parameters={'message_type': MessageType.REQUEST, 'content': {'text': 'help needed'}, 'requires_response': True})
    ...
    def test_resource_exchange_interaction(self):
        ...
        request = InteractionRequest(initiator_id='agent1', target_id='agent2', interaction_type=InteractionType.RESOURCE_EXCHANGE, parameters={'resource_type': ResourceType.ENERGY, 'amount': 25.0})
        interaction_id = system.initiate_interaction(request)
        result = system.process_interaction(interaction_id)
        assert result.success
    ...
    def test_conflict_interaction(self):
        ...
        system.register_agent(agent1)
        system.register_agent(agent2)
        request = InteractionRequest(initiator_id='agent1', target_id='agent2', interaction_type=InteractionType.CONFLICT, parameters={'conflict_type': 'resource', 'resource_type': ResourceType.ENERGY, 'disputed_amount': 50.0})
        interaction_id = system.initiate_interaction(request)
        result = system.process_interaction(interaction_id)
        assert result.success
    ...
    def test_interaction_timeout(self):
        system = InteractionSystem()
        request = InteractionRequest(initiator_id='agent1', target_id='agent2', interaction_type=InteractionType.COMMUNICATION, timeout=0.1)
        interaction_id = system.initiate_interaction(request)
        time.sleep(0.2)
        result = system.process_interaction(interaction_id)
    ...
    def test_interaction_callbacks(self):
        ...
        request = InteractionRequest(initiator_id='agent1', target_id='agent2', interaction_type=InteractionType.COMMUNICATION)
        system.initiate_interaction(request)
    ...
    def test_concurrent_interactions(self):
        ...
        def run_interaction(agent_id):
            request = InteractionRequest(initiator_id=agent_id, target_id='target', interaction_type=InteractionType.COMMUNICATION, parameters={'content': {'from': agent_id}})
            interaction_id = system.initiate_interaction(request)
            result = system.process_interaction(interaction_id)
