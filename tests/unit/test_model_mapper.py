...
class TestGraphAnalyzer:
    ...
    def test_analyze_small_graph(self):
        ...
        # Create a simple graph: 0-1-2-3-0 (cycle)
    ...
    def test_analyze_directed_graph(self):
        ...
        # Create directed graph without reciprocal edges
        edge_index = torch.tensor([
            [0, 1, 2],
            [1, 2, 0]
        ], dtype=torch.long)
    ...
    def test_analyze_bipartite_graph(self):
        ...
        # Bipartite graph: nodes 0,1 connected to nodes 2,3
        edge_index = torch.tensor([
            [0, 0, 1, 1],
            [2, 3, 2, 3]
        ], dtype=torch.long)
...
class TestModelSelector:
    ...
    def test_gcn_selection(self):
        ...
        # Create properties favorable for GCN
        props = GraphProperties(
            num_nodes=100,
            num_edges=1000,
            density=0.2,  # Dense
            ...
            has_self_loops=False,
            has_node_features=True,
            has_edge_features=False,  # No edge features
            node_feature_dim=32,
            edge_feature_dim=0,
            num_connected_components=1,
            avg_clustering_coefficient=0.2,  # Low clustering
            is_bipartite=False,
            has_cycles=True
        )

        arch = selector.select_architecture(props)
        # GCN should score well for dense graphs without edge features
    ...
    def test_gat_selection_with_attention_preference(self):
        ...
        config = MappingConfig(
            task_type=GraphTaskType.NODE_CLASSIFICATION,
            auto_select=True,
            prefer_attention=True  # Prefer attention
        )
        selector = ModelSelector(config)
        ...
        props = GraphProperties(
            ...
            num_edges=500,
            density=0.1,
            avg_degree=10,  # Sufficient neighbors
            max_degree=50,
            is_directed=False,
            is_weighted=False,
            has_self_loops=False,
        ...
        arch = selector.select_architecture(props)
        # Should prefer GAT with attention preference
    ...
    def test_sage_selection_for_large_graphs(self):
        ...
        arch = selector.select_architecture(props)
        # SAGE should be selected for large sparse graphs
    ...
    def test_gin_selection_for_graph_classification(self):
        ...
        config = MappingConfig(
            task_type=GraphTaskType.GRAPH_CLASSIFICATION,  # Graph-level task
            auto_select=True
        )
        selector = ModelSelector(config)
        ...
        props = GraphProperties(
            ...
            is_bipartite=False,
            has_cycles=True  # Has cycles
        )

        arch = selector.select_architecture(props)
        # GIN should be selected for graph classification
    ...
    def test_gat_specific_config(self):
        ...
        props = GraphProperties(
            ...
            num_edges=1000,
            density=0.2,
            avg_degree=20,  # High average degree
            max_degree=40,
            is_directed=False,
            is_weighted=False,
            has_self_loops=False,
...
class TestGraphToModelMapper:
    ...
    def test_map_large_graph(self):
        ...
        # Should select SAGE for large sparse graphs
    ...
    def test_graph_level_task(self):
        ...
        # Should have global pooling for graph-level tasks
        assert model_config.global_pool is not None

        # Test with batch
        batch = torch.zeros(num_nodes, dtype=torch.long)
        output = model(node_features, edge_index, batch)
        assert output.shape == (1, output_dim)  # Single graph output
    ...
    def test_model_validation(self):
        ...
        # Test with incompatible features
    ...
    def test_performance_priority(self):
        ...
        # Speed priority should favor simpler architectures
        ...
        # Accuracy priority should favor expressive architectures
