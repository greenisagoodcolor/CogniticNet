...
from .......agents.base.decision-making import ActionType, DecisionStrategy, Action, DecisionContext, SafetyUtility, GoalUtility, ResourceUtility, SocialUtility, DecisionMaker, BehaviorTree, SequenceNode, SelectorNode, ConditionNode, ActionNode, ActionGenerator, DecisionSystem
from .......agents.base.data-model import Agent, AgentGoal, AgentStatus, Position, AgentResources, AgentCapability, SocialRelationship
from .......agents.base.perception import Percept, Stimulus, StimulusType, PerceptionSystem, PerceptionType
...
def sample_agent():
    ...
    agent = Agent(agent_id='test_agent_1', position=Position(5.0, 5.0, 0.0), resources=AgentResources(energy=75.0, health=100.0), capabilities={AgentCapability.MOVEMENT, AgentCapability.PERCEPTION, AgentCapability.SOCIAL_INTERACTION, AgentCapability.COMMUNICATION, AgentCapability.RESOURCE_MANAGEMENT})
    return agent

@pytest.fixture
def sample_percepts():
    """Create sample percepts for testing"""
    return [Percept(stimulus=Stimulus(stimulus_id='threat_1', stimulus_type=StimulusType.DANGER, position=Position(8.0, 8.0, 0.0), intensity=0.8), perception_type=PerceptionType.VISUAL, distance=4.24, timestamp=datetime.now()), Percept(stimulus=Stimulus(stimulus_id='agent_2', stimulus_type=StimulusType.AGENT, position=Position(6.0, 6.0, 0.0), intensity=0.6, metadata={'agent_id': 'agent_2'}), perception_type=PerceptionType.VISUAL, distance=1.41, timestamp=datetime.now()), Percept(stimulus=Stimulus(stimulus_id='resource_1', stimulus_type=StimulusType.OBJECT, position=Position(3.0, 3.0, 0.0), intensity=0.5, metadata={'is_resource': True}), perception_type=PerceptionType.VISUAL, distance=2.83, timestamp=datetime.now())]

@pytest.fixture
def sample_actions():
    """Create sample actions for testing"""
    return [Action(ActionType.WAIT, cost=0.1), Action(ActionType.MOVE, target=Position(10.0, 10.0, 0.0), cost=5.0), Action(ActionType.FLEE, cost=10.0), Action(ActionType.GATHER, cost=2.0, effects={'energy': 20.0}), Action(ActionType.INTERACT, target=Mock(agent_id='agent_2'), cost=1.0), Action(ActionType.COMMUNICATE, cost=0.5)]

@pytest.fixture
def decision_context(sample_agent, sample_percepts, sample_actions):
    """Create a decision context for testing"""
    goal = AgentGoal(goal_id='goal_1', description='Reach target position', priority=0.8, target_position=Position(20.0, 20.0, 0.0))
    sample_agent.current_goal = goal
    return DecisionContext(agent=sample_agent, percepts=sample_percepts, current_goal=goal, available_actions=sample_actions, time_pressure=0.2)
...
class TestAction:
    ...
    def test_action_creation(self):
        ...
        action = Action(action_type=ActionType.MOVE, target=Position(10.0, 10.0, 0.0), cost=5.0, duration=2.0)
...
class TestDecisionContext:
    ...
    def test_get_perceived_threats(self, decision_context):
    ...
    def test_get_perceived_agents(self, decision_context):
    ...
    def test_get_perceived_resources(self, decision_context):
...
class TestUtilityFunctions:
    ...
    def test_safety_utility(self, decision_context):
    ...
    def test_goal_utility(self, decision_context):
        ...
        away_utility = utility.calculate(move_away, decision_context)
        assert away_utility < 0
        no_goal_context = DecisionContext(agent=decision_context.agent, percepts=[], current_goal=None, available_actions=[])
        assert utility.calculate(move_to_goal, no_goal_context) == 0

    def test_resource_utility(self, sample_agent, decision_context):
    ...
    def test_social_utility(self, decision_context):
        ...
        relationship = SocialRelationship(target_agent_id='agent_2', relationship_type='friend', trust_level=0.8)
...
class TestDecisionMaker:
    ...
    def test_utility_based_decision(self, decision_context):
    ...
    def test_goal_oriented_decision(self, decision_context):
    ...
    def test_reactive_decision(self, decision_context):
    ...
    def test_hybrid_decision(self, decision_context):
    ...
    def test_active_inference_decision(self, decision_context):
...
class TestBehaviorTree:
    ...
    def test_sequence_node(self, decision_context):
        ...
        sequence = SequenceNode([create_action(ActionType.WAIT), create_action(ActionType.MOVE), create_action(ActionType.INTERACT)])
        result = sequence.execute(decision_context)
        assert result is not None
        assert len(actions_executed) == 2
        assert result.action_type == ActionType.MOVE

    def test_selector_node(self, decision_context):
        ...
        selector = SelectorNode([ActionNode(failing_generator), ActionNode(failing_generator), ActionNode(success_generator)])
        result = selector.execute(decision_context)
        assert result is not None
        assert result.action_type == ActionType.WAIT

    def test_condition_node(self, decision_context):
        ...
        condition = ConditionNode(low_energy, ActionNode(gather_generator), ActionNode(explore_generator))
    ...
    def test_behavior_tree_execution(self, decision_context):
...
class TestActionGenerator:
    ...
    def test_generate_movement_actions(self, sample_agent, sample_percepts):
    ...
    def test_generate_interaction_actions(self, sample_agent, sample_percepts):
    ...
    def test_generate_communication_actions(self, sample_agent, sample_percepts):
    ...
    def test_generate_resource_actions(self, sample_agent, sample_percepts):
    ...
    def test_capability_based_generation(self, sample_agent, sample_percepts):
        """Test that actions respect agent capabilities"""
        generator = ActionGenerator()
        sample_agent.remove_capability(AgentCapability.MOVEMENT)
        actions = generator.generate_actions(sample_agent, sample_percepts)
        movement_actions = [a for a in actions if a.action_type in [ActionType.MOVE, ActionType.EXPLORE, ActionType.FLEE]]
        assert len(movement_actions) == 0
...
class TestDecisionSystem:
    ...
    def test_register_agent(self, decision_system, sample_agent):
    ...
    def test_make_decision(self, decision_system, sample_agent, sample_percepts):
    ...
    def test_execute_action_move(self, decision_system, sample_agent):
        ...
        decision_system.state_manager.update_agent_resources.assert_called_with(sample_agent.agent_id, energy_delta=-5.0)

    def test_execute_action_wait(self, decision_system, sample_agent):
        ...
        assert success
        decision_system.state_manager.update_agent_status.assert_called_with(sample_agent.agent_id, AgentStatus.IDLE)

    def test_execute_action_flee(self, decision_system, sample_agent, sample_percepts):
    ...
    def test_execute_action_with_effects(self, decision_system, sample_agent):
    ...
    def test_execute_action_insufficient_energy(self, decision_system, sample_agent):
    ...
    def test_set_decision_strategy(self, decision_system, sample_agent):
    ...
    def test_time_pressure_calculation(self, decision_system, sample_agent):
