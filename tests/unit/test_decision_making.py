...
class TestAction:
    ...
    def test_action_hash(self):
        ...
        assert len(action_set) == 2  # action1 and action2 are same
...
class TestUtilityFunctions:
    ...
    def test_safety_utility(self, decision_context):
        ...
        # Fleeing should have high utility when threats exist
        ...
        # Moving away from threat should have positive utility
        ...
        # Moving towards threat should have negative utility
        ...
        # Waiting near threats should have negative utility
    ...
    def test_goal_utility(self, decision_context):
        ...
        # Moving towards goal should have positive utility
        ...
        # Moving away from goal should have negative utility
        move_away = Action(ActionType.MOVE, target=Position(0.0, 0.0, 0.0))
        away_utility = utility.calculate(move_away, decision_context)
        assert away_utility < 0

        # No goal context should return 0
    ...
    def test_resource_utility(self, sample_agent, decision_context):
        ...
        # Low energy should make gathering valuable
        ...
        # High energy should make gathering less valuable
        ...
        # Actions that cost more than available energy should have negative utility
    ...
    def test_social_utility(self, decision_context):
        ...
        # Communication should have value with nearby agents
        ...
        # Interaction utility should depend on relationship
...
class TestDecisionMaker:
    ...
    def test_goal_oriented_decision(self, decision_context):
        ...
        # Should prioritize actions that help achieve goal
    ...
    def test_hybrid_decision(self, decision_context):
        ...
        # High time pressure should trigger reactive behavior
        decision_context.time_pressure = 0.9
        decision = maker.decide(decision_context)

        assert decision is not None
        # Should react to threat
        assert decision.action_type == ActionType.FLEE
    ...
    def test_active_inference_decision(self, decision_context):
        ...
        # Should fall back to utility-based
        assert decision is not None
        assert isinstance(decision, Action)
...
class TestBehaviorTree:
    ...
    def test_condition_node(self, decision_context):
        ...
        # Create condition based on energy
        def low_energy(ctx):
    ...
    def test_behavior_tree_execution(self, decision_context):
        ...
        tree.root = ConditionNode(
            always_true,
            ActionNode(wait_generator)
        )
...
class TestActionGenerator:
    ...
    def test_generate_interaction_actions(self, sample_agent, sample_percepts):
        ...
        # Should generate interact action for nearby agent
    ...
    def test_generate_communication_actions(self, sample_agent, sample_percepts):
        ...
        # Should generate communicate action (agent nearby)
    ...
    def test_generate_resource_actions(self, sample_agent, sample_percepts):
        ...
        # Move resource close enough to gather
        sample_percepts[2].distance = 1.5
        actions = generator.generate_actions(sample_agent, sample_percepts)

        # Should generate gather action
...
class TestDecisionSystem:
    ...
    def test_execute_action_flee(self, decision_system, sample_agent, sample_percepts):
        ...
        decision_system.state_manager.get_agent.return_value = sample_agent
        decision_system.perception_system.perceive.return_value = [sample_percepts[0]]  # Just threat
        decision_system.movement_controller.set_destination.return_value = True
        ...
        # Check flee direction is away from threat
        call_args = decision_system.movement_controller.set_destination.call_args
        flee_position = call_args[0][1]
        assert flee_position.x < sample_agent.position.x  # Moving away from threat at (8,8)
    ...
    def test_execute_action_with_effects(self, decision_system, sample_agent):
        ...
        gather_action = Action(
            ActionType.WAIT,  # Simple action type for testing
            cost=2.0,
            effects={"energy": 20.0}
        )
        ...
        # Check energy was updated twice (cost and effect)
        calls = decision_system.state_manager.update_agent_resources.call_args_list
        assert len(calls) == 2
        assert calls[0][1]["energy_delta"] == -2.0  # Cost
        assert calls[1][1]["energy_delta"] == 20.0  # Effect
    ...
    def test_set_decision_strategy(self, decision_system, sample_agent):
        ...
        decision_system.set_decision_strategy(
            ...
            DecisionStrategy.GOAL_ORIENTED
        )
    ...
    def test_time_pressure_calculation(self, decision_system, sample_agent):
        ...
        # Low resources should increase time pressure
        sample_agent.resources.energy = 20.0
        sample_agent.resources.health = 30.0

        pressure = decision_system._calculate_time_pressure(sample_agent)
        # Energy: (30 - 20) / 30 * 0.5 = 0.167
        # Health: (50 - 30) / 50 * 0.5 = 0.2
        # Total: 0.167 + 0.2 = 0.367
        assert pressure > 0.3
        assert pressure <= 1.0

        # High resources should have low pressure
