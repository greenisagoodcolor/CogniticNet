"""
Unit tests for Edge Feature Processing Module

Tests edge processing, feature extraction, and graph conversion functionality.
"""
import pytest
import numpy as np
import torch
from .......inference.gnn.edge-processor import EdgeProcessor, EdgeConfig, Edge, EdgeBatch, EdgeType, EdgeFeatureType
...
class TestEdgeConfig:
    ...
    def test_default_config(self):
        ...
        assert config.self_loops == False
        assert config.max_edges_per_node is None
        assert config.edge_sampling_strategy is None

    def test_custom_config(self):
        """Test custom configuration values"""
        config = EdgeConfig(edge_type=EdgeType.UNDIRECTED, feature_types=[EdgeFeatureType.WEIGHT, EdgeFeatureType.DISTANCE], normalize_weights=False, self_loops=True, max_edges_per_node=5, edge_sampling_strategy='topk')
        assert config.edge_type == EdgeType.UNDIRECTED
        assert len(config.feature_types) == 2
        assert config.normalize_weights == False
        assert config.self_loops == True
...
class TestEdge:
    ...
    def test_edge_with_features(self):
        """Test edge creation with features"""
        edge = Edge(source=0, target=1, weight=0.8, features={'distance': 1.5, 'category': 'friend'}, edge_type='social', metadata={'timestamp': 123456})
...
class TestEdgeProcessor:
    ...
    def test_directed_edges(self):
        ...
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1), Edge(source=1, target=2), Edge(source=2, target=0)]
    ...
    def test_undirected_edges(self):
        ...
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1), Edge(source=1, target=0), Edge(source=1, target=2)]
        batch = processor.process_edges(edges, num_nodes=3)
    ...
    def test_weight_features(self):
        ...
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, weight=0.5), Edge(source=1, target=2, weight=1.0), Edge(source=2, target=0, weight=0.2)]
        batch = processor.process_edges(edges, num_nodes=3)
    ...
    def test_distance_features(self):
        ...
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, features={'distance': 1.5}), Edge(source=1, target=2, features={'distance': 2.0}), Edge(source=2, target=0, features={'distance': 0.5})]
        batch = processor.process_edges(edges, num_nodes=3)
        assert batch.edge_attr is not None
        assert batch.edge_attr.shape == (3, 1)
    ...
    def test_categorical_features(self):
        ...
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, features={'category': 'friend'}), Edge(source=1, target=2, features={'category': 'colleague'}), Edge(source=2, target=0, features={'category': 'friend'}), Edge(source=0, target=2, features={'category': 'family'})]
        batch = processor.process_edges(edges, num_nodes=3)
        assert batch.edge_attr is not None
    ...
    def test_temporal_features(self):
        ...
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, features={'timestamp': 1642000000}), Edge(source=1, target=2, features={'timestamp': '2022-01-12T17:00:00'})]
        batch = processor.process_edges(edges, num_nodes=3)
        assert batch.edge_attr is not None
    ...
    def test_embedding_features(self):
        ...
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, features={'embedding': [0.1, 0.2, 0.3, 0.4]}), Edge(source=1, target=2), Edge(source=2, target=0, features={'embedding': 'edge_123'})]
        batch = processor.process_edges(edges, num_nodes=3)
        assert batch.edge_attr is not None
    ...
    def test_multiple_features(self):
        ...
        config = EdgeConfig(feature_types=[EdgeFeatureType.WEIGHT, EdgeFeatureType.DISTANCE, EdgeFeatureType.CATEGORICAL])
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, weight=0.8, features={'distance': 1.5, 'category': 'friend'}), Edge(source=1, target=2, weight=0.6, features={'distance': 2.0, 'category': 'colleague'})]
        batch = processor.process_edges(edges, num_nodes=3)
        assert batch.edge_attr is not None
        assert batch.edge_attr.shape[1] >= 4
    ...
    def test_edge_sampling_topk(self):
        ...
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, weight=0.1), Edge(source=0, target=2, weight=0.9), Edge(source=0, target=3, weight=0.5), Edge(source=0, target=4, weight=0.7)]
    ...
    def test_edge_types(self):
        ...
        config = EdgeConfig()
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, edge_type='social'), Edge(source=1, target=2, edge_type='professional'), Edge(source=2, target=0, edge_type='social'), Edge(source=0, target=3)]
    ...
    def test_adjacency_matrix_conversion(self):
        ...
        config = EdgeConfig()
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, weight=0.8), Edge(source=1, target=2, weight=0.6), Edge(source=2, target=0, weight=0.9)]
        batch = processor.process_edges(edges, num_nodes=3)
    ...
    def test_edge_statistics(self):
        ...
        config = EdgeConfig()
        processor = EdgeProcessor(config)
        edges = [Edge(source=0, target=1, weight=0.8, edge_type='type_a'), Edge(source=1, target=2, weight=0.6, edge_type='type_b'), Edge(source=2, target=0, weight=0.9, edge_type='type_a'), Edge(source=0, target=0, weight=1.0)]
        batch = processor.process_edges(edges, num_nodes=3)
