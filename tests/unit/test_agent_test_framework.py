...
import time
from .......agents.base.data-model import Agent, Position, AgentStatus, AgentCapability
from .......agents.testing.agent-test-framework import TestScenario, TestMetrics, AgentFactory, SimulationEnvironment, BehaviorValidator, PerformanceBenchmark, TestOrchestrator, create_basic_test_scenarios
...
class TestAgentFactory:
    ...
    def test_create_resource_agent(self):
        ...
        agent = AgentFactory.create_resource_agent('resource_agent', resource_types=['gold', 'wood'])
        assert agent is not None
    ...
    def test_create_social_agent(self):
        ...
        agent = AgentFactory.create_social_agent('social_agent', trust_threshold=0.7)
        assert agent is not None
...
class TestBehaviorValidator:
    ...
    def test_movement_validation_success(self):
        ...
        history = [{'timestamp': 0.0, 'position': Position(0, 0, 0)}, {'timestamp': 0.1, 'position': Position(1, 0, 0)}, {'timestamp': 0.2, 'position': Position(2, 1, 0)}]
    ...
    def test_movement_validation_failure(self):
        ...
        validator = BehaviorValidator()
        agent = AgentFactory.create_basic_agent('validator_agent')
        history = [{'timestamp': 0.0, 'position': Position(0, 0, 0)}, {'timestamp': 0.1, 'position': Position(200, 200, 0)}]
        success, error = validator.validate('movement_coherence', agent, history)
        assert not success
...
class TestTestOrchestrator:
    ...
    def test_add_scenario(self):
        ...
        orchestrator = TestOrchestrator()
        scenario = TestScenario(name='Test', description='Test scenario', duration=1.0, agent_configs=[{'type': 'basic', 'id': 'test1'}], environment_config={'bounds': (-10, -10, 10, 10)}, success_criteria={}, metrics_to_track=[])
    ...
    def test_run_scenario(self):
        ...
        orchestrator = TestOrchestrator()
        scenario = TestScenario(name='Quick Test', description='Quick test scenario', duration=0.5, agent_configs=[{'type': 'basic', 'id': 'agent1'}, {'type': 'resource', 'id': 'agent2'}], environment_config={'bounds': (-20, -20, 20, 20), 'time_scale': 10.0}, success_criteria={'test': True}, metrics_to_track=['position', 'energy'])
    ...
    def test_generate_report(self):
        ...
        orchestrator = TestOrchestrator()
        scenario = TestScenario(name='Report Test', description='Test for reporting', duration=0.1, agent_configs=[{'type': 'basic', 'id': 'report_agent'}], environment_config={'bounds': (-5, -5, 5, 5)}, success_criteria={}, metrics_to_track=[])
        orchestrator.add_scenario(scenario)
