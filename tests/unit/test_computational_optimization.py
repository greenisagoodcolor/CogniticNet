...
class TestSparseOperations:
    ...
    def test_sparsify_tensor(self):
        ...
        # Create tensor with many zeros
        tensor = torch.tensor([
            [1.0, 0.0, 0.0, 0.5],
            [0.0, 0.0, 2.0, 0.0],
            [0.0, 0.001, 0.0, 3.0],  # 0.001 should be zeroed by threshold
            [0.0, 0.0, 0.0, 0.0]
        ])
        ...
        # Check values above threshold are preserved
        dense_result = sparse.to_dense()
        assert dense_result[0, 0] == 1.0
        assert dense_result[1, 2] == 2.0
        assert dense_result[2, 3] == 3.0

        # Check small values are zeroed
        assert dense_result[2, 1] == 0.0  # 0.001 < threshold
...
class TestParallelInference:
    ...
    def test_parallel_belief_updates(self):
        ...
        A_matrices = [
            torch.rand(3, 4) for _ in range(3)
        ]
    ...
    def test_non_parallel_fallback(self):
        ...
        # Should still work without parallel processing
        updated_beliefs = self.parallel.parallel_belief_updates(
            beliefs, observations, A_matrices
        )
...
class TestCachedInference:
    ...
    def test_cache_ttl(self):
        ...
        self.cache.config.cache_ttl = 0.1  # 100ms TTL
...
class TestGPUOptimizer:
    ...
    def test_cuda_graph_creation(self):
        ...
        # Should return original function on CPU
...
class TestBatchProcessor:
    ...
    def test_add_request(self):
        ...
        # Should not process yet (batch size is 3)
    ...
    def test_batch_belief_updates(self):
        ...
        # Check all results were computed
...
class TestComputationalOptimizer:
    ...
    def test_automatic_sparse_detection(self):
        ...
        # Create sparse A matrix (>50% zeros)
        A = torch.zeros(15, 20)
        for i in range(15):
            A[i, i % 20] = 0.9
            A[i, (i + 1) % 20] = 0.1

        # Should automatically use sparse operations
        result = self.optimizer.optimized_belief_update(belief, observation, A)
...
class TestIntegration:
    ...
    def test_full_inference_cycle(self):
        ...
        # Simulate inference over multiple timesteps
        for t in range(10):
            # Observe
            obs_idx = t % 3
        ...
        assert report['cache_stats']['hit_rate'] > 0  # Should have some cache hits
