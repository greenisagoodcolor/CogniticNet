...
from inference.engine.belief_update import BeliefUpdateConfig, DirectGraphObservationModel, LearnedGraphObservationModel, GNNBeliefUpdater, AttentionGraphBeliefUpdater, HierarchicalBeliefUpdater, create_belief_updater
...
class TestBeliefUpdateConfig:
    ...
    def test_custom_config(self):
        ...
        config = BeliefUpdateConfig(update_method='gradient', learning_rate=0.1, feature_weighting='attention', use_gpu=False)
...
class TestDirectGraphObservationModel:
    ...
    def test_generate_observations(self):
        ...
        assert torch.allclose(norms, torch.ones(3) * config.feature_scale, atol=1e-06)
...
class TestGNNBeliefUpdater:
    ...
    def test_hybrid_update(self, setup_updater):
        ...
        updated_with_temporal = updater.update_beliefs(current_beliefs, graph_features, None, previous_beliefs)
        assert updated_with_temporal.shape == (2, 4)
        assert torch.allclose(updated_with_temporal.sum(dim=-1), torch.ones(2), atol=1e-06)
    ...
    def test_update_with_graph_sequence(self, setup_updater):
        ...
        graph_sequence = [{'graph_features': torch.randn(2, 8)}, {'graph_features': torch.randn(2, 8)}, {'graph_features': torch.randn(2, 8)}]
        belief_trajectory = updater.update_with_graph_sequence(initial_beliefs, graph_sequence)
        assert len(belief_trajectory) == 4
        for beliefs in belief_trajectory:
            assert beliefs.shape == (2, 4)
            assert torch.allclose(beliefs.sum(dim=-1), torch.ones(2), atol=1e-06)
...
class TestAttentionGraphBeliefUpdater:
    ...
    def setup_attention_updater(self):
        ...
        inference = VariationalMessagePassing(inf_config)
        obs_model = DirectGraphObservationModel(config, state_dim=4, feature_dim=8)
        updater = AttentionGraphBeliefUpdater(config, gen_model, inference, obs_model, num_heads=2)
        return updater
    ...
    def test_compute_attended_features(self, setup_attention_updater):
        ...
        graph_features = torch.randn(2, 5, 8)
        attended_features, attention_weights = updater._compute_attended_features(beliefs, graph_features)
...
class TestHierarchicalBeliefUpdater:
    ...
    def test_update_hierarchical_beliefs(self, setup_hierarchical_updater):
        ...
        updater = setup_hierarchical_updater
        current_beliefs = [torch.softmax(torch.randn(2, 4), dim=-1) for _ in range(3)]
        graph_features_per_level = [torch.randn(2, 8) for _ in range(3)]
        updated_beliefs = updater.update_hierarchical_beliefs(current_beliefs, graph_features_per_level)
...
class TestFactoryFunction:
    ...
    def test_create_standard_updater(self):
        ...
        inference = VariationalMessagePassing(inf_config)
        obs_model = DirectGraphObservationModel(config, state_dim=4, feature_dim=8)
        updater = create_belief_updater('standard', config, generative_model=gen_model, inference_algorithm=inference, observation_model=obs_model)
    ...
    def test_create_attention_updater(self):
        ...
        inference = VariationalMessagePassing(inf_config)
        obs_model = DirectGraphObservationModel(config, state_dim=4, feature_dim=8)
        updater = create_belief_updater('attention', config, generative_model=gen_model, inference_algorithm=inference, observation_model=obs_model, num_heads=4)
    ...
    def test_create_hierarchical_updater(self):
        ...
        for i in range(2):
            ...
            updater = GNNBeliefUpdater(config, gen_model, inference, obs_model)
            level_updaters.append(updater)
        hier_updater = create_belief_updater('hierarchical', config, level_updaters=level_updaters)
