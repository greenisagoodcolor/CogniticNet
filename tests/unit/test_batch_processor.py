...
import torch
import numpy as np
from .......inference.gnn.batch-processor import GraphData, BatchedGraphData, GraphBatchProcessor, DynamicBatchSampler, StreamingBatchProcessor, create_mini_batches
...
class TestGraphData:
    ...
    def test_graph_data_with_attributes(self):
        """Test GraphData with all attributes"""
        graph = GraphData(node_features=torch.randn(5, 16), edge_index=torch.tensor([[0, 1], [1, 0]], dtype=torch.long), edge_attr=torch.randn(2, 8), edge_weight=torch.tensor([0.5, 0.8]), graph_attr=torch.randn(4), target=torch.tensor([1]), mask=torch.ones(5, dtype=torch.bool), metadata={'name': 'test_graph'})
...
class TestGraphBatchProcessor:
    ...
    def test_initialization(self):
        ...
        assert processor.pad_node_features == True
    ...
    def test_create_batch_simple(self):
        ...
        graphs = [GraphData(node_features=torch.randn(3, 16), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long)), GraphData(node_features=torch.randn(4, 16), edge_index=torch.tensor([[0, 1, 2], [1, 2, 3]], dtype=torch.long))]
    ...
    def test_batch_with_edge_attributes(self):
        """Test batching with edge attributes"""
        processor = GraphBatchProcessor(use_torch_geometric=False)
        graphs = [GraphData(node_features=torch.randn(3, 16), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long), edge_attr=torch.randn(2, 4), edge_weight=torch.tensor([0.5, 0.8])), GraphData(node_features=torch.randn(2, 16), edge_index=torch.tensor([[0], [1]], dtype=torch.long), edge_attr=torch.randn(1, 4), edge_weight=torch.tensor([0.9]))]
        batch = processor.create_batch(graphs)
    ...
    def test_batch_with_padding(self):
        ...
        processor = GraphBatchProcessor(pad_node_features=True, max_nodes_per_graph=5, use_torch_geometric=False)
        graphs = [GraphData(node_features=torch.randn(3, 16), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long)), GraphData(node_features=torch.randn(2, 16), edge_index=torch.tensor([[0], [1]], dtype=torch.long))]
        batch = processor.create_batch(graphs)
    ...
    def test_batch_with_targets(self):
        ...
        processor = GraphBatchProcessor(use_torch_geometric=False)
        graphs = [GraphData(node_features=torch.randn(3, 16), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long), target=torch.tensor([0])), GraphData(node_features=torch.randn(4, 16), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long), target=torch.tensor([1]))]
        batch = processor.create_batch(graphs)
    ...
    def test_batch_with_graph_attributes(self):
        """Test batching with graph-level attributes"""
        processor = GraphBatchProcessor(pad_graph_features=True, use_torch_geometric=False)
        graphs = [GraphData(node_features=torch.randn(3, 16), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long), graph_attr=torch.randn(10)), GraphData(node_features=torch.randn(2, 16), edge_index=torch.tensor([[0], [1]], dtype=torch.long), graph_attr=torch.randn(8))]
        batch = processor.create_batch(graphs)
    ...
    def test_unbatch(self):
        ...
        processor = GraphBatchProcessor(use_torch_geometric=False)
        original_graphs = [GraphData(node_features=torch.randn(3, 16), edge_index=torch.tensor([[0, 1, 2], [1, 2, 0]], dtype=torch.long), edge_attr=torch.randn(3, 4), target=torch.tensor([0])), GraphData(node_features=torch.randn(4, 16), edge_index=torch.tensor([[0, 1], [2, 3]], dtype=torch.long), edge_attr=torch.randn(2, 4), target=torch.tensor([1]))]
    ...
    def test_collate_fn(self):
        ...
        processor = GraphBatchProcessor()
        graphs = [GraphData(node_features=torch.randn(5, 32), edge_index=torch.randint(0, 5, (2, 10))) for _ in range(4)]
...
class TestDynamicBatchSampler:
    ...
    def test_size_grouping(self):
        ...
        sampler = DynamicBatchSampler(graph_sizes=graph_sizes, batch_size=2, size_threshold=0.2, shuffle=False)
...
class TestStreamingBatchProcessor:
    ...
    def test_process_stream(self):
        ...
        def graph_generator():
            for i in range(10):
                yield GraphData(node_features=torch.randn(5, 16), edge_index=torch.randint(0, 5, (2, 10)), target=torch.tensor([i % 3]))
    ...
    def test_process_stream_with_function(self):
        ...
        def graph_generator():
            for i in range(6):
                yield GraphData(node_features=torch.randn(3, 8), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long))

        def process_fn(batch):
            return batch.x.shape[0]
        results = list(streaming.process_stream(graph_generator(), batch_size=2, process_fn=process_fn))
...
class TestUtilityFunctions:
    ...
    def test_create_mini_batches(self):
        ...
        graphs = [GraphData(node_features=torch.randn(5, 16), edge_index=torch.randint(0, 5, (2, 10))) for _ in range(10)]
    ...
    def test_create_mini_batches_drop_last(self):
