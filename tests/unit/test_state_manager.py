...
class TestStateTransitionValidator(unittest.TestCase):
    ...
    def test_valid_transitions(self):
        ...
        self.assertTrue(StateTransitionValidator.is_valid_transition(AgentStatus.IDLE, AgentStatus.MOVING))
        self.assertTrue(StateTransitionValidator.is_valid_transition(AgentStatus.IDLE, AgentStatus.PLANNING))
        self.assertTrue(StateTransitionValidator.is_valid_transition(AgentStatus.MOVING, AgentStatus.IDLE))
        self.assertTrue(StateTransitionValidator.is_valid_transition(AgentStatus.MOVING, AgentStatus.INTERACTING))
        self.assertTrue(StateTransitionValidator.is_valid_transition(AgentStatus.IDLE, AgentStatus.IDLE))

    def test_invalid_transitions(self):
        """Test invalid state transitions"""
        self.assertFalse(StateTransitionValidator.is_valid_transition(AgentStatus.OFFLINE, AgentStatus.MOVING))
        self.assertFalse(StateTransitionValidator.is_valid_transition(AgentStatus.ERROR, AgentStatus.PLANNING))
...
class TestAgentStateManager(unittest.TestCase):
    ...
    def test_status_update_forced(self):
        ...
        self.state_manager.register_agent(self.test_agent)
        self.state_manager.update_agent_status(self.test_agent.agent_id, AgentStatus.OFFLINE)
        self.state_manager.update_agent_status(self.test_agent.agent_id, AgentStatus.MOVING, force=True)
        agent = self.state_manager.get_agent(self.test_agent.agent_id)
        self.assertEqual(agent.status, AgentStatus.MOVING)
    ...
    def test_position_update(self):
        ...
        self.assertEqual(agent.status, AgentStatus.MOVING)
        events = [e for e in self.test_observer.events if e.event_type == StateEventType.POSITION_UPDATED]
        self.assertEqual(len(events), 1)
    ...
    def test_transition_callbacks(self):
        ...
        self.state_manager.register_transition_callback(self.test_agent.agent_id, AgentStatus.IDLE, AgentStatus.MOVING, test_callback)
        self.state_manager.update_agent_status(self.test_agent.agent_id, AgentStatus.MOVING)
    ...
    def test_state_history(self):
        ...
        status_history = self.state_manager.get_state_history(event_type=StateEventType.STATUS_CHANGED)
    ...
    def test_batch_update(self):
        ...
        updates = [{'type': 'status', 'agent_id': agent1.agent_id, 'value': AgentStatus.MOVING}, {'type': 'position', 'agent_id': agent1.agent_id, 'value': Position(10, 10)}, {'type': 'status', 'agent_id': agent2.agent_id, 'value': AgentStatus.PLANNING}]
    ...
    def test_thread_safety(self):
        ...
        def update_status_loop():
            try:
                for _ in range(50):
                    self.state_manager.update_agent_status(self.test_agent.agent_id, AgentStatus.PLANNING)
                    self.state_manager.update_agent_status(self.test_agent.agent_id, AgentStatus.IDLE)
        ...
        def update_position_loop():
            try:
                for i in range(50):
                    self.state_manager.update_agent_position(self.test_agent.agent_id, Position(i, i))
            except Exception as e:
                errors.append(e)
        threads = []
        for _ in range(3):
...
class TestStateMonitor(unittest.TestCase):
    ...
    def test_condition_monitoring(self):
        ...
        def low_energy_callback(agent, condition):
            ...
            condition_met = True
        low_energy_condition = StateCondition('low_energy', lambda agent: agent.resources.energy < 30.0)
        self.monitor.add_condition(self.test_agent.agent_id, low_energy_condition, low_energy_callback)
