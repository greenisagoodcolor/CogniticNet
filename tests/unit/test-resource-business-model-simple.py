...
class TestResourceBusinessModel(unittest.TestCase):
    ...
    def test_market_price_dynamics(self):
    ...
    def test_market_operations(self):
        ...
        matches = market.find_matching_offers(
            agent_id="agent2",
            wanted={ResourceType.WATER: 40.0},
            offered={ResourceType.FOOD: 30.0}
        )

        self.assertEqual(len(matches), 1)
        self.assertEqual(matches[0].offer_id, "offer1")

    def test_resource_business_manager(self):
        ...
        trust = manager.get_trust_level("agent1", "agent2")
        self.assertAlmostEqual(trust, 0.6, places=5)  # 0.8 - 0.2 (using assertAlmostEqual for floating point)
