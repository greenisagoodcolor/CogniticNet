...
class TestExperienceBuffer:
    ...
    def test_add_experience(self):
        ...
        buffer.add(exp3)
        assert len(buffer) == 2  # Should still be 2 due to max_size
...
class TestDiscreteParameterLearner:
    ...
    def test_initialization(self):
        ...
        # Check Dirichlet concentrations
    ...
    def test_bayesian_update(self):
        ...
        # Check that concentrations increased
...
class TestOnlineParameterLearner:
    def setup_method(self):
        ...
        # Create discrete model and learner
        dims = ModelDimensions(
            num_states=4,
            num_observations=3,
            num_actions=2
    ...
    def test_update_with_sufficient_buffer(self):
        ...
        # Add experiences to meet min_buffer_size
        for i in range(25):
            state = torch.tensor([1.0, 0.0, 0.0, 0.0])
            action = torch.tensor([0.0, 1.0])
            observation = torch.tensor([0.0, 1.0, 0.0])
    ...
    def test_get_statistics(self):
        ...
        # Add some experiences and updates
        for i in range(30):
            state = torch.tensor([1.0, 0.0, 0.0, 0.0])
            action = torch.tensor([0.0, 1.0])
            observation = torch.tensor([0.0, 1.0, 0.0])
    ...
    def test_no_replay_buffer(self):
        ...
        # Should update on every observation
        state = torch.tensor([1.0, 0.0, 0.0, 0.0])
        action = torch.tensor([0.0, 1.0])
        observation = torch.tensor([0.0, 1.0, 0.0])
        next_state = torch.tensor([0.0, 1.0, 0.0, 0.0])
        ...
        # Without replay buffer, should update immediately
...
class TestCreateParameterLearner:
    ...
    def test_default_config(self):
        ...
        assert isinstance(learner, DiscreteParameterLearner)
        assert learner.config.learning_rate_A == 0.01  # Default value
