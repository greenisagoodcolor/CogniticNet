...
from .......inference.engine.active-learning import ActiveLearningConfig, InformationMetric, EntropyBasedSeeker, MutualInformationSeeker, ActiveLearningAgent, InformationGainPlanner, create_active_learner
from .......inference.engine.generative-model import DiscreteGenerativeModel, ModelDimensions, ModelParameters
from inference.engine.inference import VariationalMessagePassing, InferenceConfig
from .......inference.engine.policy-selection import DiscreteExpectedFreeEnergy, PolicyConfig, Policy
...
class TestActiveLearningConfig:
    ...
    def test_custom_config(self):
        ...
        config = ActiveLearningConfig(exploration_weight=0.5, information_metric=InformationMetric.ENTROPY, planning_horizon=10, use_gpu=False)
...
class TestEntropyBasedSeeker:
    ...
    def test_compute_entropy(self, setup_seeker):
        ...
        assert torch.allclose(entropy, torch.tensor(expected_entropy), atol=1e-05)
    ...
    def test_compute_information_value(self, setup_seeker):
        ...
        assert torch.all(info_values >= 0)
...
class TestActiveLearningAgent:
    ...
    def test_compute_epistemic_value(self, setup_agent):
        ...
        policies = [Policy(actions=[0, 1], horizon=2), Policy(actions=[1, 0], horizon=2)]
...
class TestInformationGainPlanner:
    ...
    def test_plan_information_gathering(self, setup_planner):
        ...
        planned_actions = planner.plan_information_gathering(current_beliefs, target_uncertainty=0.5, max_steps=5)
...
class TestFactoryFunction:
    ...
    def test_create_agent(self):
        ...
        pol_config = PolicyConfig(use_gpu=False)
        policy_selector = DiscreteExpectedFreeEnergy(pol_config)
        agent = create_active_learner('agent', config, generative_model=gen_model, inference_algorithm=inference, policy_selector=policy_selector)
