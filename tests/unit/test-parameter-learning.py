"""
Unit tests for Active Inference Parameter Learning
"""
import pytest
import torch
import numpy as np
from unittest.mock import Mock, patch
from .......inference.engine.parameter-learning import LearningConfig, Experience, ExperienceBuffer, DiscreteParameterLearner, ContinuousParameterLearner, OnlineParameterLearner, create_parameter_learner
...
class TestExperienceBuffer:
    ...
    def test_add_experience(self):
        ...
        exp1 = Experience(state=torch.tensor([1.0, 0.0]), action=torch.tensor([0.0, 1.0]), observation=torch.tensor([0.5]), next_state=torch.tensor([0.0, 1.0]))
        buffer.add(exp1)
        assert len(buffer) == 1
        exp2 = Experience(state=torch.tensor([0.0, 1.0]), action=torch.tensor([1.0, 0.0]), observation=torch.tensor([0.7]), next_state=torch.tensor([1.0, 0.0]))
        buffer.add(exp2)
        assert len(buffer) == 2
        exp3 = Experience(state=torch.tensor([0.5, 0.5]), action=torch.tensor([0.5, 0.5]), observation=torch.tensor([0.6]), next_state=torch.tensor([0.4, 0.6]))
        buffer.add(exp3)
        assert len(buffer) == 2
    ...
    def test_sample_experiences(self):
        ...
        for i in range(5):
            exp = Experience(state=torch.tensor([float(i)]), action=torch.tensor([float(i)]), observation=torch.tensor([float(i)]), next_state=torch.tensor([float(i + 1)]))
            buffer.add(exp)
...
class TestDiscreteParameterLearner:
    ...
    def test_bayesian_update(self):
        ...
        experiences = []
        for i in range(10):
            exp = Experience(state=torch.tensor([1.0, 0.0, 0.0, 0.0]), action=torch.tensor([0.0, 1.0]), observation=torch.tensor([0.0, 1.0, 0.0]), next_state=torch.tensor([0.0, 1.0, 0.0, 0.0]), timestamp=i)
    ...
    def test_gradient_update(self):
        ...
        experiences = []
        for i in range(5):
            exp = Experience(state=torch.tensor([1.0, 0.0, 0.0, 0.0]), action=torch.tensor([0.0, 1.0]), observation=torch.tensor([0.0, 1.0, 0.0]), next_state=torch.tensor([0.0, 1.0, 0.0, 0.0]), timestamp=0)
            experiences.append(exp)
        metrics = self.learner.update_parameters(experiences, self.model)
    ...
    def test_learning_rate_decay(self):
        ...
        exp = Experience(state=torch.tensor([1.0, 0.0, 0.0, 0.0]), action=torch.tensor([0.0, 1.0]), observation=torch.tensor([0.0, 1.0, 0.0]), next_state=torch.tensor([0.0, 1.0, 0.0, 0.0]))
...
class TestContinuousParameterLearner:
    ...
    def test_update_parameters(self):
        ...
        experiences = []
        for i in range(5):
            exp = Experience(state=torch.randn(4), action=torch.randn(2), observation=torch.randn(3), next_state=torch.randn(4))
            experiences.append(exp)
        metrics = self.learner.update_parameters(experiences, self.model)
    ...
    def test_transition_model_update(self):
        """Test transition model update"""
        states = torch.randn(5, 4)
        actions = torch.randn(5, 2)
        next_states = torch.randn(5, 4)
        initial_params = [p.clone() for p in self.model.transition_net.parameters()]
        loss = self.learner._update_transition_model(states, actions, next_states, self.model)
        assert loss > 0
        for initial, current in zip(initial_params, self.model.transition_net.parameters()):
            assert not torch.allclose(initial, current)

    def test_observation_model_update(self):
        """Test observation model update"""
        states = torch.randn(5, 4)
        observations = torch.randn(5, 3)
        initial_params = [p.clone() for p in self.model.observation_net.parameters()]
        loss = self.learner._update_observation_model(states, observations, self.model)
        assert loss > 0
        for initial, current in zip(initial_params, self.model.observation_net.parameters()):
            assert not torch.allclose(initial, current)
...
class TestOnlineParameterLearner:
    ...
    def setup_method(self):
        """Setup for tests"""
        self.config = LearningConfig(use_experience_replay=True, replay_buffer_size=100, batch_size=10, min_buffer_size=20, update_frequency=5, use_gpu=False)
        dims = ModelDimensions(num_states=4, num_observations=3, num_actions=2)
        params = ModelParameters(use_gpu=False)
        self.model = DiscreteGenerativeModel(dims, params)
    ...
    def test_update_with_sufficient_buffer(self):
        ...
        for i in range(25):
            ...
            next_state = torch.tensor([0.0, 1.0, 0.0, 0.0])
            self.learner.observe(state, action, observation, next_state)
        expected_updates = 25 // self.config.update_frequency
    ...
    def test_no_replay_buffer(self):
...
class TestCreateParameterLearner:
    ...
    def test_create_discrete_learner(self):
    ...
    def test_create_continuous_learner(self):
    ...
    def test_create_learner_errors(self):
