"""
Unit tests for Hierarchical Inference
"""
...
class TestHierarchicalConfig:
    def test_default_config(self):
        ...
        assert config.use_precision_weighting == True
...
class TestHierarchicalLevel:
    ...
    def setup_level(self):
        ...
        # Create middle level (has both bottom-up and top-down)
    ...
    def test_initialization(self, setup_level):
        ...
        assert level.timescale == 4.0  # Second level
    ...
    def test_initialize_state(self, setup_level):
        ...
        # Check beliefs are normalized
        assert torch.allclose(state.beliefs.sum(dim=-1), torch.ones(3), atol=1e-6)

    def test_compute_prediction_error(self, setup_level):
    ...
    def test_update_beliefs(self, setup_level):
        """Test belief update"""
        level, _, _ = setup_level
        ...
        # Create inputs
        bottom_up_input = torch.randn(2, 8)  # From lower level
        top_down_input = torch.randn(2, 32)  # From higher level
    ...
    def test_temporal_buffer(self, setup_level):
        ...
        # Check buffer
        assert len(level.state.temporal_buffer) == 4  # prediction_horizon
...
class TestHierarchicalInference:
    @pytest.fixture
    def setup_hierarchical_system(self):
    ...
    def test_initialization(self, setup_hierarchical_system):
    ...
    def test_system_initialize(self, setup_hierarchical_system):
    ...
    def test_step(self, setup_hierarchical_system):
        ...
        for i, level_beliefs in enumerate(beliefs):
            ...
            assert torch.allclose(level_beliefs.sum(dim=-1), torch.ones(batch_size), atol=1e-6)

    def test_timescale_updates(self, setup_hierarchical_system):
        ...
        # Check updates
        # Level 0 (timescale=1) should update every step
        assert not torch.allclose(system.levels[0].state.beliefs, initial_beliefs[0])

        # Level 1 (timescale=4) should have updated once
        assert not torch.allclose(system.levels[1].state.beliefs, initial_beliefs[1])

        # Level 2 (timescale=16) should not have updated yet
        # (This assertion might fail due to top-down influences)

    def test_hierarchical_free_energy(self, setup_hierarchical_system):
    ...
    def test_get_effective_beliefs(self, setup_hierarchical_system):
        ...
        # Get effective beliefs for bottom level
        effective_beliefs = system.get_effective_beliefs(target_level=0)

        assert effective_beliefs.shape == (batch_size, 8)
        assert torch.allclose(effective_beliefs.sum(dim=-1), torch.ones(batch_size), atol=1e-6)
...
class TestTemporalHierarchicalInference:
    ...
    def test_initialization(self, setup_temporal_system):
    ...
    def test_predict_future_states(self, setup_temporal_system):
        ...
        for pred in predictions:
            ...
            assert torch.allclose(pred.sum(dim=-1), torch.ones(batch_size), atol=1e-6)

    def test_hierarchical_planning(self, setup_temporal_system):
        ...
        assert len(trajectories) == 2  # One per level

        # Check trajectory lengths match prediction horizons
        assert len(trajectories[0]) == min(5, 2)  # Level 0 horizon is 2
        assert len(trajectories[1]) == min(5, 8)  # Level 1 horizon is 8

    def test_coarse_to_fine_inference(self, setup_temporal_system):
...
class TestFactoryFunction:
    def test_create_standard_hierarchical(self):
    ...
    def test_create_temporal_hierarchical(self):
