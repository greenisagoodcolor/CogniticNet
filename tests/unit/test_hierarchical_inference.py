...
from .......inference.engine.hierarchical-inference import HierarchicalConfig, HierarchicalState, HierarchicalLevel, HierarchicalInference, TemporalHierarchicalInference, create_hierarchical_inference
...
class TestHierarchicalConfig:
    ...
    def test_custom_config(self):
        ...
        config = HierarchicalConfig(num_levels=4, level_dims=[4, 8, 16, 32], timescales=[1.0, 2.0, 4.0, 8.0], use_gpu=False)
...
class TestHierarchicalState:
    ...
    def test_state_creation(self):
        ...
        state = HierarchicalState(beliefs=beliefs, predictions=predictions, errors=errors, precision=precision)
...
class TestHierarchicalLevel:
    ...
    def setup_level(self):
        ...
        level = HierarchicalLevel(level_id=1, config=config, generative_model=gen_model, inference_algorithm=inference, precision_optimizer=precision_opt)
...
class TestHierarchicalInference:
    ...
    def setup_hierarchical_system(self):
        ...
        optimizers = []
        for i in range(3):
            dims = ModelDimensions(num_states=config.level_dims[i], num_observations=config.level_dims[i] // 2, num_actions=2)
            params = ModelParameters(use_gpu=False)
...
class TestTemporalHierarchicalInference:
    ...
    def setup_temporal_system(self):
        ...
        config = HierarchicalConfig(num_levels=2, level_dims=[8, 16], timescales=[1.0, 4.0], prediction_horizon=[2, 8], use_gpu=False)
        models = []
        algorithms = []
        for i in range(2):
            dims = ModelDimensions(num_states=config.level_dims[i], num_observations=config.level_dims[i] // 2, num_actions=2)
...
class TestFactoryFunction:
    ...
    def test_create_standard_hierarchical(self):
        ...
        for i in range(2):
            ...
            inf_config = InferenceConfig(use_gpu=False)
            algorithms.append(VariationalMessagePassing(inf_config))
        system = create_hierarchical_inference('standard', config, generative_models=models, inference_algorithms=algorithms)
    ...
    def test_create_temporal_hierarchical(self):
        ...
        for i in range(2):
            ...
            inf_config = InferenceConfig(use_gpu=False)
            algorithms.append(VariationalMessagePassing(inf_config))
        system = create_hierarchical_inference('temporal', config, generative_models=models, inference_algorithms=algorithms)
        assert isinstance(system, TemporalHierarchicalInference)
