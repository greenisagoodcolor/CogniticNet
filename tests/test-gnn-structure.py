...
class TestGNNStructure(unittest.TestCase):
    ...
    def test_documentation_files_exist(self):
        ...
        required_docs = [
            'doc/about_platform.md',
            'doc/active_inference_guide.md',
            'doc/gnn_models/model_format.md',
        ]
    ...
    def test_main_pipeline_exists(self):
        ...
        self.assertTrue(
            main_path.exists(),
            "Pipeline orchestrator 'src/main.py' does not exist"
        )
    ...
    def test_gnn_models_exist(self):
        ...
        model_files = [
            'models/base/explorer.gnn.md',
            'models/base/merchant.gnn.md',
        ]
        ...
        for model_path in model_files:
            ...
            self.assertTrue(
                full_path.exists(),
                f"GNN model '{model_path}' does not exist"
            )
    ...
    def test_no_scattered_files(self):
        ...
        # Allow setup.py and similar standard files
        ...
        self.assertEqual(
            ...
            f"Found unexpected Python files in root: {[f.name for f in unexpected_files]}"
        )
    ...
    def test_clean_separation_of_concerns(self):
        ...
        # Should not contain any .py files
        py_files = list(models_dir.rglob('*.py'))
        self.assertEqual(
            len(py_files), 0,
            f"Models directory should contain only GNN models (data), not Python code. Found: {py_files}"
        )
        
        # Should contain .gnn.md files
        gnn_files = list(models_dir.rglob('*.gnn.md'))
        self.assertGreater(
            len(gnn_files), 0,
            "Models directory should contain GNN model files (*.gnn.md)"
        )
    ...
    def test_architecture_screams_active_inference(self):
        ...
        # Check for Active Inference specific modules
        ai_indicators = [
            self.root_dir / 'src' / 'agents' / 'active_inference.py',
            self.root_dir / 'doc' / 'active_inference_guide.md',
        ]
        ...
        # Check that main.py mentions Active Inference
        main_path = self.root_dir / 'src' / 'main.py'
        with open(main_path) as f:
            content = f.read()
...
class TestGNNModelParsing(unittest.TestCase):
    ...
    def test_can_parse_base_models(self):
        ...
        parser = GNNParser()
        model_files = [
            self.root_dir / 'models' / 'base' / 'explorer.gnn.md',
            self.root_dir / 'models' / 'base' / 'merchant.gnn.md',
        ]
    ...
    def test_can_validate_models(self):
        ...
        if model_file.exists():
            try:
                ...
                self.assertTrue(
                    result.is_valid,
                    f"Model validation failed: {result.errors}"
                )
            except Exception as e:
    ...
    def test_pipeline_has_all_stages(self):
        ...
        for stage in required_stages:
            self.assertIn(
                stage,
                pipeline.stages,
                f"Required stage '{stage}' not found in pipeline"
            )
