...
class TestPerformanceMetrics:
    ...
    async def test_agent_creation_performance(self, performance_logger):
        ...
        for count in agent_counts:
            ...
            for i in range(count):
                agent = Agent(agent_id=f'agent_{i}', name=f'Agent{i}', agent_class=AgentClass.EXPLORER, initial_position=(i % 10, i // 10))
                agents.append(agent)
            creation_time = time.time() - start_time
            creation_times.append(creation_time)
            performance_logger.log('agent_creation', {'count': count, 'total_time': creation_time, 'time_per_agent': creation_time / count})
        ...
        for resolution in resolutions:
            ...
            for cell in cells[:1000]:
                ...
                if np.random.random() < 0.3:
                    world.add_resource(cell, np.random.choice(['food', 'water', 'materials']), np.random.randint(1, 20))
            generation_time = time.time() - start_time
            generation_times.append(generation_time)
            performance_logger.log('world_generation', {'resolution': resolution, 'cell_count': len(cells), 'generation_time': generation_time, 'time_per_cell': generation_time / min(1000, len(cells))})
    ...
    async def test_pathfinding_performance(self, performance_logger):
        ...
        path_test_cases = [(5, 'short'), (20, 'medium'), (50, 'long'), (100, 'very_long')]
        results = []
        for target_length, category in path_test_cases:
            ...
            if times:
                ...
                results.append({'category': category, 'target_length': target_length, 'avg_time': avg_time, 'trials': len(times)})
                performance_logger.log('pathfinding', {'category': category, 'avg_time': avg_time, 'max_time': max(times), 'min_time': min(times)})
        if len(results) >= 2:
    ...
    async def test_message_system_throughput(self, performance_logger):
        ...
        agents = []
        for i in range(20):
            agent = Agent(agent_id=f'agent_{i}', name=f'Agent{i}', agent_class=AgentClass.EXPLORER, initial_position=(i % 5, i // 5), message_system=message_system)
            agents.append(agent)
        ...
        for rate in message_rates:
            ...
            while time.time() - start_time < duration:
                ...
                if sender != receiver:
                    await sender.send_message(receiver.agent_id, MessageType.TEXT, f'Test message {messages_sent}')
                    messages_sent += 1
            ...
            performance_logger.log('message_throughput', {'target_rate': rate, 'messages_sent': messages_sent, 'messages_received': messages_received, 'actual_throughput': throughput, 'delivery_rate': delivery_rate})
    ...
    async def test_knowledge_graph_operations(self, performance_logger):
        ...
        for count in node_counts:
            start_time = time.time()
            for i in range(count):
                knowledge_graph.add_node(f'node_{i}', node_type='concept', data={'value': i, 'timestamp': time.time()})
            ...
            for i in range(count // 10):
                ...
                if node1 != node2:
                    knowledge_graph.add_edge(node1, node2, edge_type='related', weight=np.random.random())
            ...
            performance_logger.log('knowledge_graph', {'node_count': count, 'insertion_time': insertion_time, 'insertion_per_node': insertion_time / count, 'edge_creation_time': edge_time, 'avg_query_time': avg_query_time})
            knowledge_graph = KnowledgeGraph()
    ...
    async def test_simulation_cycle_performance(self, performance_logger):
        ...
        configurations = [{'agents': 10, 'world_size': 50}, {'agents': 50, 'world_size': 100}, {'agents': 100, 'world_size': 200}]
        for config in configurations:
            engine = SimulationEngine({'world': {'resolution': 5, 'size': config['world_size']}, 'agents': {'count': config['agents']}, 'simulation': {'max_cycles': 10}})
            ...
            performance_logger.log('simulation_cycle', {'agent_count': config['agents'], 'world_size': config['world_size'], 'avg_cycle_time': avg_cycle_time, 'max_cycle_time': max(cycle_times), 'min_cycle_time': min(cycle_times), 'max_memory_mb': max_memory, 'memory_growth_mb': memory_growth})
            await engine.stop()
...
class TestScalabilityLimits:
    ...
    async def test_maximum_agent_capacity(self):
        ...
        for count in test_counts:
            try:
                config = {'world': {'resolution': 6, 'size': count * 2}, 'agents': {'count': count}, 'simulation': {'max_cycles': 5}}
    ...
    async def test_concurrent_operations(self):
        ...
        engine = SimulationEngine({'world': {'resolution': 5, 'size': 100}, 'agents': {'count': 50}, 'simulation': {'max_cycles': 20}})
        await engine.initialize()
        await engine.start()
...
class TestMemoryAndResourceUsage:
    ...
    async def test_memory_leak_detection(self):
        ...
        engine = SimulationEngine({'world': {'resolution': 5, 'size': 50}, 'agents': {'count': 20}, 'simulation': {'max_cycles': 100}})
        await engine.initialize()
        await engine.start()
        process = psutil.Process()
        memory_samples = []
    ...
    async def test_resource_cleanup(self):
        ...
        for _ in range(5):
            engine = SimulationEngine({'world': {'resolution': 5, 'size': 50}, 'agents': {'count': 10}, 'simulation': {'max_cycles': 10}})
            await engine.initialize()
            await engine.start()
            for _ in range(10):
                await engine.step()
