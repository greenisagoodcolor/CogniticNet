...
from .......world.spatial.--init-- import SpatialAPI, SpatialCoordinate, ResourceType, ResourceDistribution, ObservationModel, H3World, Biome, TerrainType
...
class TestSpatialIntegration:
    ...
    def test_pathfinding_respects_terrain(self, spatial_api, h3_world):
        ...
        obstacles = {hex_id for hex_id, cell in h3_world.cells.items() if cell.terrain == TerrainType.WATER}
    ...
    def test_resource_distribution_in_world(self, spatial_api, h3_world):
        ...
        forest_centers = [hex_id for hex_id, cell in h3_world.cells.items() if cell.biome == Biome.FOREST][:3]
        food_dist = ResourceDistribution(resource_type=ResourceType.FOOD, base_amount=50.0, variance=10.0, distribution_pattern='clustered', cluster_centers=forest_centers)
    ...
    def test_visibility_with_elevation(self, spatial_api, h3_world):
        ...
        if mountain_hex:
            ...
            visible = spatial_api.get_visible_hexes(mountain_hex, mountain_cell.elevation, terrain_heights)
            ...
            if flat_hex:
                ...
                flat_visible = spatial_api.get_visible_hexes(flat_hex, flat_cell.elevation, terrain_heights)
    ...
    def test_find_nearest_resource_in_world(self, spatial_api, h3_world):
        ...
        for hex_id, cell in h3_world.cells.items():
            if cell.biome == Biome.DESERT:
                ...
                break
        if start_hex:
            nearest_water = spatial_api.find_nearest_resource(start_hex, ResourceType.WATER, world_resources, max_distance=20)
            if nearest_water:
