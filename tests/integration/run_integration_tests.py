...
class IntegrationTestRunner:
    ...
    def run_test_suite(
        self, suite_name: str, test_file: str, markers: List[str] = None
    ) -> Dict[str, Any]:
        """
        Run a test suite.
    ...
    def run_all_tests(self, include_performance: bool = False):
        ...
        if include_performance:
            test_suites.append(
                ("Performance Tests", "test_performance.py", ["performance"])
            )
    ...
    def _generate_summary(self):
        ...
        total_failed = sum(t.get("failed", 0) for t in self.results["tests"].values())
        total_skipped = sum(t.get("skipped", 0) for t in self.results["tests"].values())
        total_duration = sum(
...
"""
        ...
        for suite_name in self.results["tests"]:
            ...
            if (self.output_dir / report_name).exists():
                html_content += f'        <li><a href="{report_name}">{suite_name} Detailed Report</a></li>\n'
...
def setup_test_environment():
    ...
    for req in requirements:
        subprocess.run(
            [sys.executable, "-m", "pip", "install", req], capture_output=True
        )
...
def main():
    ...
    parser.add_argument(
        "--setup", action="store_true", help="Set up test environment first"
    )
