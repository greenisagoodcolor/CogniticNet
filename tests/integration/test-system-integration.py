...
from .......world.h3-world import H3World
...
class TestFullSystemIntegration:
    ...
    async def simulation_engine(self):
        ...
        config = {'world': {'resolution': 5, 'size': 100}, 'agents': {'count': 10, 'distribution': {'explorer': 4, 'merchant': 3, 'scholar': 2, 'guardian': 1}}, 'simulation': {'max_cycles': 100, 'time_step': 1.0}}
    ...
    async def test_multi_agent_ecosystem_dynamics(self, simulation_engine):
        ...
        await simulation_engine.start()
        ecosystem_metrics = {'resource_distribution': [], 'agent_wealth': [], 'knowledge_density': [], 'trade_volume': [], 'exploration_coverage': []}
        ...
        resource_trend = np.polyfit(range(len(ecosystem_metrics['resource_distribution'])), ecosystem_metrics['resource_distribution'], 1)[0]
        assert resource_trend < 0
        knowledge_trend = np.polyfit(range(len(ecosystem_metrics['knowledge_density'])), ecosystem_metrics['knowledge_density'], 1)[0]
    ...
    async def test_system_performance_under_load(self):
        ...
        config = {'world': {'resolution': 6, 'size': 500}, 'agents': {'count': 100, 'distribution': {'explorer': 40, 'merchant': 30, 'scholar': 20, 'guardian': 10}}, 'simulation': {'max_cycles': 20, 'time_step': 1.0}}
        engine = SimulationEngine(config)
        await engine.initialize()
        performance_metrics = {'cycle_times': [], 'memory_usage': [], 'cpu_usage': [], 'message_latency': []}
    ...
    async def test_learning_and_adaptation(self, simulation_engine):
        ...
        adaptation_metrics = {'collective_knowledge': [], 'behavior_diversity': [], 'success_rates': [], 'pattern_count': []}
        for cycle in range(40):
            await simulation_engine.step()
            if cycle % 5 == 0:
                patterns = pattern_extractor.extract_patterns(simulation_engine.get_event_history())
        ...
        knowledge_growth = adaptation_metrics['collective_knowledge'][-1] - adaptation_metrics['collective_knowledge'][0]
        assert knowledge_growth > 0
        success_improvement = adaptation_metrics['success_rates'][-1] - adaptation_metrics['success_rates'][0]
    ...
    async def test_scalability_limits(self):
        ...
        for count in agent_counts:
            config = {'world': {'resolution': 6, 'size': count * 10}, 'agents': {'count': count}, 'simulation': {'max_cycles': 5}}
            try:
                engine = SimulationEngine(config)
                await engine.initialize()
                start_time = time.time()
                ...
                scalability_results.append({'agent_count': count, 'total_time': total_time, 'time_per_agent': total_time / count, 'success': True})
                await engine.stop()
            except Exception as e:
                scalability_results.append({'agent_count': count, 'success': False, 'error': str(e)})
...
class TestEdgeCasesAndStress:
    ...
    async def test_resource_scarcity_scenario(self):
        ...
        config = {'world': {'resolution': 5, 'size': 50, 'resource_density': 0.1}, 'agents': {'count': 20}, 'simulation': {'max_cycles': 50}}
        engine = SimulationEngine(config)
        await engine.initialize()
        await engine.start()
        survival_timeline = []
        ...
        for cycle in range(30):
            ...
            cooperation_count = sum((1 for e in events if e['type'] in ['trade', 'resource_share', 'alliance_formed']))
    ...
    async def test_information_overload(self):
        ...
        config = {'world': {'resolution': 5, 'size': 50}, 'agents': {'count': 10, 'communication_rate': 10.0}, 'simulation': {'max_cycles': 20}}
        engine = SimulationEngine(config)
        await engine.initialize()
        await engine.start()
        message_stats = {'dropped_messages': 0, 'processing_delays': [], 'queue_sizes': []}
        for _ in range(10):
            await engine.step()
        ...
        drop_rate = message_stats['dropped_messages'] / (sum(message_stats['queue_sizes']) + message_stats['dropped_messages'])
    ...
    async def test_rapid_environmental_changes(self):
        ...
        engine = SimulationEngine({'world': {'resolution': 5, 'size': 100}, 'agents': {'count': 20}, 'simulation': {'max_cycles': 50}})
        await engine.initialize()
        await engine.start()
        adaptation_scores = []
        conditions = [{'resource_multiplier': 1.0, 'hazard_level': 0.1}, {'resource_multiplier': 0.3, 'hazard_level': 0.5}, {'resource_multiplier': 2.0, 'hazard_level': 0.2}, {'resource_multiplier': 0.5, 'hazard_level': 0.8}]
...
class TestExportAndDeployment:
    ...
    async def test_agent_export_validation(self):
        ...
        engine = SimulationEngine({'world': {'resolution': 5, 'size': 50}, 'agents': {'count': 1}, 'simulation': {'max_cycles': 20}})
        await engine.initialize()
        await engine.start()
        for _ in range(20):
            await engine.step()
    ...
    async def test_multi_agent_deployment_package(self):
        ...
        engine = SimulationEngine({'world': {'resolution': 5, 'size': 100}, 'agents': {'count': 4}, 'simulation': {'max_cycles': 30}})
        await engine.initialize()
        await engine.start()
        for _ in range(30):
            await engine.step()
        deployment_path = Path('/tmp/test_deployment')
        success = await engine.export_deployment(deployment_path, include_world=True, include_history=True)
        assert success
