...
from agents.base.behaviors import (
    ...
    ExplorationBehavior, WanderBehavior
)
...
class Discovery:
    ...
    def __init__(self, 
        ...
        self.discovery_id = f"{discovery_type.value}_{position.x:.1f}_{position.y:.1f}_{datetime.now().timestamp()}"
        self.verified = False
        self.shared = False
...
class ExplorationMap:
    ...
    def __init__(self, grid_size: float = 5.0):
        ...
        self.paths: Dict[Tuple[int, int], Dict[Tuple[int, int], float]] = {}  # From -> To -> Distance
...
class AdvancedExplorationBehavior(BaseBehavior):
    ...
    def _check_for_discoveries(self, agent: Agent, position: Position, context: Dict[str, Any]) -> List[Discovery]:
...
class PathfindingBehavior(BaseBehavior):
    ...
    def _execute_custom(self, agent: Agent, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        destination_goals.sort(
            key=lambda g: current_pos.distance_to(g.target_position)
        )
...
class ExplorerAgent(BaseAgent):
    ...
    def set_exploration_target(self, target_position: Position, description: str = "Explore target area"):
...
def create_explorer_agent(name: str = "Explorer", 
                        position: Optional[Position] = None,
                        **kwargs) -> ExplorerAgent:
...
def register_explorer_type():
    ...
    factory.set_default_config("explorer", {
        "agent_type": "explorer",
        "capabilities": {
            AgentCapability.MOVEMENT,
            AgentCapability.PERCEPTION,
