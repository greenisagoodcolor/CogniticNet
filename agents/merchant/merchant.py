...
from agents.base import (
    BaseAgent, Agent, Position, AgentCapability, AgentGoal,
    create_agent, get_default_factory, get_default_registry
)
from agents.base.behaviors import (
    BaseBehavior, BehaviorPriority, BehaviorStatus, 
    SocialInteractionBehavior
)
from agents.base.personality_system import create_personality_profile
...
class Market:
    ...
    def update_market_trends(self):
        ...
        for resource_type in self.market_trends:
            ...
            self.market_trends[resource_type] = max(-1.0, min(1.0, 
                self.market_trends[resource_type] + change))
...
class TradingBehavior(BaseBehavior):
    ...
    def _execute_trading_strategy(self, agent: Agent, market: Market, personality_profile) -> Dict[str, Any]:
    ...
    def _assess_resource_situation(self, agent: Agent) -> Tuple[Dict[ResourceType, float], Dict[ResourceType, float]]:
    ...
    def _create_sell_offer(self, agent: Agent, market: Market, surpluses: Dict[ResourceType, float], personality_profile) -> Dict[str, Any]:
        ...
        wanted_resources = self._determine_wanted_resources(market, total_offer_value * 0.9)  # Accept 90% value
    ...
    def _find_buy_opportunities(self, agent: Agent, market: Market, needs: Dict[ResourceType, float], personality_profile) -> Dict[str, Any]:
    ...
    def _determine_wanted_resources(self, market: Market, target_value: float) -> Dict[ResourceType, float]:
    ...
    def _evaluate_trade_offer(self, offer: TradeOffer, needs: Dict[ResourceType, float], market: Market, personality_profile) -> float:
    ...
    def _evaluate_market_conditions(self, agent: Agent, market: Market) -> Dict[str, Any]:
        ...
        agent.add_to_memory({
            ...
            'worst_trend': {'resource': worst_trend[0].value, 'trend': worst_trend[1]},
...
class MarketAnalysisBehavior(BaseBehavior):
    ...
    def _can_execute_custom(self, agent: Agent, context: Dict[str, Any]) -> bool:
        ...
        trade_memories = [m for m in agent.short_term_memory + agent.long_term_memory 
...
class MerchantAgent(BaseAgent):
    ...
    def create_trade_offer(self, offered_resources: Dict[ResourceType, float], 
                          wanted_resources: Dict[ResourceType, float],
                          expiration_hours: int = 6) -> str:
...
def create_merchant_agent(name: str = "Merchant",
                        position: Optional[Position] = None,
                        **kwargs) -> MerchantAgent:
...
def register_merchant_type():
    ...
    factory.set_default_config("merchant", {
        "agent_type": "merchant",
        "capabilities": {
            AgentCapability.COMMUNICATION,
            AgentCapability.RESOURCE_MANAGEMENT,
