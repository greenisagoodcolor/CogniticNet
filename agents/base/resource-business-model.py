...
class ResourceInventory:
    ...
    def __init__(self, agent_id: str, initial_capacity: float = 1000.0):
    ...
    def add_resource(self, resource_unit: ResourceUnit) -> bool:
        ...
        logger.debug(f"Added {resource_unit.quantity} {resource_unit.resource_type.value} to {self.agent_id}")
        return True
    ...
    def degrade_resources(self) -> None:
        ...
        for resource_type, units in self.resources.items():
            ...
            self.resources[resource_type] = [
                ...
                if unit.durability > 0.0 and unit.quality > 0.0
            ]
    ...
    def get_inventory_summary(self) -> Dict[ResourceType, Dict[str, float]]:
        ...
        for resource_type, units in self.resources.items():
            ...
            avg_quality = sum(unit.quality * unit.quantity for unit in units) / total_quantity if total_quantity > 0 else 0
            avg_durability = sum(unit.durability * unit.quantity for unit in units) / total_quantity if total_quantity > 0 else 0
...
class Market:
    ...
    def __init__(self, market_id: str, location: Optional[str] = None):
    ...
    def find_matching_offers(self, agent_id: str, wanted: Dict[ResourceType, float],
                           offered: Dict[ResourceType, float]) -> List[TradeOffer]:
    ...
    def get_market_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_transactions": len([t for t in self.transaction_history if
                                     (datetime.now(timezone.utc) - t.timestamp).days < 1]),
            "price_summary": {
                rt.value: {
...
class ResourceBusinessManager:
    ...
    def create_trade_offer(self, agent_id: str, offered: Dict[ResourceType, float],
                          requested: Dict[ResourceType, float], duration_hours: float = 24.0,
                          target_agent: Optional[str] = None) -> Optional[TradeOffer]:
    ...
    def execute_transaction(self, market_id: str, offer_id: str, buyer_id: str) -> Optional[Transaction]:
    ...
    def get_system_summary(self) -> Dict[str, Any]:
        ...
        return {
            "markets": {market_id: market.get_market_summary()
                       for market_id, market in self.markets.items()},
            "total_agents": len(self.inventories),
            "total_trade_relationships": sum(len(relationships)
                                           for relationships in self.trade_relationships.values()),
            "active_offers": sum(len(market.active_offers) for market in self.markets.values()),
            "recent_transactions": sum(len([t for t in market.transaction_history
...
def create_basic_inventory(agent_id: str, starting_resources: Optional[Dict[ResourceType, float]] = None) -> ResourceInventory:
