    ...
    def _execute_custom(self, agent: Agent, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if goal.target_position:
            ...
            if distance < 1.0:
                ...
                return {'success': True, 'goal_completed': True, 'goal': goal.description}
...
class SocialInteractionBehavior(BaseBehavior):
    ...
    def _execute_custom(self, agent: Agent, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        from .data_model import SocialRelationship
        if not relationship:
    ...
    def _perform_interaction(self, agent: Agent, relationship: SocialRelationship) -> Dict[str, Any]:
...
class ExplorationBehavior(BaseBehavior):
    ...
    def _can_execute_custom(self, agent: Agent, context: Dict[str, Any]) -> bool:
        ...
        return (
            agent.personality.openness > 0.6 or
            agent.agent_type == "explorer" or
            len(agent.goals) == 0  # Explore when no goals
        )
        ...
        for memory in agent.short_term_memory + agent.long_term_memory:
            if 'position' in memory.get('experience', {}):
                ...
                if isinstance(pos_data, list) and len(pos_data) >= 2:
                    explored_positions.append(Position(pos_data[0], pos_data[1], pos_data[2] if len(pos_data) > 2 else 0))

        return explored_positions

    def _find_best_exploration_direction(self, agent: Agent, explored_areas: List[Position]) -> float:
        ...
        for direction in directions:
            ...
            nearby_count = sum(
                1 for pos in explored_areas
                if pos.distance_to(test_position) < 3.0
            )
