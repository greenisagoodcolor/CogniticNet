...
class IntegrationMode(Enum):
    ...
    FULL = "full"  # Full active inference control
    HYBRID = "hybrid"  # Combine basic agent and active inference
    ADVISORY = "advisory"  # Active inference provides suggestions only
...
class ActiveInferenceConfig:
    ...
    belief_update_rate: float = 1.0  # Hz
    action_selection_threshold: float = 0.7
    learning_rate: float = 0.01

    # Visualization
    enable_visualization: bool = True
    visualization_update_rate: float = 10.0  # Hz
...
class StateToBeliefMapper:
    ...
    def _calculate_state_dimensions(self) -> Dict[str, int]:
        ...
        return {
            ...
            "resources": 3,  # energy, health, memory
            "goals": 5,  # Max tracked goals
            "social": 10,  # Social relationship features
        }
    ...
    def _encode_goals(self, goals: List[AgentGoal]) -> List[float]:
        ...
        for i in range(max_goals):
            if i < len(goals):
                goal = goals[i]
                # Encode priority, progress, deadline proximity
...
class PerceptionToObservationMapper:
    ...
    def _encode_visual_percepts(self, percepts: List[Percept]) -> np.ndarray:
        ...
        for i, percept in enumerate(percepts[:5]):  # Top 5 most salient
            if i * 4 < len(features):
                # Encode type, position, confidence
    ...
    def _encode_auditory_percepts(self, percepts: List[Percept]) -> np.ndarray:
        ...
        for i, percept in enumerate(percepts[:3]):  # Top 3 sounds
            if i * 3 < len(features):
    ...
    def _encode_proximity_percepts(self, percepts: List[Percept]) -> np.ndarray:
        ...
        if percepts:
            features[0] = min(len(percepts) / 10.0, 1.0)  # Normalized count
            features[1] = max(p.salience for p in percepts)  # Max salience
...
class ActionMapper:
    ...
    def map_to_agent_action(self, action_idx: int, agent: Agent) -> Action:
        ...
        action = Action(
            ...
            utility=1.0,  # Will be updated by active inference
            cost=0.1
        )
    ...
    def _get_action_parameters(self, action_type: ActionType, agent: Agent) -> Dict[str, Any]:
        ...
        if action_type == ActionType.MOVE:
            # Generate movement direction based on current goals
    ...
    def _calculate_movement_direction(self, agent: Agent) -> np.ndarray:
        ...
        # Simple implementation - move toward first goal position
    ...
    def _find_interaction_target(self, agent: Agent) -> Optional[str]:
        ...
        # Could be enhanced with perception data
        return None
...
class ActiveInferenceIntegration:
    ...
    def update(self, dt: float):
        ...
        if self.current_belief is None:
            ...
            self.current_belief = self.belief_updater.update_belief(
                self.current_belief,
                current_observation,
                self.last_action
            )
        ...
        # Update precision
        precision = self.precision_optimizer.optimize(
            self.current_belief,
            current_observation
        )
    ...
    def _initialize_belief(self, state: np.ndarray) -> np.ndarray:
        ...
        # Simple initialization - could be enhanced
    ...
    def _select_action_hybrid(self) -> Optional[Action]:
        ...
        # Combine based on confidence and utility
        if ai_action and basic_action:
            # Prefer active inference if confidence is high
    ...
    def _select_action_advisory(self) -> Optional[Action]:
        ...
        # Get basic agent decision
        basic_action = self.decision_system.decide(self.agent, self.perception_system)

        # Get active inference suggestion
        ai_action = self._select_action_full()

        # Log suggestion for debugging/visualization
        if ai_action:
            logger.debug(f"Active Inference suggests: {ai_action.action_type}")

        # Always use basic agent action in advisory mode
        return basic_action

    def _select_action_learning(self) -> Optional[Action]:
        """Select action focused on learning"""
        # Prioritize exploratory actions
    ...
    def _calculate_action_confidence(self, action: Action) -> float:
        ...
        # Based on belief entropy and expected free energy
    ...
    def _calculate_expected_info_gain(self) -> float:
        ...
        # Simplified calculation based on belief uncertainty
        belief_entropy = -np.sum(self.current_belief * np.log(self.current_belief + 1e-10))
    ...
    def _execute_action(self, action: Action):
        ...
        # Update decision system to execute action
    ...
    def get_visualization_data(self) -> Dict[str, Any]:
        ...
        return {
            "belief_state": self.current_belief.tolist() if self.current_belief is not None else None,
            "last_observation": self.last_observation.tolist() if self.last_observation is not None else None,
            "last_action": self.last_action.action_type.value if self.last_action else None,
            "mode": self.config.mode.value,
            "belief_entropy": -np.sum(self.current_belief * np.log(self.current_belief + 1e-10)) if self.current_belief is not None else None
        }
