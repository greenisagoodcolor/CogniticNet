...
class MovementConstraints:
    ...
    mode_speeds: Dict[MovementMode, float] = field(default_factory=lambda: {MovementMode.WALKING: 1.0, MovementMode.RUNNING: 2.0, MovementMode.SNEAKING: 0.5, MovementMode.JUMPING: 1.5, MovementMode.FLYING: 3.0, MovementMode.SWIMMING: 0.8})
    terrain_speeds: Dict[TerrainType, float] = field(default_factory=lambda: {TerrainType.GROUND: 1.0, TerrainType.WATER: 0.3, TerrainType.AIR: 1.0, TerrainType.ROUGH: 0.6, TerrainType.IMPASSABLE: 0.0})
...
class CollisionSystem:
    ...
    def check_collision(self, position: Position, radius: float, exclude_id: Optional[str]=None) -> bool:
...
class MovementController:
    ...
    def __init__(self, state_manager: AgentStateManager, collision_system: CollisionSystem, pathfinding_grid: PathfindingGrid):
    ...
    def register_agent(self, agent: Agent, constraints: Optional[MovementConstraints]=None) -> None:
    ...
    def _update_path_following(self, agent: Agent, state: MovementState, delta_time: float) -> None:
        ...
        if distance > 0:
            ...
            desired_velocity = direction * min(desired_speed, distance / delta_time)
    ...
    def _apply_movement(self, agent: Agent, state: MovementState, delta_time: float) -> None:
        ...
        if not self.collision_system.check_collision(new_position, constraints.collision_radius, agent.agent_id):
            ...
            normal = self.collision_system.get_collision_normal(new_position, constraints.collision_radius)
    ...
    def apply_force(self, agent_id: str, force: np.ndarray) -> None:
        ...
        if np.linalg.norm(acceleration) > constraints.max_acceleration:
            acceleration = acceleration / np.linalg.norm(acceleration) * constraints.max_acceleration
    ...
    def get_movement_info(self, agent_id: str) -> Optional[Dict]:
        ...
        if not agent:
            return None
        return {'position': {'x': agent.position.x, 'y': agent.position.y, 'z': agent.position.z}, 'velocity': state.velocity.tolist(), 'speed': float(np.linalg.norm(state.velocity)), 'mode': state.mode.value, 'is_grounded': state.is_grounded, 'has_path': state.path is not None, 'destination': {'x': state.destination.x, 'y': state.destination.y, 'z': state.destination.z} if state.destination else None}
...
class SteeringBehaviors:
    ...
    @staticmethod
    def arrive(position: np.ndarray, target: np.ndarray, max_speed: float, slowing_radius: float=5.0) -> np.ndarray:
    ...
    @staticmethod
    def wander(velocity: np.ndarray, wander_angle: float, wander_rate: float, max_speed: float) -> Tuple[np.ndarray, float]:
    ...
    @staticmethod
    def separate(position: np.ndarray, neighbors: List[np.ndarray], separation_radius: float, max_speed: float) -> np.ndarray:
