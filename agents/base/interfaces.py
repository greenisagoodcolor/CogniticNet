...
This module defines the core interfaces and abstract base classes for agent components,
following the clean architecture principles and ADR-003 dependency rules.
"""
...
class IAgentFactory(ABC):
    ...
    @abstractmethod
    def register_type(self, agent_type: str, factory_func) -> None:
...
class ICommunicationProtocol(ABC):
    ...
    @abstractmethod
    def broadcast_message(self, from_agent: str, message: Dict[str, Any], radius: Optional[float] = None) -> List[str]:
...
class IAgentEventHandler(ABC):
    ...
    @abstractmethod
    def on_agent_status_changed(self, agent: Agent, old_status: AgentStatus, new_status: AgentStatus) -> None:
