...
function generateFrontendCoverage() {
  ...
  if (!fs.existsSync("package.json")) {
    console.error(
      `${colors.red}No package.json found. Are you in the project root?${colors.reset}`,
    );
    return false;
  }
...
function generateBackendCoverage() {
  ...
  if (!commandExists("coverage")) {
    console.log(
      `${colors.yellow}Coverage.py not found. Installing...${colors.reset}`,
    );
  ...
  const success = execute("coverage run -m pytest src/tests/", {
    ignoreError: true,
  });
...
function mergeCoverageReports() {
  ...
  if (fs.existsSync(pythonXml) && commandExists("pycobertura")) {
    execute(
      `pycobertura convert ${pythonXml} -o ${path.join(mergedDir, "backend.lcov")}`,
      { ignoreError: true },
    );
  }

  console.log(
    `${colors.green}Coverage reports prepared for merging${colors.reset}`,
  );
}
...
function viewCoverageReport() {
  ...
  const pythonHtmlReport = path.join(
    process.cwd(),
    "coverage",
    "html",
    "index.html",
  ...
  if (fs.existsSync(htmlReport)) {
    console.log(
      `${colors.green}Opening frontend coverage report...${colors.reset}`,
    );
    execute(`open ${htmlReport}`, { ignoreError: true });
  } else {
    console.log(
      `${colors.yellow}No frontend coverage report found${colors.reset}`,
    );
  }

  if (fs.existsSync(pythonHtmlReport)) {
    console.log(
      `${colors.green}Opening backend coverage report...${colors.reset}`,
    );
    execute(`open ${pythonHtmlReport}`, { ignoreError: true });
  } else {
    console.log(
      `${colors.yellow}No backend coverage report found${colors.reset}`,
    );
  }
}
...
function uploadToCodecov() {
  ...
  if (!process.env.CODECOV_TOKEN) {
    console.log(
      `${colors.yellow}No CODECOV_TOKEN found in environment${colors.reset}`,
    );
    console.log(
      "Coverage upload skipped. Set CODECOV_TOKEN to enable uploads.",
    );
    return;
  }
  ...
  if (fs.existsSync(frontendLcov)) {
    execute(`npx codecov -f ${frontendLcov} -F frontend`, {
      ignoreError: true,
    });
  }
...
function generateCoverageBadge() {
  ...
  try {
    ...
    console.log(
      `${colors.green}Coverage badge generated: ${percentage}%${colors.reset}`,
    );
  } catch (error) {
    console.error(
      `${colors.red}Error generating badge: ${error.message}${colors.reset}`,
    );
  }
}
...
function showMenu() {
  ...
  rl.question("\nSelect an option: ", (answer) => {
    ...
    switch (answer) {
      ...
      case "7":
        console.log(
          `${colors.bright}Running full coverage workflow...${colors.reset}`,
        );
        generateFrontendCoverage();
        generateBackendCoverage();
        mergeCoverageReports();
...
if (args.length > 0) {
  switch (args[0]) {
    ...
    default:
      ...
      console.log(
        "Usage: coverage-report.js [frontend|backend|all|view|upload|badge|full]",
      );
  }
  rl.close();
} else {
