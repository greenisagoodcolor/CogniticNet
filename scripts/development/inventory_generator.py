...
class RepositoryInventory:
    ...
    reorganization_risk: str = 'UNKNOWN'
...
class InventoryGenerator:
    ...
    def __init__(self, project_root: Path, output_dir: Optional[Path]=None, include_external_deps: bool=True, strict_naming: bool=False):
        """
        ...
        self.dependency_analyzer = create_analyzer(str(self.project_root), include_external=include_external_deps)
        self.entry_detector = create_detector(str(self.project_root))
        self.naming_analyzer = create_naming_analyzer(str(self.project_root), strict_mode=strict_naming)
        self.start_time: Optional[datetime] = None
        self.error_count = 0
        self.warning_count = 0

    def generate_complete_inventory(self, save_intermediate: bool=True, max_files: Optional[int]=None) -> RepositoryInventory:
        """
        ...
        try:
            ...
            inventory = self._integrate_all_analyses(files, metadata_list, dependency_graph, entry_analysis, naming_analysis)
    ...
    def _perform_entry_point_analysis(self, metadata_list: List[ExtendedMetadata], dependency_graph: DependencyGraph) -> EntryPointAnalysis:
    ...
    def _integrate_all_analyses(self, files: List[FileInfo], metadata_list: List[ExtendedMetadata], dependency_graph: DependencyGraph, entry_analysis: EntryPointAnalysis, naming_analysis: NamingAnalysis) -> RepositoryInventory:
        """Integrate all analysis results into a complete inventory."""
        duration = (datetime.now(timezone.utc) - self.start_time).total_seconds()
        import sys
        inventory_metadata = InventoryMetadata(generation_timestamp=datetime.now(timezone.utc).isoformat(), project_root=str(self.project_root), total_files_analyzed=len(files), total_size_bytes=sum((f.size for f in files)), analysis_duration_seconds=duration, python_version=f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}', error_count=self.error_count, warning_count=self.warning_count)
        inventory = RepositoryInventory(metadata=inventory_metadata)
        inventory.files = [{'path': str(metadata.file_path), 'size_bytes': metadata.size_bytes, 'hash': getattr(metadata, 'hash', ''), 'language': metadata.language, 'file_type': metadata.file_type.value if metadata.file_type else 'unknown', 'is_binary': metadata.is_binary, 'is_test': metadata.is_test, 'last_modified': metadata.last_modified.isoformat() if metadata.last_modified else None, 'git_tracked': metadata.git_tracked, 'line_count': getattr(metadata, 'line_count', 0), 'dependencies': len([dep for dep in dependency_graph.edges if str(dep.source_file) == str(metadata.file_path)])} for metadata in metadata_list]
        inventory.file_hashes = {str(metadata.file_path): getattr(metadata, 'hash', '') for metadata in metadata_list}
    ...
    def _generate_summaries(self, inventory: RepositoryInventory, metadata_list: List[ExtendedMetadata]) -> None:
        ...
        size_ranges = [('0-1KB', 0, 1024), ('1KB-10KB', 1024, 10240), ('10KB-100KB', 10240, 102400), ('100KB-1MB', 102400, 1048576), ('1MB+', 1048576, float('inf'))]
        for metadata in metadata_list:
    ...
    def _assess_reorganization_risk(self, inventory: RepositoryInventory, entry_analysis: EntryPointAnalysis, naming_analysis: NamingAnalysis) -> None:
        ...
        if inventory.dependency_graph:
            for edge in inventory.dependency_graph.get('edges', []):
                ...
                if target and target not in inventory.protected_files:
                    dependents = sum((1 for e in inventory.dependency_graph.get('edges', []) if e.get('target') == target))
                    if dependents > 5:
    ...
    def _generate_recommendations(self, inventory: RepositoryInventory) -> None:
        ...
        if inventory.reorganization_risk == 'CRITICAL':
            ...
            action_items.append({'priority': 'critical', 'action': 'Create detailed reorganization plan', 'description': 'Break down reorganization into safe, testable stages', 'estimated_effort': 'high'})
            ...
            action_items.append({'priority': 'high', 'action': 'Validate all entry points', 'description': 'Ensure all critical entry points work after reorganization', 'estimated_effort': 'medium'})
        if inventory.entry_points:
            ...
            if critical_count > 0:
                ...
                action_items.append({'priority': 'high', 'action': 'Document entry point dependencies', 'description': 'Map all dependencies for critical entry points', 'estimated_effort': 'medium'})
        if inventory.dependency_graph:
            circular_deps = len(inventory.dependency_graph.get('circular_dependencies', []))
            if circular_deps > 0:
                recommendations.append(f'Resolve {circular_deps} circular dependencies before reorganization')
                action_items.append({'priority': 'high', 'action': 'Fix circular dependencies', 'description': 'Refactor code to eliminate circular import dependencies', 'estimated_effort': 'high'})
        if inventory.naming_analysis:
            ...
            if total_violations > 100:
                ...
                action_items.append({'priority': 'medium', 'action': 'Standardize naming conventions', 'description': 'Implement consistent naming across the codebase', 'estimated_effort': 'medium'})
        recommendations.extend(['Run all tests before and after reorganization', 'Update documentation to reflect new structure', 'Use version control to track all changes', 'Validate build processes after reorganization'])
        action_items.extend([{'priority': 'medium', 'action': 'Update CI/CD pipelines', 'description': 'Ensure build and deployment scripts work with new structure', 'estimated_effort': 'low'}, {'priority': 'low', 'action': 'Update documentation', 'description': 'Reflect new file locations in all documentation', 'estimated_effort': 'low'}])
...
def create_generator(project_root: str, output_dir: Optional[str]=None, **kwargs: Any) -> InventoryGenerator:
    """
    ...
    """
    return InventoryGenerator(project_root=Path(project_root), output_dir=Path(output_dir) if output_dir else None, **kwargs)
if __name__ == '__main__':
    ...
    inventory = generator.generate_complete_inventory(max_files=200)
