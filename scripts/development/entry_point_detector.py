...
class EntryPoint:
    ...
    def _generate_protection_recommendations(self) -> None:
        ...
        base_recommendations = ['Create comprehensive tests before moving', 'Update all import paths referencing this file', 'Verify functionality after relocation']
        type_specific = {EntryPointType.MAIN_APPLICATION: ['Test complete application startup', 'Verify command-line arguments parsing', 'Check environment variable handling'], EntryPointType.WEB_SERVER: ['Test all HTTP endpoints', 'Verify middleware chain', 'Check static file serving'], EntryPointType.API_ENDPOINT: ['Test all API routes', 'Verify authentication/authorization', 'Check request/response schemas'], EntryPointType.DATABASE_MIGRATION: ['Test migration rollback', 'Verify data integrity', 'Check schema consistency'], EntryPointType.CONFIGURATION: ['Validate all configuration values', 'Test environment-specific configs', 'Verify secret/credential handling']}
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'file_path': str(self.file_path), 'entry_type': self.entry_type.value, 'criticality': self.criticality.value, 'description': self.description, 'confidence': self.confidence, 'framework': self.framework, 'language': self.language, 'incoming_dependencies': list(self.incoming_dependencies), 'outgoing_dependencies': list(self.outgoing_dependencies), 'detection_method': self.detection_method, 'line_numbers': self.line_numbers, 'code_patterns': self.code_patterns, 'risk_factors': self.risk_factors, 'protection_recommendations': self.protection_recommendations}
...
class EntryPointAnalysis:
    ...
    def get_critical_entry_points(self) -> List[EntryPoint]:
        ...
        return [ep for ep in self.entry_points if ep.criticality in [CriticalityLevel.CRITICAL, CriticalityLevel.HIGH]]
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for JSON serialization"""
        return {'entry_points': [ep.to_dict() for ep in self.entry_points], 'criticality_summary': self.criticality_summary, 'framework_summary': self.framework_summary, 'risk_assessment': self.risk_assessment, 'total_entry_points': len(self.entry_points), 'critical_count': len(self.get_critical_entry_points())}
...
class EntryPointDetector:
    ...
    """
    PYTHON_MAIN_PATTERNS = ['if\\s+__name__\\s*==\\s*["\\\']__main__["\\\']', 'def\\s+main\\s*\\(', 'app\\.run\\s*\\(', 'uvicorn\\.run\\s*\\(', 'gunicorn', 'manage\\.py', 'wsgi\\.py', 'asgi\\.py']
    JS_MAIN_PATTERNS = ['app\\.listen\\s*\\(', 'server\\.listen\\s*\\(', 'express\\s*\\(\\)', 'fastify\\s*\\(\\)', 'process\\.argv', 'commander\\.program', 'yargs\\.argv']
    CONFIG_PATTERNS = {'requirements.txt': EntryPointType.CONFIGURATION, 'package.json': EntryPointType.CONFIGURATION, 'pyproject.toml': EntryPointType.CONFIGURATION, 'setup.py': EntryPointType.CONFIGURATION, 'Dockerfile': EntryPointType.BUILD_SCRIPT, 'docker-compose.yml': EntryPointType.BUILD_SCRIPT, 'Makefile': EntryPointType.BUILD_SCRIPT, '.env': EntryPointType.CONFIGURATION, 'settings.py': EntryPointType.CONFIGURATION, 'config.py': EntryPointType.CONFIGURATION, 'alembic.ini': EntryPointType.DATABASE_MIGRATION}
    FRAMEWORK_PATTERNS = {'django': {'patterns': ['django\\.', 'DJANGO_SETTINGS_MODULE', 'manage\\.py'], 'entry_files': ['manage.py', 'wsgi.py', 'asgi.py', 'settings.py'], 'criticality': CriticalityLevel.CRITICAL}, 'flask': {'patterns': ['from\\s+flask\\s+import', 'Flask\\s*\\(', 'app\\.run\\s*\\('], 'entry_files': ['app.py', 'main.py', 'wsgi.py'], 'criticality': CriticalityLevel.CRITICAL}, 'fastapi': {'patterns': ['from\\s+fastapi\\s+import', 'FastAPI\\s*\\(', 'uvicorn\\.run'], 'entry_files': ['main.py', 'app.py', 'asgi.py'], 'criticality': CriticalityLevel.CRITICAL}, 'express': {'patterns': ['express\\s*\\(\\)', 'app\\.listen', 'require\\s*\\(\\s*["\\\']express["\\\']'], 'entry_files': ['app.js', 'server.js', 'index.js', 'main.js'], 'criticality': CriticalityLevel.CRITICAL}, 'react': {'patterns': ['React\\.', 'from\\s+["\\\']react["\\\']', 'ReactDOM\\.render'], 'entry_files': ['index.js', 'App.js', 'main.js'], 'criticality': CriticalityLevel.HIGH}, 'pytest': {'patterns': ['import\\s+pytest', '@pytest\\.', 'conftest\\.py'], 'entry_files': ['conftest.py', 'pytest.ini'], 'criticality': CriticalityLevel.MEDIUM}}

    def __init__(self, project_root: Path, confidence_threshold: float=0.6):
        """
    ...
    def detect_entry_points(self, metadata_list: List[ExtendedMetadata], dependency_graph: Optional[DependencyGraph]=None) -> EntryPointAnalysis:
        """
    ...
    def _check_filename_patterns(self, metadata: ExtendedMetadata) -> List[EntryPoint]:
        ...
        if filename in self.CONFIG_PATTERNS:
            ...
            entry_points.append(EntryPoint(file_path=metadata.file_path, entry_type=entry_type, criticality=criticality, description=f'Configuration/build file: {filename}', confidence=0.9, language=metadata.language, detection_method='filename_pattern'))
        entry_names = {'main.py': (EntryPointType.MAIN_APPLICATION, CriticalityLevel.CRITICAL), 'app.py': (EntryPointType.WEB_SERVER, CriticalityLevel.CRITICAL), 'manage.py': (EntryPointType.CLI_SCRIPT, CriticalityLevel.CRITICAL), 'wsgi.py': (EntryPointType.WEB_SERVER, CriticalityLevel.CRITICAL), 'asgi.py': (EntryPointType.WEB_SERVER, CriticalityLevel.CRITICAL), 'index.js': (EntryPointType.MAIN_APPLICATION, CriticalityLevel.HIGH), 'server.js': (EntryPointType.WEB_SERVER, CriticalityLevel.CRITICAL), 'app.js': (EntryPointType.WEB_SERVER, CriticalityLevel.HIGH), '__init__.py': (EntryPointType.PACKAGE_INIT, CriticalityLevel.MEDIUM), 'conftest.py': (EntryPointType.TEST_RUNNER, CriticalityLevel.MEDIUM)}
        if filename in entry_names:
            entry_type, criticality = entry_names[filename]
            entry_points.append(EntryPoint(file_path=metadata.file_path, entry_type=entry_type, criticality=criticality, description=f'Entry point file: {filename}', confidence=0.8, language=metadata.language, detection_method='filename_pattern'))
        return entry_points
    ...
    def _check_python_patterns(self, metadata: ExtendedMetadata, content: str) -> List[EntryPoint]:
        ...
        for line_num, line in enumerate(lines, 1):
            if re.search(self.PYTHON_MAIN_PATTERNS[0], line):
                entry_points.append(EntryPoint(file_path=metadata.file_path, entry_type=EntryPointType.MAIN_APPLICATION, criticality=CriticalityLevel.CRITICAL, description="Python main entry point with __name__ == '__main__' guard", confidence=0.95, language='Python', detection_method='content_pattern', line_numbers=[line_num], code_patterns=[line.strip()]))
        main_func_pattern = 'def\\s+main\\s*\\('
        for line_num, line in enumerate(lines, 1):
            if re.search(main_func_pattern, line):
                entry_points.append(EntryPoint(file_path=metadata.file_path, entry_type=EntryPointType.MAIN_APPLICATION, criticality=CriticalityLevel.HIGH, description='Python main function definition', confidence=0.8, language='Python', detection_method='content_pattern', line_numbers=[line_num], code_patterns=[line.strip()]))
        web_patterns = [('app\\.run\\s*\\(', EntryPointType.WEB_SERVER, 'Flask app.run()'), ('uvicorn\\.run\\s*\\(', EntryPointType.WEB_SERVER, 'Uvicorn server run'), ('gunicorn', EntryPointType.WEB_SERVER, 'Gunicorn WSGI server')]
        for pattern, entry_type, description in web_patterns:
            for line_num, line in enumerate(lines, 1):
                if re.search(pattern, line):
                    entry_points.append(EntryPoint(file_path=metadata.file_path, entry_type=entry_type, criticality=CriticalityLevel.CRITICAL, description=description, confidence=0.9, language='Python', detection_method='content_pattern', line_numbers=[line_num], code_patterns=[line.strip()]))
        return entry_points
    ...
    def _check_js_patterns(self, metadata: ExtendedMetadata, content: str) -> List[EntryPoint]:
        ...
        entry_points = []
        lines = content.splitlines()
        server_patterns = [('app\\.listen\\s*\\(', EntryPointType.WEB_SERVER, 'Express app.listen()'), ('server\\.listen\\s*\\(', EntryPointType.WEB_SERVER, 'Server listen()'), ('express\\s*\\(\\)', EntryPointType.WEB_SERVER, 'Express application'), ('fastify\\s*\\(\\)', EntryPointType.WEB_SERVER, 'Fastify application')]
        for pattern, entry_type, description in server_patterns:
            for line_num, line in enumerate(lines, 1):
                if re.search(pattern, line):
                    entry_points.append(EntryPoint(file_path=metadata.file_path, entry_type=entry_type, criticality=CriticalityLevel.CRITICAL, description=description, confidence=0.9, language=metadata.language, detection_method='content_pattern', line_numbers=[line_num], code_patterns=[line.strip()]))
        cli_patterns = [('process\\.argv', EntryPointType.CLI_SCRIPT, 'Command-line argument processing'), ('commander\\.program', EntryPointType.CLI_SCRIPT, 'Commander.js CLI'), ('yargs\\.argv', EntryPointType.CLI_SCRIPT, 'Yargs CLI')]
        for pattern, entry_type, description in cli_patterns:
            for line_num, line in enumerate(lines, 1):
                if re.search(pattern, line):
                    entry_points.append(EntryPoint(file_path=metadata.file_path, entry_type=entry_type, criticality=CriticalityLevel.HIGH, description=description, confidence=0.8, language=metadata.language, detection_method='content_pattern', line_numbers=[line_num], code_patterns=[line.strip()]))
        return entry_points
        ...
        try:
            ...
            for framework, config in self.FRAMEWORK_PATTERNS.items():
                ...
                if framework_score > 0:
                    ...
                    entry_points.append(EntryPoint(file_path=metadata.file_path, entry_type=EntryPointType.FRAMEWORK_HOOK, criticality=config['criticality'], description=f'{framework.title()} framework entry point', confidence=confidence, framework=framework, language=metadata.language, detection_method='framework_pattern', code_patterns=matched_patterns))
        except Exception as e:
            self.logger.warning(f'Error checking framework patterns for {metadata.file_path}: {e}')
        return entry_points

    def _enhance_with_dependencies(self, entry_points: List[EntryPoint], dependency_graph: DependencyGraph) -> List[EntryPoint]:
        ...
        for entry_point in entry_points:
            ...
            if key not in seen:
                ...
                for existing in unique_entries:
                    if existing.file_path == entry_point.file_path and existing.entry_type == entry_point.entry_type:
    ...
    def _generate_risk_assessment(self, analysis: EntryPointAnalysis) -> Dict[str, Any]:
        ...
        return {'total_entry_points': len(analysis.entry_points), 'critical_entry_points': len(critical_points), 'risk_level': self._calculate_overall_risk(analysis), 'highest_risk_files': [str(ep.file_path) for ep in critical_points[:5]], 'framework_dependencies': analysis.framework_summary, 'recommendations': self._generate_recommendations(analysis)}
    ...
    def _generate_recommendations(self, analysis: EntryPointAnalysis) -> List[str]:
        ...
        recommendations = ['Create comprehensive backup before reorganization', 'Test all entry points before and after moves', 'Update documentation with new file locations']
        critical_points = analysis.get_critical_entry_points()
        if len(critical_points) > 5:
            recommendations.append(f'Special attention needed: {len(critical_points)} critical entry points detected')
    ...
    def _get_file_content(self, file_path: Path) -> str:
        ...
        if file_path not in self._content_cache:
            try:
                self._content_cache[file_path] = file_path.read_text(encoding='utf-8', errors='ignore')
            except Exception as e:
    ...
    def get_statistics(self, analysis: EntryPointAnalysis) -> Dict[str, Any]:
        ...
        return {'total_entry_points': len(analysis.entry_points), 'by_type': {entry_type.value: len(analysis.get_entry_points_by_type(entry_type)) for entry_type in EntryPointType}, 'by_criticality': analysis.criticality_summary, 'by_framework': analysis.framework_summary, 'risk_level': analysis.risk_assessment.get('risk_level', 'UNKNOWN'), 'average_confidence': sum((ep.confidence for ep in analysis.entry_points)) / max(len(analysis.entry_points), 1)}

def create_detector(project_root: str, confidence_threshold: float=0.6, **kwargs: Any) -> EntryPointDetector:
    """
    ...
    """
    return EntryPointDetector(project_root=Path(project_root), confidence_threshold=confidence_threshold, **kwargs)
