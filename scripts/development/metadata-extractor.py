...
class MetadataExtractor:
    ...
    def _extract_basic_metadata(self, file_info: FileInfo) -> ExtendedMetadata:
        ...
        try:
            ...
            self.logger.error(
                f"Error extracting basic metadata for {file_info.path}: {e}"
            )
            raise
    ...
    def _add_content_metadata(self, metadata: ExtendedMetadata) -> None:
        ...
        try:
            ...
            if not metadata.is_binary:
                try:
                    content = metadata.file_path.read_text(
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Could not decode text content for {metadata.file_path}"
                        )
                        return
                metadata.line_count = len(content.splitlines())
                metadata.char_count = len(content)
                if metadata.language:
                    metadata.syntax_valid = self._validate_syntax(
                        content, metadata.language
                    )
        except Exception as e:
            self.logger.warning(
                f"Error extracting content metadata for {metadata.file_path}: {e}"
            )
    ...
    def _add_git_metadata(self, metadata: ExtendedMetadata) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"Error extracting git metadata for {metadata.file_path}: {e}"
            )
    ...
    def _add_dependency_metadata(self, metadata: ExtendedMetadata) -> None:
        ...
        try:
            if metadata.is_binary or metadata.size_bytes > self.max_file_size:
                return
            content = metadata.file_path.read_text(
                encoding=metadata.encoding or "utf-8", errors="ignore"
            )
            ...
            self.logger.warning(
                f"Error extracting dependency metadata for {metadata.file_path}: {e}"
            )
    ...
    def get_metadata_statistics(
        self, metadata_list: List[ExtendedMetadata]
    ) -> Dict[str, Any]:
        """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python metadata_extractor.py <file_or_directory> [file2] [file3] ..."
        )
        sys.exit(1)
    logging.basicConfig(
        level=logging.INFO,
    ...
    extractor = create_extractor(
        extract_git=True, extract_content=True, extract_dependencies=True
    )
    all_files = []
    ...
    print(
        f"  Git tracked: {stats.get('git_tracked', 0)} ({stats.get('git_tracked_percentage', 0):.1f}%)"
    )
