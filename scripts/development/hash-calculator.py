...
class MD5Calculator:
    ...
    def __init__(
        self, buffer_size: Optional[int] = None, max_workers: Optional[int] = None
    ):
        """
    ...
    def calculate_hash(self, file_path: Path) -> HashResult:
        ...
        try:
            ...
            return HashResult(
                file_path=file_path,
                hash_value=None,
                status=HashStatus.PERMISSION_DENIED,
            ...
            return HashResult(
                file_path=file_path,
                hash_value=None,
                status=HashStatus.UNKNOWN_ERROR,
    ...
    def calculate_hashes_parallel(self, file_paths: List[Path]) -> List[HashResult]:
        ...
        if not file_paths:
            return []
        self.logger.info(
            f"Calculating hashes for {len(file_paths)} files using {self.max_workers} workers"
        )
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=self.max_workers
        ) as executor:
            future_to_path = {
                executor.submit(self.calculate_hash, path): path for path in file_paths
            }
            results = []
            completed = 0
            for future in concurrent.futures.as_completed(future_to_path):
                try:
                    ...
                    if completed % 100 == 0 or completed == len(file_paths):
                        self.logger.info(
                            f"Completed {completed}/{len(file_paths)} hash calculations"
                        )
                except Exception as e:
    ...
    def calculate_hashes_from_file_info(
    ...
    async def calculate_hashes_async(self, file_paths: List[Path]) -> List[HashResult]:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            results = await loop.run_in_executor(
                executor, self.calculate_hashes_parallel, file_paths
            )
        return results

    def get_duplicate_files(
        self, hash_results: List[HashResult]
    ) -> Dict[str, List[Path]]:
        """
    ...
    def get_statistics(
        ...
        calculation_times = [
            r.calculation_time for r in hash_results if r.calculation_time is not None
        ]
        status_counts = {}
        for result in hash_results:
        ...
        file_sizes = [
            r.file_size for r in successful_results if r.file_size is not None
        ]
        stats = {
            "total_files": len(hash_results),
            "successful": len(successful_results),
            "failed": len(hash_results) - len(successful_results),
            "success_rate": (
                len(successful_results) / len(hash_results) if hash_results else 0
            ),
            "status_breakdown": status_counts,
            "duplicates_found": len(self.get_duplicate_files(hash_results)),
        }
        if calculation_times:
            stats.update(
                {
                    "total_calculation_time": sum(calculation_times),
                    "average_calculation_time": sum(calculation_times)
...
def create_calculator(
    buffer_size: Optional[int] = None, max_workers: Optional[int] = None
) -> MD5Calculator:
    """
    ...
    try:
        if shutil.which("md5sum"):
            result = subprocess.run(
                ["md5sum", str(file_path)], capture_output=True, text=True, check=True
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python hash_calculator.py <file_or_directory> [file2] [file3] ..."
        )
        sys.exit(1)
    logging.basicConfig(
        level=logging.INFO,
    ...
    if failed_results:
        ...
        for result in failed_results[:10]:
            print(
                f"  {result.file_path}: {result.status.value} - {result.error_message}"
            )
