...
class NamingAuditor:
    ...
    def _check_typescript_file_name(self, filename: str, filepath: str):
        ...
        if "components" in filepath and filename.endswith(".tsx"):
            pattern = self.conventions["fileNaming"]["typescript"]["components"][
                "pattern"
            ]
    ...
    def audit_python_code(self, root_dir: str = "."):
        ...
        for root, dirs, files in os.walk(root_dir):
            if any(
                skip in root for skip in [".git", "node_modules", "__pycache__", "venv"]
            ):
                continue
        ...
        try:
            ...
            try:
                ...
                self.violations["syntax_errors"].append(
                    {"file": filepath, "issue": "Python syntax error - cannot parse"}
                )

        except Exception as e:
            self.violations["read_errors"].append(
                {"file": filepath, "issue": f"Error reading file: {str(e)}"}
            )
    ...
    def _check_typescript_patterns(self, filepath: str, content: str):
        ...
        component_pattern = (
            r"(?:export\s+)?(?:const|function)\s+([A-Za-z0-9_]+).*?:\s*(?:React\.)?FC"
        )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        sorted_categories = sorted(
            self.violations.items(), key=lambda x: severity_order.get(x[0], 99)
        )
        ...
        for category, violations in sorted_categories:
            if violations:
                ...
                if len(violations) > 10:
                    report["violations_by_category"][category]["total"] = len(
                        violations
                    )
    ...
    def _create_markdown_summary(self, report: Dict[str, Any], output_path: str):
        ...
        with open(output_path, "w") as f:
            ...
            f.write("**Date**: 2025-06-18\n")
            f.write("**Auditor**: Robert Martin (Clean Code Lead)\n\n")
            ...
            f.write(
                f"- Total violations found: {report['summary']['total_violations']}\n"
            )
            f.write(
                f"- Violation categories: {report['summary']['violation_categories']}\n\n"
            )
...
def main():
    ...
    print("   Report saved to: NAMING_AUDIT_REPORT.json")
    print("   Summary saved to: NAMING_AUDIT_REPORT.md")
