...
try:
    ...
    print("Please ensure that dependency_analyzer.py and metadata_extractor.py are accessible.")
    sys.exit(1)


def build_dependency_graph(project_root: Path, files_to_analyze: list[str]) -> nx.DiGraph:
    """
    ...
    for edge in dep_graph.edges:
        if edge.dependency_type == "internal":
            ...
            target_module_path = analyzer._resolve_python_module(edge.target_module, edge.source_file)
            if not target_module_path:
                 target_module_path = analyzer._resolve_js_module(edge.target_module, edge.source_file)
...
def main():
    ...
    try:
        ...
        print("Cannot determine a safe movement order. Please fix the circular dependencies.")
    ...
    for file_path in final_order:
        if file_path in migration_map:
            migration_plan.append({
                "source": file_path,
                "target": migration_map[file_path]
            })

    with open(plan_path, 'w') as f:
        json.dump(migration_plan, f, indent=2)

    print(f"Generated migration plan with {len(migration_plan)} movements at: {plan_path}")
