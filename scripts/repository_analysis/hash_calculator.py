...
class MD5Calculator:
    ...
    # Buffer size for reading large files (64KB for optimal performance)
    BUFFER_SIZE = 65536
    ...
    def calculate_hash(self, file_path: Path) -> HashResult:
        ...
        try:
            ...
            # Calculate hash using appropriate method
    ...
    def calculate_hashes_parallel(self, file_paths: List[Path]) -> List[HashResult]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all tasks
            future_to_path = {
                executor.submit(self.calculate_hash, path): path
    ...
    async def calculate_hashes_async(self, file_paths: List[Path]) -> List[HashResult]:
        ...
        # Run the parallel calculation in a thread pool
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
    ...
    def get_statistics(self, hash_results: List[HashResult]) -> Dict[str, Union[int, float, Dict[str, int]]]:
        ...
        stats = {
            ...
            "success_rate": len(successful_results) / len(hash_results) if hash_results else 0,
        ...
        if calculation_times:
            stats.update({
                ...
                "average_calculation_time": sum(calculation_times) / len(calculation_times),
