...
class NamingConvention(Enum):
    ...
    KEBAB_CASE = "kebab-case"          # my-file
    SCREAMING_SNAKE = "SCREAMING_SNAKE" # MY_CONSTANT
    MIXED = "mixed"                    # Inconsistent naming
...
class NamingViolation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "name": self.name,
            "expected_convention": self.expected_convention.value,
            "actual_convention": self.actual_convention.value,
            "line_number": self.line_number,
            "severity": self.severity,
            "surrounding_names": self.surrounding_names,
...
class NamingAnalysis:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "convention_summary": dict(self.convention_summary),
            "context_patterns": {
                context: dict(patterns)
...
class NamingAnalyzer:
    ...
    PYTHON_CONVENTIONS = {
        NamingContext.PYTHON_VARIABLE: NamingConvention.SNAKE_CASE,
        NamingContext.PYTHON_FUNCTION: NamingConvention.SNAKE_CASE,
        NamingContext.PYTHON_CLASS: NamingConvention.PASCAL_CASE,
        NamingContext.PYTHON_MODULE: NamingConvention.SNAKE_CASE,
        NamingContext.PYTHON_CONSTANT: NamingConvention.SCREAMING_SNAKE
    }

    JS_CONVENTIONS = {
        NamingContext.JS_VARIABLE: NamingConvention.CAMEL_CASE,
        NamingContext.JS_FUNCTION: NamingConvention.CAMEL_CASE,
        NamingContext.JS_CLASS: NamingConvention.PASCAL_CASE,
        NamingContext.JS_CONSTANT: NamingConvention.SCREAMING_SNAKE
    }

    FILE_CONVENTIONS = {
        NamingContext.FILE_NAME: NamingConvention.KEBAB_CASE,
        NamingContext.DIRECTORY_NAME: NamingConvention.KEBAB_CASE
    }

    # Naming patterns for detection
    NAMING_PATTERNS = {
        NamingConvention.SNAKE_CASE: r'^[a-z][a-z0-9_]*[a-z0-9]$',
        NamingConvention.CAMEL_CASE: r'^[a-z][a-zA-Z0-9]*$',
        NamingConvention.PASCAL_CASE: r'^[A-Z][a-zA-Z0-9]*$',
        NamingConvention.KEBAB_CASE: r'^[a-z][a-z0-9\-]*[a-z0-9]$',
        NamingConvention.SCREAMING_SNAKE: r'^[A-Z][A-Z0-9_]*[A-Z0-9]$'
    }
    ...
    def __init__(
        ...
        custom_conventions: Optional[Dict[NamingContext, NamingConvention]] = None
    ):
        """
        ...
        # Merge default and custom conventions
        self.conventions = {}
    ...
    def analyze_naming(self, metadata_list: List[ExtendedMetadata]) -> NamingAnalysis:
        ...
        # First pass: detect existing patterns
    ...
    def _detect_existing_patterns(self, metadata_list: List[ExtendedMetadata]) -> None:
        ...
        for metadata in metadata_list:
            try:
                ...
                # Analyze code content for Python/JS patterns
    ...
    def _analyze_path_patterns(self, file_path: Path) -> None:
        ...
        try:
            # Only analyze paths within our project root
            ...
            try:
                ...
                # Path is not under project root, skip
                return
            ...
            if '.' in filename:
                ...
                if name_part:  # Make sure we have a name
            ...
            # Analyze directory names in the relative path
            for part in relative_path.parts[:-1]:  # Exclude the filename
                if part and part != '.':  # Skip empty parts and current directory
    ...
    def _analyze_python_patterns(self, metadata: ExtendedMetadata) -> None:
        ...
        try:
            ...
            for match in re.finditer(class_pattern, content):
                ...
                if class_name:  # Ensure we have a valid name
            ...
            for match in re.finditer(func_pattern, content):
                ...
                if func_name and not func_name.startswith('__'):  # Skip magic methods
            ...
            for line in content.splitlines():
                ...
                if match:
                    ...
                    if var_name:  # Ensure we have a valid name
                        if var_name.isupper():
                            # Likely a constant
    ...
    def _analyze_js_patterns(self, metadata: ExtendedMetadata) -> None:
        ...
        try:
            ...
            for pattern in func_patterns:
                for match in re.finditer(pattern, content):
                    func_name = match.group(1)
                    if func_name:  # Ensure we have a valid name
                        convention = self._detect_naming_convention(func_name)
                        if convention != NamingConvention.UNKNOWN:
            ...
            var_patterns = [
                ...
                r'([A-Za-z_][A-Za-z0-9_]*)\s*:\s*[A-Za-z]'  # TypeScript type annotations
            ]
    ...
    def _analyze_file_naming(self, metadata: ExtendedMetadata) -> List[NamingViolation]:
        ...
        # Check content naming for code files
        if not metadata.is_binary and metadata.size_bytes < 5 * 1024 * 1024:
            if metadata.language == 'Python':
    ...
    def _check_file_naming(self, metadata: ExtendedMetadata) -> List[NamingViolation]:
        ...
        if expected_convention != actual_convention and actual_convention != NamingConvention.UNKNOWN:
            ...
            violations.append(NamingViolation(
                ...
                context=NamingContext.FILE_NAME,
                name=name_part,
        ...
        # Check directory names in path (only within project)
        try:
                ...
                for part in relative_path.parts[:-1]:  # Exclude filename
                    if part and part != '.':
                        ...
                        if expected_convention != actual_convention and actual_convention != NamingConvention.UNKNOWN:
                            ...
                            violations.append(NamingViolation(
                                file_path=metadata.file_path.parent,
                                context=NamingContext.DIRECTORY_NAME,
                                name=part,
                                expected_convention=expected_convention,
                                actual_convention=actual_convention,
                                severity=severity,
                                detection_method="directory_analysis"
                            ))
        except ValueError:
            # File not under project root, skip directory analysis
            pass
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if match:
                    ...
                    if expected != actual and actual != NamingConvention.UNKNOWN:
                        ...
                        violations.append(NamingViolation(
                            file_path=metadata.file_path,
                            context=NamingContext.PYTHON_CLASS,
                            name=class_name,
                            expected_convention=expected,
                            actual_convention=actual,
                            line_number=line_num,
                            severity=severity,
                            detection_method="python_ast_analysis",
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if match:
                    ...
                    if expected != actual and actual != NamingConvention.UNKNOWN:
                        ...
                        violations.append(NamingViolation(
                            file_path=metadata.file_path,
                            context=NamingContext.PYTHON_FUNCTION,
                            name=func_name,
                            expected_convention=expected,
                            actual_convention=actual,
                            line_number=line_num,
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if match:
                        ...
                        if expected != actual and actual != NamingConvention.UNKNOWN:
                            ...
                            violations.append(NamingViolation(
                                file_path=metadata.file_path,
                                context=NamingContext.JS_CLASS,
                                name=class_name,
                                expected_convention=expected,
                                actual_convention=actual,
                                line_number=line_num,
                                severity=severity,
                                detection_method="js_regex_analysis",
                                pattern_matched=line.strip()
                            ))
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if match:
                        ...
                        if expected != actual and actual != NamingConvention.UNKNOWN:
                            ...
                            violations.append(NamingViolation(
                                file_path=metadata.file_path,
                                context=NamingContext.JS_FUNCTION,
                                name=func_name,
                                expected_convention=expected,
                                actual_convention=actual,
                                line_number=line_num,
    ...
    def _calculate_severity(self, context: NamingContext, actual: NamingConvention) -> str:
        ...
        # Mixed naming is always problematic
        if actual == NamingConvention.MIXED:
            return "high"

        # File and directory naming affects organization
        if context in [NamingContext.FILE_NAME, NamingContext.DIRECTORY_NAME]:
            return "medium"

        # Variables and functions are lower priority
        return "low"
    ...
    def get_statistics(self, analysis: NamingAnalysis) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_severity": {
                "critical": len(analysis.get_violations_by_severity("critical")),
                "high": len(analysis.get_violations_by_severity("high")),
                "medium": len(analysis.get_violations_by_severity("medium")),
                "low": len(analysis.get_violations_by_severity("low"))
            },
            "by_context": {
                context.value: len(analysis.get_violations_by_context(context))
                for context in NamingContext
            },
            "detected_patterns": {
                context: dict(patterns)
                for context, patterns in self._detected_patterns.items()
            },
...
def convert_naming_convention(name: str, target: NamingConvention) -> str:
    ...
    # First, normalize the name by splitting it into words
    words = []
