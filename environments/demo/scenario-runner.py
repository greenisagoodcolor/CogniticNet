...
class ScenarioRunner:
    ...
    async def run_explorer_discovery(self, scenario: Dict[str, Any]):
        ...
        locations = ['Northern Valley', 'Eastern Ridge', 'Southern Plains', 'Western Forest']
        for location in locations:
            ...
            if random.random() > 0.3:
                resources = random.choice(['abundant food', 'fresh water', 'rare metals', 'ancient artifacts'])
                self.broadcast_scenario_event({
                    'scenario': 'explorer_discovery',
                    'phase': 'discovery',
                    'agent': explorer['name'],
    ...
    def update_agent_experience(self, agent_id: str, experience_type: str):
    ...
    def log_scenario_completion(self, scenario_id: str, agent_id: str):
        """Log scenario completion."""
        try:
            with self.db_conn.cursor() as cursor:
                # Update scenario
